...
theorem pseudoMassFromParamsAtPairFV_pow_succ_hasDeriv_abs_le_binding_core {α d : ℕ} (hα : 1 ≤ α)
  ...
  have hdiff : DifferentiableAt ℝ
    ...
    rw [hfun]
    exact (HasDerivAt.comp (h := fun β' => Ambient.correlationAlongExhaustion
        (IsingModel.latticeGraph d) (cubicExhaustion d)
        (⟨J, 0, β'⟩ : IsingParams ℝ) {x, z} n)
      (h₂ := pseudoMassExt hα hpos) β
      (pseudoMassExt_hasStrictDerivAt hα hpos hcorr).hasDerivAt hc_deriv).differentiableAt
