...
project-level correlation bounds for open layer slabs.  It also exposes wrappers
that discharge the denominator lower-bound hypothesis from boundary-vector
orthogonal spectral dominance.
...
/-- Physical open-slab orthogonal boundary-dominance hypotheses produce an open
min-gap certificate when the denominator lower bound is supplied by the
matching spectral denominator bridge. -/
noncomputable def
    layerOpenMinSpectralGapCertificate_of_layerOrthogonalBoundaryDominantBounds_spectralDenominator
    {S : Type*} [Fintype S] [DecidableEq S]
    (H : SimpleGraph S) [Fintype H.edgeSet] (transitionPairs : Finset (S × S))
    (p : IsingParams ℝ) (x : S)
    (spec : RealOrthogonalSpectralData
  ...
  layerOpenMinSpectralGapCertificate_of_orthogonalBoundaryDominantBounds
    (layerInternalWeight H p) (layerTransitionWeight transitionPairs p) (layerSpinAt x)
    (fun _ => Real.exp_pos _) spec top scale theta
    scale_pos theta_nonneg theta_lt_one partitionPrefactor_pos
    dominant_eigenvalue subdominant_abs_le central_dominant_channel_zero

/-- Orthogonal boundary-dominance hypotheses give project-level finite open-slab
same-transverse-site correlation decay, with the denominator lower bound
discharged by the spectral denominator bridge. -/
theorem
  ...
  let cert :
      LayerOpenMinSpectralGapCertificate
        ...
        (layerSpinAt x) :=
    layerOpenMinSpectralGapCertificate_of_layerOrthogonalBoundaryDominantBounds_spectralDenominator
      H transitionPairs p x spec top scale theta scale_pos theta_nonneg theta_lt_one
      partitionPrefactor_pos dominant_eigenvalue subdominant_abs_le
      central_dominant_channel_zero
  exact
    correlation_layerOpenSlabGraph_same_transverse_abs_le_of_openMinSpectralGapCertificate
      (S := S) H transitionPairs p x cert left sep right hsep
...
/-- Cubic transverse open slabs inherit the finite open spectral-decay consumer
whose denominator lower bound is supplied by the spectral denominator bridge. -/
theorem
    correlation_cubicLayerOpenSlabGraph_same_transverse_abs_le_of_boundarySpectralDenominator
    (d R : ℕ) (p : IsingParams ℝ) (x : CubicLayerSite d R)
    (spec : RealOrthogonalSpectralData
      (layerSymmetricTransferMatrix
        (layerInternalWeight (cubicLayerGraph d R) p)
        (layerTransitionWeight (cubicLayerTransitionPairs d R) p)))
    (top : LayerState (CubicLayerSite d R)) (scale theta : ℝ)
    (scale_pos : 0 < scale)
    (theta_nonneg : 0 ≤ theta)
    (theta_lt_one : theta < 1)
    (partitionPrefactor_pos :
  ...
  exact
    correlation_layerOpenSlabGraph_same_transverse_abs_le_of_boundarySpectralDenominator
      (S := CubicLayerSite d R) (cubicLayerGraph d R)
      (cubicLayerTransitionPairs d R) p x spec top scale theta
      scale_pos theta_nonneg theta_lt_one partitionPrefactor_pos
      dominant_eigenvalue subdominant_abs_le central_dominant_channel_zero
      left sep right hsep
