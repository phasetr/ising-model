...
/-!
# Bounded random-current expansion and N → ∞ convergence

`CurrentBounded`, spin-sum identities, the Taylor partial-sum expansion
(finite-N bounded expansion), and the N → ∞ convergence to the
unconditional capstone `pow_card_mul_iSup_weightSum_eq_sum_spinA_prod_exp`.
