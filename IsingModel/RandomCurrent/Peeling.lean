...
/-!
# Edge peeling and `weightSum_pair_le_edge_sum`

Connectivity prerequisites (even source-set cardinality, reachability),
edge-peeling weight bounds, and the main Simon-Lieb edge-peeling inequality
`weightSum {i,j} β J ≤ β*J * Σ_{e∋i} weightSum (symmDiff {i,j} endpoints(e))`.
