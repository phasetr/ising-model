...
Foundational definitions for the random-current representation:
the `Current` type, parity, sources, weights, and related predicates.

## Definitions

* `IsingModel.Ambient.Current` — ℕ-valued functions on induced-graph edges.
* `Current.parity`, `degreeAt`, `sources` — source-set infrastructure.
* `Current.weight`, `weightSum` — Boltzmann weight and its sum.
* `Current.IsSourceFree`, `HasSources` — predicates for the source set.
