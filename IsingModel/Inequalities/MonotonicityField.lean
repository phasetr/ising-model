...
The same argument goes through verbatim for the **boundary-condition** Gibbs
expectation `⟨·⟩^η_Λ` of `FKGBoundaryCondition.lean`: conditioning on agreement
with `η` off `Λ` preserves the Holley domination condition, because the set of
configurations agreeing with `η` off `Λ` is a sublattice (`agreesOff_inf` /
`agreesOff_sup`), and off that set the left-hand side vanishes.  This
boundary-condition version is the form consumed by the `+`/`−` extremal states
and the infinite-volume magnetization; it is generic in the graph `G` and carries
no lattice (`ℤ^d`) dependence.
...
* `boltzmannWeightBC_field_cross_supermodular` — the Holley domination condition
  for the `η`-boundary Boltzmann weights.
* `gibbsExpectationBC_field_mono_of_nonneg` — field monotonicity of the
  `η`-boundary expectation for nonnegative monotone observables.
* `gibbsExpectationBC_field_mono` — field monotonicity of the `η`-boundary
  expectation for arbitrary monotone observables.

Reference: Friedli–Velenik, *Statistical Mechanics of Lattice Systems* (2017),
§3.6.2 (FKG) and the Holley inequality, and §3.6 (monotonicity in the field,
Theorem 3.50); Glimm–Jaffe, *Quantum Physics*, §4.1.
-/
