...
/-!
# Sharp HLS convolution (1/3): shell reorganization and integral comparisons

Structural split (1/3) of `HLSConvolutionSharp`.  This child holds the shell
reorganization of a radial `ℝ≥0∞` sum over `ℤ^d` (at the origin and at an arbitrary
centre), the shell-cardinality power reduction `latticeSphere_card_mul_rpow_le`, and the
two finite-interval integral comparisons `sum_Ioc_nat_rpow_le` (tail, `e < -1`) and
`sum_Ioc_zero_nat_rpow_le` (head, `-1 < e`).  The ball/tail/region bounds built on these
live in the sibling `...RadialRegionBounds`, and the constant reduction together with the
headline decay bound in `...ConstantReductionCapstone`.  See the `HLSConvolutionSharp`
