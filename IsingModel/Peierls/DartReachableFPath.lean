...
the per-dart anchoring `hanchor` is constructed — those encode the degree-0/2/4 local crossing
analysis of FV §3.7.2 (cf. `SingleOrbitDegTwoPairing`, `SingleOrbitDegFourPairing`) and remain
the open geometric core. This file only isolates them as the exact remaining local obligations,
lowering the global `dartDualCut_isEdgeConnected_of_connected_filled` target to local, per-edge
statements.
...
into the two *local* obligations `hanchor` (same boundary site) and `hstep` (one `F`-edge). -/
...
produces `IsEdgeConnected (dartDualCut F)` from the local data `(φ, hanchor, hstep)` plus
`F`-connectivity, with no remaining global obligation — exactly the form to discharge the
single-orbit hypothesis once the local crossing analysis supplies `φ`, `hanchor`, `hstep`. -/
