...
/-!
# Dobrushin exponential spatial mixing (Issue #4214 §A)
...
set — the quantitative Dobrushin-uniqueness/decay-of-influence statement (Georgii Ch. 8). The
bound is uniform in the volume `Λ` (it does not appear on the right), the seed of the
infinite-volume uniqueness argument.
...
/-- **Per-distance Dobrushin locality bound**: for `f` local at `x₀`, the
...
/-- **Exponential spatial mixing**: if `f` is local at `x₀` and the boundary conditions
...
/-- **Exponential spatial mixing, internal disagreement support**: the same bound with
...
/-- **The Dobrushin influence bound vanishes exponentially**: for any constant `c`, the
