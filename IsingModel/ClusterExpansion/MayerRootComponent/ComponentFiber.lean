...
/-!
# Mayer K_n root-component recurrence (1/5): the root-component fibre

Structural split (1/5) of `IsingModel.ClusterExpansion.MayerRootComponent`.
This child holds the root-component vertex set `rootComponentFinset` together with
everything needed to characterise its fibre: component membership, self-membership, the
crossing-edge-free membership criterion, the inside/outside edge-count split, the `supp`
identification, the induced-graph identity, reachability confinement, inside connectivity,
the fibre characterisation `rootComponentFinset_eq_iff` and the `C.sym2` / `Cᶜ.sym2`
disjointness.  See the `IsingModel.ClusterExpansion.MayerRootComponent` facade module for
the full contents overview.
-/
