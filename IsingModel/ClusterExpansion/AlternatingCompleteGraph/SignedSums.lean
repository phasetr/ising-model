...
At the complete graph on `Fin n` this yields the boundary values used by the recurrence: `D`
vanishes whenever `2 ≤ n`, because `s(0, 1)` is then an edge, and equals `1` when `Fin n` is a
subsingleton. Separately, and for an arbitrary vertex type with no instance assumption, an edge
of an edge subset `S` never crosses connected components of the graph generated by `S`: for
`s(a, b) ∈ S` with `a ≠ b`, the two endpoints have the same connected component.
-/
