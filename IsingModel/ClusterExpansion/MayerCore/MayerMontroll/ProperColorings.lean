...
The first stage of the Mayer–Montroll regrouping.  It re-expresses each
log-Taylor term as a sum over vertex-disjoint compatible polymer-family tuples
(`logTaylor_eps_term_eq_sum_vdFamilyTuples`) and builds the local proper
surjective colouring universe (`IsProperColoring` / `properSurjectiveColorings`)
whose colour classes are exactly the compatible polymer families
(`colorClass_mem_vdCompatiblePolymerFamilies`).  Part of the `MayerMontroll`
identity split.
