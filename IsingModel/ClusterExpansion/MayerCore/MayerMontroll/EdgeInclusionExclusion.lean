...
The second stage of the Mayer–Montroll regrouping.  The proper indicator of a
colouring is expanded edge-by-edge (`proper_indicator_eq_signed_bad_edges`); the
resulting inner colour count is reduced to a surjection count on connected
components (`card_constantOnEdgeSet_surjective`) and collapsed via the surjective
log-weight identity, yielding the Mayer–Montroll colouring identity and its
rewriting of each Mayer expansion term into colouring form
(`clusterSeqActivity_eq_prod_colorClass`).  Part of the `MayerMontroll`
identity split.
