...
theorem freeEnergyComplexAlongExhaustion_branchData_localCover_real
    ...
    ∃ realFamily : LeeYangRealBranchLimitFamily G Λ p,
      ∃ g : ℂ → ℂ,
        ...
        DifferentiableOn ℂ g IsingModel.leeYangDomain ∧
        g (p.h : ℂ) = ((freeEnergyInfinite G Λ p : ℝ) : ℂ) := by
  rcases exists_leeYangRealBranchLimitFamily_of_branchData
      G Λ p hp hr hsub hbranch hconv hcompat hcenter with
    ⟨realFamily⟩
  exact ⟨realFamily,
    freeEnergyComplexAlongExhaustion_realBranchLimitFamily_localCover_real
      G Λ p hBED hd realFamily⟩
