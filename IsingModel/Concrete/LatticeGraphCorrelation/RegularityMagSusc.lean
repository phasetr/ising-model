...
/-!
# Concrete ℤ^d Λ-layer `susceptibilityΛ` `hasDerivAt` wrappers

Instantiates the Λ-level parameter derivatives of the susceptibility at
`IsingModel.latticeGraph d`, in the `β`, general-field `β`, `J` and field directions, stated
in existence form `∃ c : ℝ, HasDerivAt _ c _`.
-/
