...
/-!
# Substantive HLS states aliases bundle

GJ-proposition-unit bundle providing `states_*` style aliases.
...
/-! ## States-style aliases -/

/-- **States: HLS cluster**. -/
theorem states_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **States: HLS mass positive**. -/
theorem states_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
