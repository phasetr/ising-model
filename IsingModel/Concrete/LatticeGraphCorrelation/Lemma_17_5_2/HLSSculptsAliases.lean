...
/-!
# Substantive HLS sculpts aliases bundle

GJ-proposition-unit bundle providing `sculpts_*` style aliases.
...
/-! ## Sculpts-style aliases -/

/-- **Sculpts: HLS cluster**. -/
theorem sculpts_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Sculpts: HLS mass positive**. -/
theorem sculpts_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
