...
/-!
# Substantive HLS induces aliases bundle

GJ-proposition-unit bundle providing `induces_*` style aliases.
...
/-! ## Induces-style aliases -/

/-- **Induces: HLS cluster**. -/
theorem induces_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Induces: HLS mass positive**. -/
theorem induces_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
