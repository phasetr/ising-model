...
/-!
# Substantive HLS grants aliases bundle

GJ-proposition-unit bundle providing `grants_*` style aliases.
...
/-! ## Grants-style aliases -/

/-- **Grants: HLS cluster**. -/
theorem grants_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Grants: HLS mass positive**. -/
theorem grants_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
