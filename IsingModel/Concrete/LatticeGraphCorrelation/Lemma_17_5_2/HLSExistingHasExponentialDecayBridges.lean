...
/-!
# Existing HasExponentialDecay → substantive HLS sum bridges

GJ-proposition-unit bundle bridging the **existing** infrastructure:
- `hasExponentialDecay_of_high_temp` (`LatticeMassHighTemperature/ExpDecay.lean`):
  rate `-log(β·J·(2d))`, witness `C := 1/(1 - β·J·(2d))`.

with `tsum_truncated2Infinite_prod_le` directly. This yields a STRONGER
tsum decay rate than #3199 (which used `simonLiebRate β J d / 2`,
half the rate).
...
/-! ## Substantive tsum bound via existing high-temp HasExponentialDecay -/
