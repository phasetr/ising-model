...
/-!
# Substantive HLS formal aliases bundle

GJ-proposition-unit bundle providing `formal_*` style aliases.
...
/-! ## Formal-style aliases -/

/-- **Formal: HLS cluster**. -/
theorem formal_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Formal: HLS mass positive**. -/
theorem formal_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
