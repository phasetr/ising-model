...
/-!
# Substantive HLS statement aliases bundle

GJ-proposition-unit bundle providing `statement_*` style aliases.
...
/-! ## Statement-style aliases -/

/-- **Statement: HLS cluster**. -/
theorem statement_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Statement: HLS mass positive**. -/
theorem statement_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
...
/-- **Statement: HLS master**. -/
theorem statement_hls_master
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
