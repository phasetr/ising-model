...
/-!
# Substantive HLS asserts aliases bundle

GJ-proposition-unit bundle providing `asserts_*` style aliases.
...
/-! ## Asserts-style aliases -/

/-- **Asserts: HLS cluster**. -/
theorem asserts_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Asserts: HLS mass positive**. -/
theorem asserts_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
