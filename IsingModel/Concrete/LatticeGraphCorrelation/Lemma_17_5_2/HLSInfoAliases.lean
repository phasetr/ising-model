...
/-!
# Substantive HLS info aliases bundle

GJ-proposition-unit bundle providing `info_*` style aliases.
...
/-- **Info: HLS mass positive**. -/
theorem info_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
