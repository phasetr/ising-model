...
/-!
# Substantive HLS produces aliases bundle

GJ-proposition-unit bundle providing `produces_*` style aliases.
...
/-! ## Produces-style aliases -/

/-- **Produces: HLS cluster**. -/
theorem produces_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Produces: HLS mass positive**. -/
theorem produces_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
