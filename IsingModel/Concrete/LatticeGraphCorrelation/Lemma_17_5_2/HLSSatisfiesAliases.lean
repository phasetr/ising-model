...
/-!
# Substantive HLS satisfies aliases bundle

GJ-proposition-unit bundle providing `satisfies_*` style aliases.
...
/-! ## Satisfies-style aliases -/

/-- **Satisfies: HLS cluster**. -/
theorem satisfies_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Satisfies: HLS mass positive**. -/
theorem satisfies_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
...
/-- **Satisfies: HLS master**. -/
theorem satisfies_hls_master
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
