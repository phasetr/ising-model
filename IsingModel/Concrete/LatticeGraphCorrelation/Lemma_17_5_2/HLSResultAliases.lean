...
/-!
# Substantive HLS result aliases bundle

GJ-proposition-unit bundle providing `result_*` style aliases.
...
/-! ## Result-style aliases -/

/-- **Result: HLS cluster**. -/
theorem result_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Result: HLS mass positive**. -/
theorem result_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
