...
/-!
# Substantive HLS shapes aliases bundle

GJ-proposition-unit bundle providing `shapes_*` style aliases.
...
/-! ## Shapes-style aliases -/

/-- **Shapes: HLS cluster**. -/
theorem shapes_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Shapes: HLS mass positive**. -/
theorem shapes_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
