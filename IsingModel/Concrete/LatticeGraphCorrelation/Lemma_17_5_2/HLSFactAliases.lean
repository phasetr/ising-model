...
/-!
# Substantive HLS fact aliases bundle

GJ-proposition-unit bundle providing `fact_*` style aliases.
...
/-- **Fact: HLS mass positive**. -/
theorem fact_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
