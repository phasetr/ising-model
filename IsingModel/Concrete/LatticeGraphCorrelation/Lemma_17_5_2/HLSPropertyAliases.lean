...
/-!
# Substantive HLS property aliases bundle

GJ-proposition-unit bundle providing `property_*` style aliases.
...
/-! ## Property-style aliases -/

/-- **Property: HLS cluster**. -/
theorem property_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Property: HLS mass positive**. -/
theorem property_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
