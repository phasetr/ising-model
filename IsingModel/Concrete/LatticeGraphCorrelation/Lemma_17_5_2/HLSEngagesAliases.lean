...
/-!
# Substantive HLS engages aliases bundle

GJ-proposition-unit bundle providing `engages_*` style aliases.
...
/-! ## Engages-style aliases -/

/-- **Engages: HLS cluster**. -/
theorem engages_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Engages: HLS mass positive**. -/
theorem engages_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
