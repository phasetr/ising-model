...
/-!
# Substantive HLS derived aliases bundle

GJ-proposition-unit bundle providing `derived_*` style aliases.
...
/-! ## Derived-style aliases -/

/-- **Derived: HLS cluster**. -/
theorem derived_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Derived: HLS mass positive**. -/
theorem derived_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
