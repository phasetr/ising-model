...
/-!
# Substantive HLS engineers aliases bundle

GJ-proposition-unit bundle providing `engineers_*` style aliases.
...
/-! ## Engineers-style aliases -/

/-- **Engineers: HLS cluster**. -/
theorem engineers_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Engineers: HLS mass positive**. -/
theorem engineers_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
...
/-- **Engineers: HLS master**. -/
theorem engineers_hls_master
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
