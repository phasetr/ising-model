...
/-!
# Substantive HLS conveys aliases bundle

GJ-proposition-unit bundle providing `conveys_*` style aliases.
...
/-! ## Conveys-style aliases -/

/-- **Conveys: HLS cluster**. -/
theorem conveys_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Conveys: HLS mass positive**. -/
theorem conveys_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
