...
/-!
# Substantive HLS warrants aliases bundle

GJ-proposition-unit bundle providing `warrants_*` style aliases.
...
/-! ## Warrants-style aliases -/

/-- **Warrants: HLS cluster**. -/
theorem warrants_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Warrants: HLS mass positive**. -/
theorem warrants_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
