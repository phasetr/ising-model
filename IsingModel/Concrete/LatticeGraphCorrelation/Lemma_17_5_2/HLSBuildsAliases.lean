...
/-!
# Substantive HLS builds aliases bundle

GJ-proposition-unit bundle providing `builds_*` style aliases.
...
/-! ## Builds-style aliases -/

/-- **Builds: HLS cluster**. -/
theorem builds_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Builds: HLS mass positive**. -/
theorem builds_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
