...
/-!
# Substantive HLS addresses aliases bundle

GJ-proposition-unit bundle providing `addresses_*` style aliases.
...
/-! ## Addresses-style aliases -/

/-- **Addresses: HLS cluster**. -/
theorem addresses_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Addresses: HLS mass positive**. -/
theorem addresses_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
...
/-- **Addresses: HLS master**. -/
theorem addresses_hls_master
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
