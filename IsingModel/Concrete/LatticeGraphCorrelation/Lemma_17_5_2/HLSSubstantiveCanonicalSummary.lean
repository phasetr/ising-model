...
`HLSExistingHasExponentialDecayBridges` wrapper module. The older Simon-Lieb
half-rate path remains available by importing `HLSSubstantiveBundle` directly,
but it is no longer re-exported from this canonical summary.
