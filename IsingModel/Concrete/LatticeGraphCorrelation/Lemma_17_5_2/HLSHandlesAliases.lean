...
/-!
# Substantive HLS handles aliases bundle

GJ-proposition-unit bundle providing `handles_*` style aliases.
...
/-! ## Handles-style aliases -/

/-- **Handles: HLS cluster**. -/
theorem handles_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Handles: HLS mass positive**. -/
theorem handles_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
