...
/-!
# Substantive HLS affirms aliases bundle

GJ-proposition-unit bundle providing `affirms_*` style aliases.
...
/-! ## Affirms-style aliases -/

/-- **Affirms: HLS cluster**. -/
theorem affirms_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Affirms: HLS mass positive**. -/
theorem affirms_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
