...
/-!
# Substantive HLS tackles aliases bundle

GJ-proposition-unit bundle providing `tackles_*` style aliases.
...
/-! ## Tackles-style aliases -/

/-- **Tackles: HLS cluster**. -/
theorem tackles_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Tackles: HLS mass positive**. -/
theorem tackles_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
