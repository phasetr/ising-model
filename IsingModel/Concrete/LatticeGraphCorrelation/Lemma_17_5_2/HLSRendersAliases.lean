...
/-!
# Substantive HLS renders aliases bundle

GJ-proposition-unit bundle providing `renders_*` style aliases.
...
/-! ## Renders-style aliases -/

/-- **Renders: HLS cluster**. -/
theorem renders_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Renders: HLS mass positive**. -/
theorem renders_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
