...
/-!
# Substantive HLS equips aliases bundle

GJ-proposition-unit bundle providing `equips_*` style aliases.
...
/-! ## Equips-style aliases -/

/-- **Equips: HLS cluster**. -/
theorem equips_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Equips: HLS mass positive**. -/
theorem equips_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
