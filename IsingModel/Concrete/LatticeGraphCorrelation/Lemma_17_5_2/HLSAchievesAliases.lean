...
/-!
# Substantive HLS achieves aliases bundle

GJ-proposition-unit bundle providing `achieves_*` style aliases.
...
/-! ## Achieves-style aliases -/

/-- **Achieves: HLS cluster**. -/
theorem achieves_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Achieves: HLS mass positive**. -/
theorem achieves_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
