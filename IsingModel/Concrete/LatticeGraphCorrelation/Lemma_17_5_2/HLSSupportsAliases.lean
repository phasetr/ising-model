...
/-!
# Substantive HLS supports aliases bundle

GJ-proposition-unit bundle providing `supports_*` style aliases.
...
/-! ## Supports-style aliases -/

/-- **Supports: HLS cluster**. -/
theorem supports_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Supports: HLS mass positive**. -/
theorem supports_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
