...
/-!
# Substantive HLS realizes aliases bundle

GJ-proposition-unit bundle providing `realizes_*` style aliases.
...
/-! ## Realizes-style aliases -/

/-- **Realizes: HLS cluster**. -/
theorem realizes_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Realizes: HLS mass positive**. -/
theorem realizes_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
