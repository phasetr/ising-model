...
/-!
# Substantive HLS implies aliases bundle

GJ-proposition-unit bundle providing `implies_*` style aliases.
...
/-! ## Implies-style aliases -/

/-- **Implies: HLS cluster**. -/
theorem implies_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Implies: HLS mass positive**. -/
theorem implies_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
