...
/-!
# Substantive HLS output aliases bundle

GJ-proposition-unit bundle providing `output_*` style aliases.
...
/-! ## Output-style aliases -/

/-- **Output: HLS cluster**. -/
theorem output_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Output: HLS mass positive**. -/
theorem output_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
