...
/-!
# Substantive HLS supplies aliases bundle

GJ-proposition-unit bundle providing `supplies_*` style aliases.
...
/-! ## Supplies-style aliases -/

/-- **Supplies: HLS cluster**. -/
theorem supplies_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Supplies: HLS mass positive**. -/
theorem supplies_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
