...
/-!
# Substantive HLS summary aliases bundle

GJ-proposition-unit bundle providing `summary_*` style aliases.
...
/-- **Summary: HLS mass positive**. -/
theorem summary_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
