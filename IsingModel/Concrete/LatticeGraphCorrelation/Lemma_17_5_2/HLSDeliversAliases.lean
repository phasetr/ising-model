...
/-!
# Substantive HLS delivers aliases bundle

GJ-proposition-unit bundle providing `delivers_*` style aliases.
...
/-! ## Delivers-style aliases -/

/-- **Delivers: HLS cluster**. -/
theorem delivers_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Delivers: HLS mass positive**. -/
theorem delivers_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
