...
/-!
# Substantive HLS furnishes aliases bundle

GJ-proposition-unit bundle providing `furnishes_*` style aliases.
...
/-! ## Furnishes-style aliases -/

/-- **Furnishes: HLS cluster**. -/
theorem furnishes_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Furnishes: HLS mass positive**. -/
theorem furnishes_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
...
/-- **Furnishes: HLS master**. -/
theorem furnishes_hls_master
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
