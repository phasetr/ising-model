...
/-- **GJ §17.5 Lemma 17.5.2 concrete compact-ratio upper bound with automatic
active range**: the high-temperature interval inclusion supplies the
active-range hypothesis for the concrete pseudo-mass profile, while the
derivative-limit input remains explicit. -/
theorem lemma_17_5_2_upper_bound_of_concrete_pseudoMass_compact_ratio_bounds_auto_active
...
/-- **GJ §17.5 Lemma 17.5.2 concrete compact-ratio upper bound on its own
beta interval, with automatic active range**: specializes the auxiliary compact
...
-- Adds the lower validating-decay side to the automatic active-range upper
-- wrapper.
/-- **GJ §17.5 Lemma 17.5.2 concrete compact-ratio sandwich with automatic
active range**: the active-range premise is derived from the high-temperature
interval inclusion; the lower validating-decay input remains explicit. -/
theorem lemma_17_5_2_sandwich_of_concrete_pseudoMass_compact_ratio_bounds_auto_active
    {d α : ℕ} (hα : 1 ≤ α) (hαd : 2 * α > d) (hd : 1 ≤ d)
    {Λ : Ambient.Exhaustion (Fin d → ℤ)}
    {J : ℝ} (hJ_pos : 0 < J)
    {x z : Fin d → ℤ} (hxz : x ≠ z)
...
set_option maxHeartbeats 2000000 in
-- Specializes the compact auxiliary interval to the beta interval itself.
/-- **GJ §17.5 Lemma 17.5.2 concrete compact-ratio sandwich on its own beta
interval, with automatic active range**: the only remaining external analytic
inputs are the derivative-limit and lower validating-decay hypotheses. -/
theorem
    lemma_17_5_2_sandwich_of_concrete_pseudoMass_compact_ratio_bounds_on_self_Icc_auto_active
    {d α : ℕ} (hα : 1 ≤ α) (hαd : 2 * α > d) (hd : 1 ≤ d)
    {Λ : Ambient.Exhaustion (Fin d → ℤ)}
    {J : ℝ} (hJ_pos : 0 < J)
    {x z : Fin d → ℤ} (hxz : x ≠ z)
...
set_option maxHeartbeats 2000000 in
-- Repackages the automatic active-range compact sandwich with the matching
-- named upper-bound predicate for the same HLS constant.
/-- **GJ §17.5 Lemma 17.5.2 concrete compact-ratio capstone with automatic
active range**: the high-temperature interval inclusion supplies the
active-range premise, and the wrapper returns the matching named upper-bound
predicate and sandwich for one HLS constant. -/
theorem
    lemma_17_5_2_capstone_of_concrete_pseudoMass_compact_ratio_bounds_auto_active
    {d α : ℕ} (hα : 1 ≤ α) (hαd : 2 * α > d) (hd : 1 ≤ d)
    {Λ : Ambient.Exhaustion (Fin d → ℤ)}
    {J : ℝ} (hJ_pos : 0 < J)
    {x z : Fin d → ℤ} (hxz : x ≠ z)
  ...
  obtain ⟨K, hK_pos, hconv, hlower, hupper_ineq⟩ :=
    lemma_17_5_2_sandwich_of_concrete_pseudoMass_compact_ratio_bounds_auto_active
      (d := d) (α := α) (Λ := Λ) (J := J) (x := x) (z := z)
      (β₁ := β₁) (β₂ := β₂) (a := a) (b := b) (rho := rho)
      hα hαd hd hJ_pos hxz hβ₁₂ hIcc ha hab hlt hβ_mem hrho g'
      hderiv_lim hdecay
...
set_option maxHeartbeats 2000000 in
-- Repackages the self-interval automatic active-range compact sandwich with
-- the matching named upper-bound predicate for the same HLS constant.
/-- **GJ §17.5 Lemma 17.5.2 concrete compact-ratio capstone on its own beta
interval, with automatic active range**: specializes the auxiliary compact
interval to `[β₁, β₂]`, derives active range from the high-temperature
inclusion, and returns the matching named upper-bound predicate and sandwich
for one HLS constant. -/
theorem
    lemma_17_5_2_capstone_of_concrete_pseudoMass_compact_ratio_bounds_on_self_Icc_auto_active
    {d α : ℕ} (hα : 1 ≤ α) (hαd : 2 * α > d) (hd : 1 ≤ d)
    {Λ : Ambient.Exhaustion (Fin d → ℤ)}
    {J : ℝ} (hJ_pos : 0 < J)
    {x z : Fin d → ℤ} (hxz : x ≠ z)
  ...
  obtain ⟨K, hK_pos, hconv, hlower, hupper_ineq⟩ :=
    lemma_17_5_2_sandwich_of_concrete_pseudoMass_compact_ratio_bounds_on_self_Icc_auto_active
      (d := d) (α := α) (Λ := Λ) (J := J) (x := x) (z := z)
      (β₁ := β₁) (β₂ := β₂) (rho := rho)
      hα hαd hd hJ_pos hxz hβ₁₂ hIcc hrho g' hderiv_lim hdecay
  have hupper :
      Lemma_17_5_2_UpperBound hα hrho Λ J β₂ x z
        (ENNReal.ofReal (((2 * α + 1 : ℕ) : ℝ) * K / rho)) := by
    simpa [Lemma_17_5_2_UpperBound] using hupper_ineq
