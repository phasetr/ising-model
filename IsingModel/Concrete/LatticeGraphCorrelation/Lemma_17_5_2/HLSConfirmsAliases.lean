...
/-!
# Substantive HLS confirms aliases bundle

GJ-proposition-unit bundle providing `confirms_*` style aliases.
...
/-! ## Confirms-style aliases -/

/-- **Confirms: HLS cluster**. -/
theorem confirms_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Confirms: HLS mass positive**. -/
theorem confirms_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
