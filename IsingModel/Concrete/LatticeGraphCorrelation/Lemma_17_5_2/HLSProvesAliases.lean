...
/-!
# Substantive HLS proves aliases bundle

GJ-proposition-unit bundle providing `proves_*` style aliases.
...
/-! ## Proves-style aliases -/

/-- **Proves: HLS cluster**. -/
theorem proves_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Proves: HLS mass positive**. -/
theorem proves_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
