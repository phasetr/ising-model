...
/-!
# Substantive HLS causes aliases bundle

GJ-proposition-unit bundle providing `causes_*` style aliases.
...
/-! ## Causes-style aliases -/

/-- **Causes: HLS cluster**. -/
theorem causes_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Causes: HLS mass positive**. -/
theorem causes_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
