...
/-!
# Substantive HLS verifies aliases bundle

GJ-proposition-unit bundle providing `verifies_*` style aliases.
...
/-! ## Verifies-style aliases -/

/-- **Verifies: HLS cluster**. -/
theorem verifies_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Verifies: HLS mass positive**. -/
theorem verifies_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
