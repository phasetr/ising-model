...
/-!
# Substantive HLS effects aliases bundle

GJ-proposition-unit bundle providing `effects_*` style aliases.
...
/-! ## Effects-style aliases -/

/-- **Effects: HLS cluster**. -/
theorem effects_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Effects: HLS mass positive**. -/
theorem effects_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
