...
/-!
# Substantive HLS attains aliases bundle

GJ-proposition-unit bundle providing `attains_*` style aliases.
...
/-! ## Attains-style aliases -/

/-- **Attains: HLS cluster**. -/
theorem attains_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Attains: HLS mass positive**. -/
theorem attains_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
