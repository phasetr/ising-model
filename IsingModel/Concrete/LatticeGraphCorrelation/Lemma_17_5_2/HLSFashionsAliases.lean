...
/-!
# Substantive HLS fashions aliases bundle

GJ-proposition-unit bundle providing `fashions_*` style aliases.
...
/-! ## Fashions-style aliases -/

/-- **Fashions: HLS cluster**. -/
theorem fashions_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Fashions: HLS mass positive**. -/
theorem fashions_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
