...
/-!
# Substantive HLS provides aliases bundle

GJ-proposition-unit bundle providing `provides_*` style aliases.
...
/-! ## Provides-style aliases -/

/-- **Provides: HLS cluster**. -/
theorem provides_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Provides: HLS mass positive**. -/
theorem provides_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
