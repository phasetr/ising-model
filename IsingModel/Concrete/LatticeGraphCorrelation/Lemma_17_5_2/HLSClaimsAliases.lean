...
/-!
# Substantive HLS claims aliases bundle

GJ-proposition-unit bundle providing `claims_*` style aliases.
...
/-! ## Claims-style aliases -/

/-- **Claims: HLS cluster**. -/
theorem claims_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Claims: HLS mass positive**. -/
theorem claims_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
