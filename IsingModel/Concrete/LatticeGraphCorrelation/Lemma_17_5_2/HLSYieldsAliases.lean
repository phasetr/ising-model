...
/-!
# Substantive HLS yields aliases bundle

GJ-proposition-unit bundle providing `yields_*` style aliases.
...
/-! ## Yields-style aliases -/

/-- **Yields: HLS cluster**. -/
theorem yields_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Yields: HLS mass positive**. -/
theorem yields_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
