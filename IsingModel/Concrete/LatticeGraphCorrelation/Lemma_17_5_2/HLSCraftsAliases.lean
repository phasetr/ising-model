...
/-!
# Substantive HLS crafts aliases bundle

GJ-proposition-unit bundle providing `crafts_*` style aliases.
...
/-! ## Crafts-style aliases -/

/-- **Crafts: HLS cluster**. -/
theorem crafts_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Crafts: HLS mass positive**. -/
theorem crafts_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
