...
/-!
# Substantive HLS attests aliases bundle

GJ-proposition-unit bundle providing `attests_*` style aliases.
...
/-! ## Attests-style aliases -/

/-- **Attests: HLS cluster**. -/
theorem attests_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Attests: HLS mass positive**. -/
theorem attests_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
