...
/-!
# Substantive HLS devises aliases bundle

GJ-proposition-unit bundle providing `devises_*` style aliases.
...
/-! ## Devises-style aliases -/

/-- **Devises: HLS cluster**. -/
theorem devises_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Devises: HLS mass positive**. -/
theorem devises_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
