...
/-!
# Substantive HLS affords aliases bundle

GJ-proposition-unit bundle providing `affords_*` style aliases.
...
/-! ## Affords-style aliases -/

/-- **Affords: HLS cluster**. -/
theorem affords_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Affords: HLS mass positive**. -/
theorem affords_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
