...
/-!
# Substantive HLS fulfills aliases bundle

GJ-proposition-unit bundle providing `fulfills_*` style aliases.
...
/-! ## Fulfills-style aliases -/

/-- **Fulfills: HLS cluster**. -/
theorem fulfills_hls_cluster
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
...
/-- **Fulfills: HLS mass positive**. -/
theorem fulfills_hls_mass_positive
    {d : ℕ} (hd : 1 ≤ d) {β J : ℝ}
    (hf : IsingModel.Ferromagnetic (⟨J, (0 : ℝ), β⟩ : IsingParams ℝ))
    (hβJ : 0 < β * J)
    (hβJd_lt : β * J * ↑(2 * d) < 1) :
