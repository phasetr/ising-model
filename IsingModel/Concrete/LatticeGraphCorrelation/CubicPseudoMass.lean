...
/-!
# Cubic named-rate product-sum and capstone wrappers

This capstone module contains anchored cubic named-rate product-sum and bundled
downstream APIs. Basic names live in `CubicPseudoMassBasic`; tanh-profile
bridges live in `CubicPseudoMassTanhProfile`; named-rate lattice-mass and
interval bridges live in `CubicPseudoMassNamedRate`; cluster and summability
wrappers live in `CubicPseudoMassClusterSummability`.
-/
