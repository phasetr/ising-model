...
hypothesis**: this packages the transferred Simon--Lieb high-temperature lower
bound together with the anchored cubic active-range input generated by the same
tanh-profile hypothesis.

The lower-bound conclusion is the existing high-temperature estimate; the
profile hypothesis is retained so downstream callers can use one hypothesis
bundle while the direct concrete `pseudoMassFromParamsAtPair` wrapper remains
too expensive to expose in a new theorem conclusion.
...
/-- **Positive lattice mass under a cubic tanh-profile hypothesis**: combines
the profile-compatible active-range package with the transferred
high-temperature positive-mass theorem.
