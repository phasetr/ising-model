...
directly. Sandwich and downstream wrappers remain in
`HighTemperatureBounds` (deviation / continuity wrappers were further
moved to `HighTemperatureBoundsDeviation` in PR #1936; ratio_sandwich /
ratio_bound wrappers were further moved to
`HighTemperatureBoundsRatioBounds` in PR #1937).
...
`IsingModel.Concrete.LatticeGraphCorrelation.ComplexBranches` directly.
For concrete continuity, `AnalyticOnNhd`, and norm-bound wrappers for
`partitionFunctionComplex` / `freeEnergyComplex` on `latticeGraph d` (15
theorems including `continuous_partitionFunctionComplex_*`,
`AnalyticOn_leeYangDomain/Subdomain` variants, and
`norm_*Complex_le_*` bounds), import
`IsingModel.Concrete.LatticeGraphCorrelation.ComplexContinuityNorm`
directly.
For concrete real-complex compatibility, Lee-Yang-domain
...
`deviation_sandwich_exp`, `ratio_bound_bundle`), import
`IsingModel.Concrete.LatticeGraphCorrelation.HighTemperatureBoundsFreeEnergyInfinite`
directly.
For concrete §18.3-§18.4 alongExhaustion Z/f/log Z `ratio_sandwich_bundle` /
`ratio_bound` / `triple_ratio_*` wrappers at `h = 0` (29 theorems for
...
directly.
For concrete §18.3-§18.4 alongExhaustion deviation_bound_exp /
continuity_bundle / deviation_sandwich / relative_sandwich /
deviation_pos / pow_two_lt / strict_deviation_bundle wrappers at
`h = 0` (18 theorems for `freeEnergyAlongExhaustion_latticeGraph`,
...
directly.
For concrete §18.3-§18.4 alongExhaustion sharper-exp upper-bound /
sandwich / complete-summary wrappers at `h = 0` (17 theorems for
...
families plus ferromagnetic variants), import
`IsingModel.Concrete.LatticeGraphCorrelation.HighTemperatureBoundsAlongExhaustionExpSharper`
directly.
For concrete §18.3-§18.4 alongExhaustion correlation / sandwich /
...
`triple_*` variants plus ferromagnetic counterparts), import
`IsingModel.Concrete.LatticeGraphCorrelation.HighTemperatureBoundsRatioBounds`
directly.
For concrete §18.3-§18.4 f/Z/log Z deviation / continuity wrappers at
