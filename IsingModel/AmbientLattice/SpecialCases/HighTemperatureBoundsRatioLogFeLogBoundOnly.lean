...
wrapper. The two slice-singleton wrappers
(`_ratio_bound`, `_ratio_bound_beta_zero`) and the ferromagnetic
bundle wrapper (`_ratio_bound_bundle_ferromagnetic`) now live in
`IsingModel.AmbientLattice.SpecialCases.HighTemperatureBoundsRatioLogFeLogBoundOnlySingletons`
and
`IsingModel.AmbientLattice.SpecialCases.HighTemperatureBoundsRatioLogFeLogBoundOnlyFerro`
respectively, and are re-imported through this parent module. The
general bundle is a thin pass-through to the corresponding
`log_partitionFunctionΛ_*` ambient lemma. Theorem names are
...
/-! ## Moved: 1 ferromagnetic log Z ratio_bound_bundle wrapper
