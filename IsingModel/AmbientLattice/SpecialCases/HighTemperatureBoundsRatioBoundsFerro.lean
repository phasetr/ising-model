...
/-!
# The packaged two-sided ratio bound under `0 ≤ J` and `0 < β`
...
Under `0 ≤ J` and `0 < β`, a conjunction records for each of those two ratios that it lies
between `Real.cosh (β * J) ^ |E|` and `Real.exp (β * J * |E|)`.
-/
