...
nonnegative ray: from `0 ≤ t` and `t ≤ s` the value at `t` is at most the value at `s`.
-/
...
/-- Along an exhaustion, polymer free energy preserves order when the smaller activity is
nonnegative. -/
theorem polymerFreeEnergyAlongExhaustion_le_of_le_of_nonneg_left
  ...
  polymerFreeEnergy_Λ_le_of_le_of_nonneg_left
    G (Λ.volume n) ht hts
