...
/-!
# Closed form of the partition function at `J = 0`
...
At `J = 0` with `h` and `β` arbitrary, the stage partition function equals
`(2 * Real.cosh (β * h)) ^ (Λ.volume n).card`. With the coupling switched off the sites
decouple, each contributing the single-site factor `2 * Real.cosh (β * h)`, so the right-hand
side is determined by `β * h` and the cardinality of the stage volume alone.
-/
