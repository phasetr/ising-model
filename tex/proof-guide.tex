...
Let $N := \max\{|a_i| : a \in A,\ i : \mathrm{Fin}\ d\}$ (well defined
as the max of a finite multiset of natural numbers; if both $A$ and
$\mathrm{Fin}\ d$ are non-empty, otherwise we take $N = 0$ and
\texttt{exfalso}). For any $n \ge N$ and $a \in A$, $i : \mathrm{Fin}\ d$,
$|a_i| \le N \le n$, so $-n \le a_i \le n$ and
$a \in \mathrm{cubicBox}\ d\ n$.
\end{proof}
...
subsequent PR.
Reference: Glimm--Jaffe \cite{glimm-jaffe} \S 4.6, p.~64.
