...
Eight entry points feed \texttt{Lemma\_17\_5\_2\_DerivativeLimitProvider} into
the scalar-provider, ratio-lower, uniform-correlation-lower, and
compact-ratio-bounds upper/sandwich wrappers.  This extends the provider-shaped
interface below the concrete capstones: callers of the enlarged finite-HLS
chain also no longer need to name the limiting derivative profile \(g'\), while
the remaining analytic work is unchanged.
...
scalar-provider, ratio-lower, uniform-correlation-lower, and
compact-ratio-bounds upper/sandwich assemblies.  This keeps the concrete
Cauchy estimate as the remaining analytic input while making it directly
consumable by the finite-HLS capstone wrappers.
...
\texttt{DiniFiniteHLS.lean} then feeds that provider into the scalar-provider,
ratio-lower, uniform-correlation-lower, and compact-ratio-bounds
upper/sandwich assemblies.  This gives the finite-HLS layer the same direct
Dini-shaped API already available through the provider-shaped and
Cauchy-shaped entry points.
