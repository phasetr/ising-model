...
\[
  ...
  \le
  \operatorname{ofReal}\!\left(\frac{(2\alpha+1)K}{r}\right)
  \operatorname{ofReal}(m^-_{\Lambda}(x,z;\beta_2))
\]
is enough to prove
\texttt{Lemma\_17\_5\_2\_InfiniteHLSLipschitzAllRateBridge}.  Indeed, any
validating target-exhaustion decay rate transfers to the cubic exhaustion,
where Step~115 gives the all-rate bound by \(-\log(\tanh(\beta_2J))\).  The
theorems
...
package itself, closing the upper-bound predicate or conditional sandwich for
the returned HLS constant without importing the finite derivative-limit package.
