...
\subsection{Generic weighted-expectation linearity
(\texttt{Inequalities/WeightedExpectation.lean})}

Every Gibbs expectation in the FKG/Holley development has the shape
$\langle F \rangle = Z^{-1} \sum_\sigma F(\sigma)\, w(\sigma)$ for a partition
value $Z = \sum_\sigma w(\sigma)$ and a weight $w$ (the Boltzmann weight, the
inhomogeneous Boltzmann weight, or the boundary-condition Boltzmann weight). The
constant / additivity / scalar linearity of such a normalised weighted average is
purely algebraic and is proved once, generic in $w$ and $Z$, as
\texttt{weightedExpectation\_const} ($Z^{-1}\sum c\,w = c$),
\texttt{weightedExpectation\_add}, and \texttt{weightedExpectation\_const\_mul}.
The nine \texttt{gibbsExpectation\_*} / \texttt{gibbsExpectationJ\_*} /
\texttt{gibbsExpectationBC\_*} wrappers delegate to these helpers (single source of
truth).

\subsection{General FKG for arbitrary monotone observables
(\texttt{Inequalities/FKGGeneral.lean})}

The theorem \texttt{fkg\_ising} above, inheriting the interface of Mathlib's
\texttt{fkg}, additionally requires the observables to be \emph{nonnegative}
($0 \le f$, $0 \le g$). The genuine Friedli--Velenik Theorem~3.21 holds for
\emph{any} monotone nondecreasing $f, g$ of arbitrary sign. We remove the
nonnegativity hypothesis by covariance shift-invariance.

\begin{theorem}[\texttt{fkg\_ising\_of\_monotone}]
For ferromagnetic parameters and \emph{arbitrary} monotone nondecreasing
$f, g : \mathrm{Config}\;\iota \to \mathbb{R}$,
$\langle f \rangle \langle g \rangle \le \langle fg \rangle$.
\end{theorem}

\noindent\textbf{Proof.} The Gibbs covariance
$\mathrm{Cov}(f, g) = \langle fg \rangle - \langle f \rangle\langle g \rangle$
is invariant under adding constants to $f$ and $g$: by linearity of
$\langle \cdot \rangle$ and the normalisation $\langle 1 \rangle = 1$, the
constant terms cancel. Since $\mathrm{Config}\;\iota$ is finite (and nonempty),
each monotone observable attains a minimum; subtracting it gives nonnegative
monotone observables $f' = f - \min f \ge 0$ and $g' = g - \min g \ge 0$ with
$\mathrm{Cov}(f', g') = \mathrm{Cov}(f, g)$. Applying \texttt{fkg\_ising} to
$f', g'$ yields $\mathrm{Cov}(f, g) = \mathrm{Cov}(f', g') \ge 0$. The argument
uses the Gibbs-expectation linearity lemmas \texttt{gibbsExpectation\_const}
($\langle c \rangle = c$), \texttt{gibbsExpectation\_add}
($\langle F + H \rangle = \langle F \rangle + \langle H \rangle$), and
\texttt{gibbsExpectation\_const\_mul} ($\langle cF \rangle = c\langle F \rangle$),
all proved in the same file. \hfill$\square$

\subsection{FKG with inhomogeneous per-edge couplings
(\texttt{Inequalities/FKGInhomogeneous.lean})}

Friedli--Velenik Theorem~3.21 is stated for a \emph{collection of nonnegative
couplings} $J = (J_{ij})$ (one per edge) and an \emph{arbitrary} field $h$, not
just the uniform scalar coupling \texttt{IsingParams.J}. We build the
inhomogeneous Gibbs stack
\[
  H_J(\sigma) = -\sum_{e \in E} J(e)\, s(\sigma_i) s(\sigma_j)
                - h \sum_i s(\sigma_i),
  \qquad w_J(\sigma) = e^{-\beta H_J(\sigma)},
\]
with \texttt{interactionEnergyJ}, \texttt{hamiltonianJ}, \texttt{boltzmannWeightJ},
\texttt{partitionFunctionJ}, and \texttt{gibbsExpectationJ} (with linearity
lemmas).

\begin{theorem}[\texttt{boltzmannWeightJ\_log\_supermodular}]
For $\beta \ge 0$ and every coupling $J(e) \ge 0$ (the field $h$ may have any
sign, its term being modular),
$w_J(\sigma) w_J(\sigma') \le w_J(\sigma \vee \sigma') w_J(\sigma \wedge \sigma')$.
\end{theorem}

\noindent The proof is the uniform-coupling argument with the single $J$ replaced
by the per-edge weights: each edge term is supermodular
(\texttt{spin\_edge\_supermodularJ}, 16-case check), and the nonnegative weights
$J(e)$ preserve the inequality under summation, while the field term is modular.

\begin{theorem}[\texttt{fkg\_isingJ\_of\_monotone}, FV Thm 3.21 free boundary]
For nonnegative per-edge couplings $J(e) \ge 0$, $\beta \ge 0$, arbitrary field
$h$, and arbitrary monotone nondecreasing $f, g$,
$\langle f \rangle \langle g \rangle \le \langle fg \rangle$.
\end{theorem}

\noindent Proved from \texttt{fkg\_isingJ} (the nonnegative-observable case, via
Mathlib's \texttt{fkg} applied to $w_J$) by the same covariance shift-invariance
as \texttt{fkg\_ising\_of\_monotone}. This matches FV~3.21 exactly for the
free-boundary case; arbitrary boundary conditions remain follow-up (Issue~3558).

\subsection{FKG with arbitrary boundary conditions
(\texttt{Inequalities/FKGBoundaryCondition.lean})}

Friedli--Velenik Theorem~3.21 allows an \emph{arbitrary boundary condition}: the
spins outside the finite volume $\Lambda$ are frozen to a fixed external
configuration $\eta$. We obtain the boundary-condition Gibbs measure from the
inhomogeneous Boltzmann weight by \emph{conditioning} on configurations agreeing
with $\eta$ off $\Lambda$ --- no new Hamiltonian is needed, since the boundary
interactions are already present in $w_J$. Writing
$\mathrm{agreesOff}\;\Lambda\;\eta\;\sigma := (\forall i \notin \Lambda,\;
\sigma_i = \eta_i)$, the conditioned weight is
\[
  w^{\eta}_\Lambda(\sigma)
    = \mathbf{1}_{\{\sigma : \mathrm{agreesOff}\;\Lambda\;\eta\;\sigma\}}(\sigma)
      \cdot w_J(\sigma).
\]
The crucial point is that $\{\sigma : \mathrm{agreesOff}\;\Lambda\;\eta\;\sigma\}$
is a \emph{sublattice}: if $\sigma, \sigma'$ agree with $\eta$ off $\Lambda$ then so
do $\sigma \vee \sigma'$ and $\sigma \wedge \sigma'$ (off $\Lambda$ both equal
$\eta_i$, so the join/meet stay $\eta_i$; \texttt{agreesOff\_sup} /
\texttt{agreesOff\_inf}). Hence the indicator of that sublattice times the
log-supermodular weight is again log-supermodular
(\texttt{boltzmannWeightBC\_log\_supermodular}).

\begin{theorem}[\texttt{fkg\_isingBC\_of\_monotone}, FV Thm 3.21 full form]
For nonnegative per-edge couplings $J(e) \ge 0$, $\beta \ge 0$, arbitrary field
$h$, an arbitrary boundary condition $\eta$ outside $\Lambda$, and arbitrary
monotone nondecreasing $f, g$,
$\langle f \rangle^{\eta}_\Lambda \langle g \rangle^{\eta}_\Lambda
  \le \langle fg \rangle^{\eta}_\Lambda$.
\end{theorem}

\noindent Proved exactly as the free-boundary case (Mathlib \texttt{fkg} on the
conditioned weight, then the covariance shift). Together with the inhomogeneous
case this realizes the complete FV~3.21 statement.

\subsection{Field monotonicity via Holley's inequality
(\texttt{Inequalities/MonotonicityField.lean})}

Increasing the external field $h$ increases the Gibbs expectation of every
monotone observable:
\[
  h \le h' \;\Longrightarrow\; \langle \varphi \rangle_h \le \langle \varphi
  \rangle_{h'}
  \qquad (\varphi \text{ nondecreasing}).
\]
This is foundational for the stochastic ordering of Ising measures, the $\pm$
extremal states, and the infinite-volume limit. It is proved via Mathlib's
\texttt{holley} inequality. The two normalised Boltzmann weights at fields
$h \le h'$ satisfy the Holley domination condition
(\texttt{boltzmannWeight\_field\_cross\_supermodular})
\[
  w_h(a)\, w_{h'}(b) \le w_h(a \wedge b)\, w_{h'}(a \vee b),
\]
whose interaction part is the edge log-supermodularity (weight $\beta J \ge 0$)
and whose field part is the per-site inequality
$h\, s(a) + h'\, s(b) \le h\, s(a \wedge b) + h'\, s(a \vee b)$
(\texttt{field\_site\_mono}, valid for $h \le h'$, weight $\beta \ge 0$; the only
nontrivial case gives $(h'-h)(s(a)-s(b)) \ge 0$).

\begin{theorem}[\texttt{gibbsExpectation\_field\_mono}]
For a ferromagnetic Ising model ($\beta, J \ge 0$), $h \le h'$, and any monotone
nondecreasing $\varphi$,
$\langle \varphi \rangle_h \le \langle \varphi \rangle_{h'}$.
\end{theorem}

\noindent The nonnegative case is \texttt{holley} applied to the normalised
measures; the general case follows by the constant shift (the inequality is
invariant under $\varphi \mapsto \varphi - c$).

\subsection{Boundary-condition monotonicity via Holley
(\texttt{Inequalities/MonotonicityBoundary.lean})}

Raising the boundary condition raises the Gibbs expectation of every monotone
observable:
\[
  \eta \le \eta' \;\Longrightarrow\; \langle \varphi \rangle^{\eta}_\Lambda
  \le \langle \varphi \rangle^{\eta'}_\Lambda
  \qquad (\varphi \text{ nondecreasing}).
\]
This is the cornerstone of the theory of extremal ($\pm$) Gibbs states and the
infinite-volume limit: the maximal ($+$) boundary condition yields the maximal
state. It is proved via \texttt{holley} on the conditioned boundary-condition
weights. The crucial point is the order-aware conditioning closure: if $a$ agrees
with $\eta$ off $\Lambda$ and $b$ with $\eta'$, then (since $\eta \le \eta'$)
$a \wedge b$ agrees with $\eta$ (as $\eta \wedge \eta' = \eta$) and $a \vee b$
agrees with $\eta'$ (as $\eta \vee \eta' = \eta'$); hence the domination
condition
$w^{\eta}(a)\, w^{\eta'}(b) \le w^{\eta}(a \wedge b)\, w^{\eta'}(a \vee b)$
(\texttt{boltzmannWeightBC\_cross\_supermodular}) reduces to
\texttt{boltzmannWeightJ\_log\_supermodular} on the agreeing case and is $0 \le$
nonnegative otherwise.

\begin{theorem}[\texttt{gibbsExpectationBC\_boundary\_mono}]
For $\beta, J \ge 0$, $\eta \le \eta'$, and any monotone nondecreasing $\varphi$,
$\langle \varphi \rangle^{\eta}_\Lambda \le \langle \varphi
\rangle^{\eta'}_\Lambda$.
\end{theorem}

\subsection{Extremal $\pm$ states and magnetization ordering
(\texttt{Inequalities/MonotonicityExtremal.lean})}

The boundary-condition monotonicity is sharpest at the two extremal
configurations $\texttt{plusConfig}$ (all spins up, maximal) and
$\texttt{minusConfig}$ (all spins down, minimal): every configuration satisfies
$\texttt{minusConfig} \le \sigma \le \texttt{plusConfig}$. Hence, for any boundary
condition $\eta$ and any monotone $\varphi$,
\[
  \langle \varphi \rangle^{-}_\Lambda \le \langle \varphi \rangle^{\eta}_\Lambda
  \le \langle \varphi \rangle^{+}_\Lambda
  \qquad (\texttt{gibbsExpectationBC\_extremal\_sandwich}),
\]
so the $\pm$ boundary conditions bracket all boundary conditions: at this finite
volume the $+$ boundary condition gives the largest expectation of every monotone
observable (the foundation for the extremal infinite-volume $\pm$ states,
constructed as thermodynamic limits). As a concrete application, the single-spin observable
$\sigma \mapsto s(\sigma_i)$ is monotone (\texttt{singleSpinObs\_monotone}), so the
local magnetization is squeezed:
$\langle \sigma_i \rangle^{-}_\Lambda \le \langle \sigma_i \rangle^{\eta}_\Lambda
\le \langle \sigma_i \rangle^{+}_\Lambda$
(\texttt{magnetizationBC\_extremal\_sandwich}).

\subsection{Volume monotonicity of the $+$ state
(\texttt{Inequalities/VolumeMonotonicity.lean})}

The $+$ boundary expectation of a monotone observable \emph{decreases as the
volume grows}:
\[
  \Lambda_1 \subseteq \Lambda_2 \;\Longrightarrow\;
  \langle \varphi \rangle^{+}_{\Lambda_2} \le \langle \varphi
  \rangle^{+}_{\Lambda_1}
  \qquad (\texttt{gibbsExpectationBC\_plus\_volume\_antitone}).
\]
With the dual $-$ bound this exhibits the $\pm$ states as monotone limits over an
exhaustion --- the construction of the extremal infinite-volume Gibbs states. The
proof uses the conditioning structure: the $\Lambda_1$ $+$ state is the
$\Lambda_2$ $+$ state conditioned on the shell $\Lambda_2 \setminus \Lambda_1$
being all $+$ (\texttt{agreesOff\_plus\_iff\_shell}). The shell event is an
up-set, so its indicator $\mathbf{1}_A$ is monotone
(\texttt{upShellIndicator\_monotone}); the weight identity
$w^{+}_{\Lambda_1} = \mathbf{1}_A \cdot w^{+}_{\Lambda_2}$
(\texttt{boltzmannWeightBC\_plus\_eq\_shell\_mul}) gives
$\langle \varphi \cdot \mathbf{1}_A \rangle^{+}_{\Lambda_2}
= \langle \varphi \rangle^{+}_{\Lambda_1} \cdot
\langle \mathbf{1}_A \rangle^{+}_{\Lambda_2}$, and the FKG inequality
$\langle \varphi \cdot \mathbf{1}_A \rangle \ge \langle \varphi \rangle
\langle \mathbf{1}_A \rangle$ then yields the claim after cancelling
$\langle \mathbf{1}_A \rangle^{+}_{\Lambda_2} > 0$.

\subsection{$+$ boundary expectation on a cubic box
(\texttt{Concrete/LatticeGraphCorrelation/CubicBoxPlusState.lean})}

The first step toward the infinite-volume $+$ Gibbs state on $\mathbb{Z}^d$
(Issue~3565): \texttt{plusBoxExpectation}~$d\,n\,m\,J\,h\,\beta\,\varphi$ is the
expectation of $\varphi$ under the Ising measure on the box
$\texttt{cubicBox}\,d\,m$ with the annulus
$\texttt{cubicBox}\,d\,m \setminus \texttt{cubicBox}\,d\,n$ frozen to $+$ --- i.e.
\texttt{gibbsExpectationBC} over the induced cubic-lattice graph with inner region
\texttt{plusBoxInterior}~$d\,n\,m$ and boundary configuration \texttt{plusConfig}.
Within a fixed ambient box $m$, growing the inner region makes the $+$ expectation
of a monotone observable decrease
(\texttt{plusBoxExpectation\_antitone\_interior}), the cubic-box instance of
FV~Lemma~3.22. Lifting this to a genuine thermodynamic limit (growing $m$ as well)
is the subject of the later PRs of Issue~3565.

\subsection{Bounded BC observables and the single-site $+$ box spin
(\texttt{Concrete/LatticeGraphCorrelation/CubicBoxPlusStateBounds.lean})}

Two generic reusable bounds: the boundary-condition expectation of a bounded
observable is bounded by the pointwise bound (it is a nonnegative-weight average),
\texttt{gibbsExpectationBC\_le\_of\_forall\_le} and
\texttt{gibbsExpectationBC\_ge\_of\_forall\_ge}. The single-site $+$ box spin
\texttt{plusBoxSpin}~$d\,n\,m\,J\,h\,\beta\,x$ (the $+$ boundary expectation of
$s(\sigma_x)$) then lies in $[-1, 1]$ (\texttt{plusBoxSpin\_mem\_Icc}, the
bounded-below input for the eventual monotone-convergence limit) and is antitone in
the inner region (\texttt{plusBoxSpin\_antitone\_interior}, the single-site
cubic-box instance of FV~Lemma~3.22). The remaining ingredient --- the
nearest-neighbour screening lemma (ambient-box independence) --- is the next PR of
Issue~3565.

\subsection{Cubic-box adjacency geometry
(\texttt{Concrete/CubicBoxAdjacencyGeometry.lean})}

The geometric facts underlying the screening of the $+$ box state.
\texttt{latticeGraph\_adj\_abs\_le\_one}: adjacent sites differ by at most $1$ in
each coordinate. \texttt{cubicBox\_adj\_mem\_succ}: a neighbour of a
$\texttt{cubicBox}\,d\,n$ site lies in $\texttt{cubicBox}\,d\,(n+1)$, so the inner
free region reaches only one box further out (shielded from the outer shell).
\texttt{cubicBox\_shell\_adj\_not\_mem\_inner}: for $n+1 \le m$, a shell site of
$\texttt{cubicBox}\,d\,(m+1) \setminus \texttt{cubicBox}\,d\,m$ and any of its
neighbours lie outside $\texttt{cubicBox}\,d\,n$, so every shell-touching
interaction is between frozen $+$ spins --- the geometric input to the
constant-factor cancellation in the screening proof.

\subsection{Induced-graph energy split over the extension graph
(\texttt{AmbientLattice/Monotonicity/InducedEnergySplit.lean})}

The energy-level heart of the $+$-state screening, reusing the extension-graph
factoring of \texttt{EdgeSiteSums.lean}. The interaction energy on
$\texttt{inducedGraph}\,G\,\Lambda_2$ equals that on
$\texttt{extendGraphFromΛ₁}\,G\,\Lambda_1\,\Lambda_2$ plus the $-J$-weighted
edge-spin sum over the \emph{extra edges}
$(\texttt{inducedGraph}\,G\,\Lambda_2).\mathrm{edgeFinset} \setminus
(\texttt{extendGraphFromΛ₁}\,G\,\Lambda_1\,\Lambda_2).\mathrm{edgeFinset}$
(\texttt{interactionEnergy\_inducedGraph\_extendGraph\_split}, via
\texttt{edgeFinset\_mono} and \texttt{Finset.sum\_sdiff}). Composing the full
Hamiltonian split with \texttt{hamiltonian\_extendGraph\_factor} gives
(\texttt{hamiltonian\_inducedGraph\_restrict\_add})
\[
  H_{\texttt{induce}\,\Lambda_2}(\sigma)
    = H_{\texttt{induce}\,\Lambda_1}(\mathrm{restrict}\,\sigma)
      + (\text{complement field}) + (\text{extra-edge interaction}).
\]
On the cubic box the extra edges all touch the frozen $+$ shell (\S above), so the
extra-edge interaction is a constant that cancels in the normalised expectation
--- the screening.

\subsection{Boltzmann-weight factoring of an induced graph
(\texttt{AmbientLattice/Monotonicity/InducedWeightFactor.lean})}

Exponentiating the energy split gives
(\texttt{boltzmannWeight\_inducedGraph\_restrict\_factor})
\[
  w_{\texttt{induce}\,\Lambda_2}(\sigma)
    = w_{\texttt{induce}\,\Lambda_1}(\mathrm{restrict}\,\sigma)
      \cdot \exp\!\big(-\beta\,(\text{complement field} + \text{extra-edge
      interaction})\big).
\]
When the complement sites are all $\mathrm{up}$ and the extra edges all have
$\mathrm{edgeSpin} = 1$ (the frozen $+$ case),
\texttt{boltzmannWeight\_inducedGraph\_restrict\_factor\_const} collapses the
exponential factor to a constant depending only on the complement-site count and
the extra-edge count --- the constant that cancels in the normalised $+$
boundary-condition expectation (the screening).

\subsection{Cubic-box screening conditions
(\texttt{Concrete/LatticeGraphCorrelation/CubicBoxScreening.lean})}

The frozen-$+$ ingredients turning the generic weight factoring into the cubic-box
screening. \texttt{boltzmannWeightJ\_uniform\_eq}: at a uniform coupling
$\texttt{fun \_} \Rightarrow J$, the inhomogeneous Boltzmann weight equals the
ordinary one, $w_J^{(\texttt{fun \_}\Rightarrow J)} = w_{\langle J,h,\beta\rangle}$,
bridging the boundary-condition framework
($w^{\mathrm{BC}} = \mathbf{1}_{\mathrm{agrees}} \cdot w_J$) to the
uniform-coupling factoring lemmas.
\texttt{cubicBox\_extra\_edge\_endpoints\_not\_mem\_inner}: for $n+1 \le m$, both
endpoints of an extra edge (in the induced graph on $\texttt{cubicBox}\,d\,(m+1)$
but not in the extension graph over $\texttt{cubicBox}\,d\,m$) lie outside
$\texttt{cubicBox}\,d\,n$ --- by \texttt{Sym2.ind} case analysis, an endpoint lies
in the shell, and \texttt{cubicBox\_shell\_adj\_not\_mem\_inner} then places both
outside $\texttt{cubicBox}\,d\,n$. Hence on a $+$-off-$\texttt{box}\,n$
configuration every extra edge has $\mathrm{edgeSpin} = 1$ --- the $\texttt{hextra}$
input to the screening.

\subsection{Configuration-split decomposition for the cubic-box screening
(\texttt{Concrete/LatticeGraphCorrelation/CubicBoxScreeningDecomp.lean})}

The configuration-side ingredients. \texttt{restrictConfig\_plusConfig}:
restricting the all-$+$ configuration is all-$+$. \texttt{mem\_plusBoxInterior}:
$j \in \texttt{plusBoxInterior}\,d\,n\,m \iff (j : \mathbb Z^d) \in
\texttt{cubicBox}\,d\,n$. \texttt{agreesOff\_plus\_configEquiv\_iff}: under the
configuration split $\texttt{configEquivSubtypeProd}\,(\texttt{cubicBox}\,d\,m
\subseteq \texttt{cubicBox}\,d\,(m+1))$, a recombined configuration
$(\mathrm{equiv})^{-1}(\sigma_1, \sigma_2)$ agrees with $+$ off
$\texttt{cubicBox}\,d\,n$ iff $\sigma_1$ agrees with $+$ off
$\texttt{cubicBox}\,d\,n$ \emph{and} $\sigma_2$ is all-$+$ on the shell (case
analysis on $i.\mathrm{val} \in \texttt{cubicBox}\,d\,m$, via the equiv component
lemmas). This is the $+$-shell indicator collapse that makes the $+$ box partition
function and numerator factor over the box-$m$ ones times a shell constant (the
final assembly of the screening).

\subsection{Cubic-box screening: pointwise $+$ weight factoring
(\texttt{Concrete/LatticeGraphCorrelation/CubicBoxScreeningCapstone.lean})}

The per-configuration heart of the screening.
\texttt{edgeSpin\_eq\_one\_of\_agreesOff\_extra}: for $\sigma$ agreeing with $+$ off
$\texttt{cubicBox}\,d\,n$, every extra edge has $\mathrm{edgeSpin} = 1$ (both
endpoints outside $\texttt{cubicBox}\,d\,n$ by \S above). \texttt{cubicBoxShellConst}
is the shell constant
$\exp(-\beta\,(-h\cdot\#\mathrm{compl} - J\cdot\#\mathrm{extra}))$. Then
\texttt{boltzmannWeightBC\_cubicBox\_succ\_pointwise}: under
\texttt{configEquivSubtypeProd},
\[
  w^{+}_{\texttt{box}\,(m+1)}\big((\mathrm{equiv})^{-1}(\sigma_1,\sigma_2)\big)
    = w^{+}_{\texttt{box}\,m}(\sigma_1) \cdot
      \begin{cases} \mathrm{shellConst} & \sigma_2 \equiv +\\ 0 & \text{else}
      \end{cases}
\]
(agreeing case: \texttt{boltzmannWeight\_inducedGraph\_restrict\_factor\_const} $+$
the BC$\leftrightarrow$uniform bridge $+$ the agrees-decomposition; non-agreeing
case $= 0$ via the boundary indicator). This factors the $+$ box partition function
and numerator over the box-$m$ ones (final $Z/N$ assembly next).

\subsection{Cubic-box screening: single-step ambient independence
(\texttt{Concrete/LatticeGraphCorrelation/CubicBoxScreeningLimit.lean})}

The $+$ box state is independent of the ambient box size (nearest-neighbour
screening). \texttt{bcSum\_cubicBox\_succ\_factor}: for an observable depending only
on the inner configuration, the boundary-condition sum over
$\texttt{cubicBox}\,d\,(m+1)$ factors as the box-$m$ sum times the shell constant
(reindex by \texttt{configEquivSubtypeProd} $+$ the pointwise factoring $+$ the
shell collapse \texttt{sum\_shell\_ite\_eq}). Hence
$Z^{+}_{m+1} = Z^{+}_m \cdot \mathrm{shellConst}$
(\texttt{partitionFunctionBC\_cubicBox\_succ}) and the shell constant cancels in the
normalised ratio (\texttt{gibbsExpectationBC\_cubicBox\_succ}). In particular
(\texttt{plusBoxSpin\_screening\_succ})
\[
  \texttt{plusBoxSpin}\,d\,n\,(m+1)\,\dots\,x
    = \texttt{plusBoxSpin}\,d\,n\,m\,\dots\,x
  \qquad (n+1 \le m,\ x \in \texttt{cubicBox}\,d\,m):
\]
the single-site $+$ box spin depends only on the immediate $+$ boundary layer, not
the ambient box size. The remaining steps (induction to full ambient independence
and the $n \to \infty$ monotone-convergence limit) complete the infinite-volume $+$
state.

\subsection{Infinite-volume $+$ expectation of a single spin on $\mathbb Z^d$
(\texttt{Concrete/LatticeGraphCorrelation/InfiniteVolumePlusState.lean})}

The culmination of the cubic-box $+$-state programme: the infinite-volume $+$
boundary expectation of a single spin exists as the monotone limit along the cubic
exhaustion. For a fixed site $x \in \texttt{cubicBox}\,d\,N$, the screened
single-site $+$ box spins
$k \mapsto \texttt{plusBoxSpin}\,d\,(N+k)\,(N+k+1)\,\dots\,x$ (free inner box
$\texttt{cubicBox}\,d\,(N+k)$, immediate $+$ boundary layer) are \emph{antitone}
(\texttt{plusBoxSpin\_infiniteVolume\_antitone}: FV~Lemma~3.22
$+$ the ambient screening) and bounded in $[-1, 1]$, so they converge
(\texttt{tendsto\_plusBoxSpin\_infiniteVolume}, via \texttt{tendsto\_atTop\_ciInf}):
\[
  \texttt{plusBoxSpin}\,d\,(N+k)\,(N+k+1)\,\dots\,x
    \;\longrightarrow\;
  \inf_{k} \texttt{plusBoxSpin}\,d\,(N+k)\,(N+k+1)\,\dots\,x
  \qquad (k \to \infty),
\]
the infinite-volume $+$ expectation of $\sigma_x$ --- a contribution to the
project's central long-term goal, the thermodynamic limit / extremal $+$ state.
Extending the limit to all local observables (the full state functional) is
follow-up.

\subsection{Infinite-volume $+$ expectation of a monotone local observable
(\texttt{Concrete/LatticeGraphCorrelation/LocalObservableLimit.lean})}

The single-spin limit generalises verbatim to an arbitrary \emph{monotone local
observable}, giving the standard $+$-state API. A
\texttt{LocalMonotoneObservable}~$d$ bundles a finite support
$S : \texttt{Finset}\,(\texttt{Fin}\,d \to \mathbb Z)$, a function
$\varphi : \texttt{Config}\,S \to \mathbb R$, and a proof that $\varphi$ is
monotone in the spin order. Its lift to a containing box,
$\texttt{lift}\,h_S = \varphi \circ \texttt{restrictConfig}\,h_S$, is again
monotone (\texttt{lift\_monotone}, via \texttt{restrictConfig\_monotone}), and
depends only on the inner configuration (\texttt{restrictConfig\_trans}), so the
ambient screening applies unchanged
(\texttt{plusBoxLocalExpectation\_screening\_succ}). Being a monotone function on
the \emph{finite} configuration space $\texttt{Config}\,S$, $\varphi$ attains a
minimum, which bounds the screened expectations from below
(\texttt{plusBoxLocalExpectation\_ge}) --- the bounded-below input replacing
$[-1,1]$. Hence for $S \subseteq \texttt{cubicBox}\,d\,N$ the screened
$+$ local expectations are antitone
(\texttt{plusBoxLocalExpectation\_infiniteVolume\_antitone}) and converge
(\texttt{tendsto\_plusBoxLocalObservable\_infiniteVolume}, via
\texttt{tendsto\_atTop\_ciInf}):
\[
  \texttt{plusBoxLocalExpectation}\,(N+k)\,(N+k+1)\,\dots\,O
    \;\longrightarrow\;
  \inf_{k} \texttt{plusBoxLocalExpectation}\,(N+k)\,(N+k+1)\,\dots\,O
  \qquad (k \to \infty).
\]
Since any local observable is a difference of monotone ones, this is the building
block of the full $+$-state functional.

\subsection{The cubic-exhaustion $+$-state functional
(\texttt{Concrete/LatticeGraphCorrelation/LocalObservableState.lean})}

The monotone limit extends to \emph{all} local observables, yielding the
cubic-exhaustion $+$-state functional and its \emph{same-support} linearity,
normalisation, and positivity (a step towards the FV~Theorem~3.17 $+$ state;
sequence independence, translation invariance, the $-$ state, and cross-support
union linearity remain follow-up).  A \texttt{LocalObservable}~$d$ is an arbitrary real function
$\varphi : \texttt{Config}\,S \to \mathbb R$ on a finite support (no
monotonicity).  The key elementary fact is the \emph{monotone-difference
decomposition}, playing the role of FV~Lemma~3.19: writing
$\texttt{rank}\,\sigma = \#\{i \in S : \sigma_i = +\}$ (monotone,
\texttt{configUpRank\_mono}, strictly increasing along covering steps,
\texttt{configUpRank\_lt\_of\_lt}) and $K = 2\max_\sigma |\varphi(\sigma)|$, the
function
\[
  \varphi = \underbrace{(K\cdot\texttt{rank} + \varphi)}_{=:~\varphi_+}
            - \underbrace{(K\cdot\texttt{rank})}_{=:~\varphi_-}
\]
is a difference of two \emph{monotone} functions: across any single covering step
$\texttt{rank}$ increases by at least $1$, and $K$ dominates the variation
$|\varphi(\sigma') - \varphi(\sigma)| \le 2\max|\varphi| = K$, so $\varphi_+$ is
nondecreasing (\texttt{LocalObservable.upper}/\texttt{lower},
\texttt{phi\_eq\_upper\_sub\_lower}).

The $+$ box expectation of an arbitrary observable
(\texttt{plusBoxObsExpectation}) is linear in $\varphi$ (from
\texttt{gibbsExpectationBC\_add}/\texttt{const\_mul}/\texttt{const}) and splits as
$\langle\varphi\rangle = \langle\varphi_+\rangle - \langle\varphi_-\rangle$
(\texttt{plusBoxObsExpectation\_eq\_sub}).  Defining the $+$-state functional as
the difference of the two monotone limits
(\texttt{plusStateExpectation}), the screened box expectations converge to it
(\texttt{tendsto\_plusStateExpectation}), and uniqueness of limits propagates the
finite-volume algebra: the functional is linear (on a fixed support,
\texttt{plusStateExpectation\_add}, \texttt{\_const\_mul}), normalised
($\langle c\rangle = c$, \texttt{\_const}), positive
($\varphi \ge 0 \Rightarrow \langle\varphi\rangle \ge 0$, \texttt{\_nonneg}),
monotone (\texttt{\_mono}), and agrees with the monotone-observable limit on
monotone observables (\texttt{\_of\_monotone}).
...
$\lvert n \rvert = (\lvert n \rvert_{\mathbb N} : \mathbb Z)$.
The shortest-path metric agrees with this $\ell^1$ distance:
\texttt{latticeGraph\_dist\_eq\_latticeDistance} ($(\mathrm{latticeGraph}\,d).\mathrm{dist}\,x\,y
= \mathrm{latticeDistance}\,d\,x\,y$), using \texttt{latticeGraph\_connected},
the inductive geodesic step \texttt{latticeDistance\_exists\_adj\_step} (move one
coordinate toward $y$, reducing the distance by one), the geodesic walk
\texttt{latticeGraph\_exists\_walk\_length} (length $=$ distance, giving $\le$),
and the walk-length lower bound \texttt{latticeDistance\_le\_walk\_length} (giving $\ge$). The
companion Chebyshev ($\ell^\infty$) distance \texttt{latticeDistanceInf} $=
\max_i \lvert x_i - y_i\rvert$ is the lattice \emph{hyperplane separation} of
Glimm--Jaffe~\S17.5 (p.~312), in which the transfer matrix yields exponential
decay $e^{-m\cdot\mathrm{dist}}$. With \texttt{latticeDistanceInf\_self},
\texttt{latticeDistanceInf\_comm}, \texttt{latticeDistanceInf\_eq\_zero\_iff},
\texttt{latticeDistanceInf\_triangle}, and \texttt{latticeDistanceInf\_pos\_of\_ne}
(the $\ell^\infty$ pseudo-metric structure), together with
\texttt{latticeDistanceInf\_le\_latticeDistance}
($\ell^\infty \le \ell^1$), and \texttt{latticeDistance\_le\_card\_mul\_latticeDistanceInf}
($\lvert x-y\rvert_1 \le d\,\lvert x-y\rvert_\infty$), this records the geometric
input $\mathrm{dist} \ge \lvert x-y\rvert/a_0$ with $a_0 = d$ that converts
hyperplane-separation decay into $\ell^1$-distance decay at rate $m/d$. The
rate conversion itself (\texttt{ChebyshevRateBridge.lean}) is
\texttt{rate\_div\_card\_mul\_latticeDistance\_le} ($\tfrac{m}{d}\lvert x-y\rvert_1
\le m\lvert x-y\rvert_\infty$ for $m\ge 0$), \texttt{exp\_neg\_rate\_latticeDistanceInf\_le}
($e^{-m\lvert x-y\rvert_\infty} \le e^{-(m/d)\lvert x-y\rvert_1}$), and
\texttt{le\_exp\_neg\_rate\_latticeDistance\_of\_le\_exp\_neg\_latticeDistanceInf}
(a bound $c \le A e^{-m\lvert x-y\rvert_\infty}$ transfers to $c \le A
e^{-(m/d)\lvert x-y\rvert_1}$), exactly the step turning transfer-matrix decay
into the $e^{-m\cdot\mathrm{dist}}$ bound of Glimm--Jaffe~\S17.5. The bridge to
the project's $\ell^1$ machinery is \texttt{HasExponentialDecay\_of\_latticeDistanceInf\_bound}:
a hyperplane-separation decay $\lvert S^T_2(i,j)\rvert \le A e^{-m\lvert i-j\rvert_\infty}$
($i \ne j$) implies $\mathrm{HasExponentialDecay}\,d\,\Lambda\,p\,(m/d)$, feeding a
future transfer-matrix decay estimate into the $\mathrm{latticeMass}$ development
without the spectral operator. The
...
\paragraph{1D Ising transfer matrix (\S17.1, Issue~\#3511 PR~1).}
The concrete instance of the Glimm--Jaffe~\S17.1 transfer-matrix method is the
$2\times 2$ real matrix governing the one-dimensional Ising chain at zero
external field. With the two spin values $s : \mathrm{Fin}\,2 \to \mathbb R$,
$s_0 = +1$, $s_1 = -1$ (\texttt{spin1D}), the transfer matrix is
\texttt{isingTransferMatrix1D}$(a)_{ij} = e^{a\,s_i s_j}$ with $a = \beta J$, i.e.
$T(a) = \bigl(\begin{smallmatrix} e^a & e^{-a} \\ e^{-a} & e^a \end{smallmatrix}\bigr)$.
It is symmetric (\texttt{isingTransferMatrix1D\_transpose}), entrywise positive
(\texttt{isingTransferMatrix1D\_pos}), with trace $2e^a$
(\texttt{trace\_isingTransferMatrix1D}) and determinant $e^{2a}-e^{-2a}$
(\texttt{det\_isingTransferMatrix1D}). The Hadamard vectors $(1,1)$ and $(1,-1)$
are eigenvectors (\texttt{isingTransferMatrix1D\_mulVec\_hadamardTop} /
\texttt{...\_hadamardBot}) with eigenvalues
$\lambda_+ = e^a + e^{-a} = 2\cosh a$ (\texttt{transferEigenvalueTop}) and
$\lambda_- = e^a - e^{-a} = 2\sinh a$ (\texttt{transferEigenvalueBot}). The
spectral ratio $\lambda_-/\lambda_+ = \tanh a$
(\texttt{transferEigenvalue\_ratio}) is the source of the correlation decay rate
$-\log\tanh a$ for the ferromagnetic one-dimensional chain ($a = \beta J > 0$;
Glimm--Jaffe~\S17.5).

\paragraph{Transfer matrix with external field (\S17.1, Issue~\#3538 PR~1).}
The zero-field matrix extends to a general external field $h \ne 0$. Writing
$a = \beta J$, $b = \beta h$, the symmetric transfer matrix is
\texttt{isingTransferMatrix1DField}$(a,b)_{ij} =
\exp(a\,s_i s_j + \tfrac{b}{2}(s_i + s_j))$, explicitly
\[
  T(a,b) = \begin{pmatrix} e^{a+b} & e^{-a} \\ e^{-a} & e^{a-b} \end{pmatrix},
\]
symmetric (\texttt{isingTransferMatrix1DField\_transpose}), entrywise positive
(\texttt{isingTransferMatrix1DField\_pos}), with trace $2e^a\cosh b$
(\texttt{trace\_isingTransferMatrix1DField}) and determinant $e^{2a} - e^{-2a}$
(\texttt{det\_isingTransferMatrix1DField}). Unlike the zero-field case the
eigenvectors are no longer the fixed Hadamard pair, so the spectral data is
organised through the characteristic polynomial. With discriminant
$D(a,b) = e^{2a}\sinh^2 b + e^{-2a} > 0$
(\texttt{fieldTransferDiscriminant}, \texttt{fieldTransferDiscriminant\_pos}) the
eigenvalues are
\[
  \lambda_\pm = e^a\cosh b \pm \sqrt{D}
  \qquad (\texttt{fieldTransferEigenvalueTop} / \texttt{Bot}),
\]
satisfying Vieta's relations $\lambda_+ + \lambda_- = \mathrm{tr}\,T = 2e^a\cosh b$
(\texttt{fieldTransferEigenvalueTop\_add\_bot}) and $\lambda_+\lambda_- = \det T
= e^{2a} - e^{-2a}$ (\texttt{fieldTransferEigenvalueTop\_mul\_bot}, using
$\sqrt{D}^2 = D$ and $\cosh^2 b = \sinh^2 b + 1$), hence the characteristic
equation $\lambda_\pm^2 = (\mathrm{tr}\,T)\,\lambda_\pm - \det T$
(\texttt{fieldTransferEigenvalue\{Top,Bot\}\_sq}). The eigenvalues are ordered
$\lambda_- < \lambda_+$ (\texttt{fieldTransferEigenvalueBot\_lt\_top}) and
positive, $0 < \lambda_+$ always and $0 < \lambda_-$ for $a > 0$
(\texttt{fieldTransferEigenvalueTop\_pos},
\texttt{fieldTransferEigenvalueBot\_pos}). At $b = 0$ the matrix and eigenvalues
reduce to the zero-field objects (\texttt{isingTransferMatrix1DField\_zero},
\texttt{fieldTransferEigenvalue\{Top,Bot\}\_zero}). Foundational PR of the
general external-field 1D programme (Issue~\#3538); the partition function
$Z_N = \lambda_+^N + \lambda_-^N$, free energy, magnetization $m(h)$, and
susceptibility $\partial_h m$ follow in later PRs.

\paragraph{Field partition function and free energy (\S17.1, Issue~\#3538 PR~2).}
With no Hadamard eigenbasis available for $h \ne 0$, the cyclic-chain partition
function $Z_N = \mathrm{Tr}\,T(a,b)^N$ is computed through the Cayley--Hamilton
recurrence. From \texttt{Matrix.aeval\_self\_charpoly} and
\texttt{Matrix.charpoly\_fin\_two} one gets
$T^2 = (\mathrm{tr}\,T)\,T - (\det T)\,\mathbb 1$
(\texttt{isingTransferMatrix1DField\_sq}), hence the matrix recurrence
$T^{N+2} = \mathrm{tr}\cdot T^{N+1} - \det\cdot T^N$
(\texttt{isingTransferMatrix1DField\_pow\_succ\_succ}) and, by linearity of the
trace, $s_{N+2} = \mathrm{tr}\cdot s_{N+1} - \det\cdot s_N$ with $s_N =
\mathrm{Tr}\,T^N$
(\texttt{trace\_isingTransferMatrix1DField\_pow\_succ\_succ}). The eigenvalue
powers $\lambda_\pm^N$ obey the same recurrence (the characteristic equation
$\lambda_\pm^2 = \mathrm{tr}\,\lambda_\pm - \det$) with the same initial data
$s_0 = 2 = \lambda_+^0 + \lambda_-^0$, $s_1 = \mathrm{tr} = \lambda_+ +
\lambda_-$, so two-step induction (\texttt{Nat.twoStepInduction}) gives
\[
  \mathrm{Tr}\,T(a,b)^N = \lambda_+^N + \lambda_-^N
  \qquad (\texttt{trace\_isingTransferMatrix1DField\_pow}).
\]
For $a = \beta J > 0$ the subdominant ratio $\lambda_-/\lambda_+ \in [0,1)$
(\texttt{fieldTransferEigenvalue\_ratio\_\{nonneg,lt\_one\}}) gives
$(\lambda_-/\lambda_+)^N \to 0$
(\texttt{tendsto\_fieldTransferEigenvalue\_ratio\_pow}) and the free-energy
density $\tfrac1N \log(\lambda_+^N + \lambda_-^N) \to \log\lambda_+$
(\texttt{tendsto\_log\_fieldEigenvalueSum\_div\_nat}), hence
$\tfrac1N \log\mathrm{Tr}\,T(a,b)^N \to \log\lambda_+$
(\texttt{tendsto\_log\_trace\_isingTransferMatrix1DField\_pow\_div\_nat}) --- the
free-energy density of the 1D Ising chain in a field. Issue~\#3538 PR~2; next:
magnetization $m(h)$ and susceptibility $\partial_h m$.

\paragraph{Field magnetization (\S17.1, Issue~\#3538 PR~3).}
The magnetization of the 1D Ising chain in a field is the field-derivative of the
free-energy density $\log\lambda_+$:
\[
  m(h) = \partial_b \log\lambda_+(a,b)
       = \frac{\sinh b}{\sqrt{\sinh^2 b + e^{-4a}}}
  \qquad (\texttt{fieldMagnetization}).
\]
The closed form rests on the discriminant factorisation $\sqrt{D} =
e^a\sqrt{\sinh^2 b + e^{-4a}}$ (\texttt{sqrt\_fieldTransferDiscriminant}, from
$D = e^{2a}(\sinh^2 b + e^{-4a})$ and $\sqrt{e^{2a}} = e^a$). Differentiating
$D$ (\texttt{hasDerivAt\_fieldTransferDiscriminant}, $\partial_b D =
e^{2a}\cdot 2\sinh b\cosh b$), then $\sqrt{D}$
(\texttt{HasDerivAt.sqrt}), $e^a\cosh b$ (\texttt{Real.hasDerivAt\_cosh}), and
$\log\lambda_+$ (\texttt{HasDerivAt.log}), the logarithmic derivative
$(\partial_b\lambda_+)/\lambda_+$ collapses to $\sinh b/\sqrt{\sinh^2 b +
e^{-4a}}$ (\texttt{hasDerivAt\_log\_fieldTransferEigenvalueTop}). The
magnetization vanishes at zero field, $m(a,0) = 0$
(\texttt{fieldMagnetization\_zero}) --- no spontaneous magnetization in one
dimension --- and is strictly sub-saturated, $|m| < 1$
(\texttt{abs\_fieldMagnetization\_lt\_one}), since $\sqrt{\sinh^2 b + e^{-4a}} >
|\sinh b|$ (the field gap $e^{-4a} > 0$). Issue~\#3538 PR~3; next: susceptibility
$\partial_h m$.

\paragraph{Field susceptibility (\S17.1, Issue~\#3538 PR~4).}
Differentiating the magnetization once more gives the susceptibility
\[
  \chi = \partial_b m
       = \frac{\cosh b\,e^{-4a}}{(\sinh^2 b + e^{-4a})^{3/2}}
  \qquad (\texttt{fieldSusceptibility}),
\]
strictly positive (\texttt{fieldSusceptibility\_pos}). By the quotient rule
(\texttt{HasDerivAt.div}) on $m = \sinh b/\sqrt{X}$ with $X = \sinh^2 b +
e^{-4a}$, using $(\sqrt{X})^2 = X$ and $X - \sinh^2 b = e^{-4a}$, the derivative
collapses to the closed form (\texttt{hasDerivAt\_fieldMagnetization}). The
physical susceptibility is $\partial_h m = \beta\,\chi$
(\texttt{hasDerivAt\_fieldMagnetization\_field}, chain rule through $b = \beta h$),
and at zero field $\chi(a,0) = e^{2a}$ (\texttt{fieldSusceptibility\_zero}), so
$\partial_h m|_0 = \beta e^{2a}$ --- the gap-enhanced zero-field susceptibility of
the 1D chain. This completes the exact solution of the 1D Ising chain in a field:
matrix $\to$ eigenvalues $\to$ partition function $\to$ free energy $\to$
magnetization $\to$ susceptibility (Issue~\#3538).

\paragraph{Diagonalization and matrix powers (\S17.1, Issue~\#3511 PR~2).}
The Hadamard matrix $H = \bigl(\begin{smallmatrix} 1 & 1 \\ 1 & -1 \end{smallmatrix}\bigr)$
(\texttt{hadamardMatrix}) satisfies $H^2 = 2\,\mathbb 1$
(\texttt{hadamardMatrix\_mul\_self}), $\det H = -2$
(\texttt{hadamardMatrix\_det}), $H^{-1} = \tfrac12 H$
(\texttt{hadamardMatrix\_inv}). With $D(a) = \mathrm{diag}(\lambda_+, \lambda_-)$
(\texttt{transferDiagonal}), the inverse-free Hadamard intertwining
$T(a)\,H = H\,D(a)$ (\texttt{isingTransferMatrix1D\_mul\_hadamard}) powers by
induction to $T(a)^N H = H\,D(a)^N$
(\texttt{isingTransferMatrix1D\_pow\_mul\_hadamard}), i.e.\ the diagonalization
$T(a)^N = H\,D(a)^N H^{-1}$ (\texttt{isingTransferMatrix1D\_pow\_eq\_conj}) with
$D(a)^N = \mathrm{diag}(\lambda_+^N, \lambda_-^N)$
(\texttt{transferDiagonal\_pow}). The trace is the cyclic-chain partition
function (for physical $N \ge 1$) $Z_N = \operatorname{Tr}(T(a)^N) = \lambda_+^N + \lambda_-^N$
(\texttt{trace\_isingTransferMatrix1D\_pow}, a pure algebraic identity for all $N$, via \texttt{trace\_mul\_comm} and
$H^{-1}H = \mathbb 1$), and $\det(T(a)^N) = (\lambda_+\lambda_-)^N$
(\texttt{det\_isingTransferMatrix1D\_pow}) -- the transfer-matrix solution of the
one-dimensional Ising chain (Glimm--Jaffe~\S17.1).

\paragraph{1D free-energy density (\S17.1, Issue~\#3511 PR~3).}
Building on $\operatorname{Tr}(T(a)^N) = \lambda_+^N + \lambda_-^N$, the per-site
log-partition (dimensionless free-energy) density of the one-dimensional chain
($a = \beta J > 0$) is
$\lim_{N\to\infty} \tfrac1N \log \operatorname{Tr}(T(a)^N) = \log \lambda_+ = \log(2\cosh a)$
(\texttt{tendsto\_log\_trace\_pow\_div\_nat}, \texttt{log\_transferEigenvalueTop\_eq}).
The mechanism is the spectral gap $\lambda_- < \lambda_+$
(\texttt{transferEigenvalueBot\_lt\_top}) with ratio
$\lambda_-/\lambda_+ = \tanh a \in (0,1)$
(\texttt{transferEigenvalue\_ratio\_pos}, \texttt{transferEigenvalue\_ratio\_lt\_one}):
writing $\lambda_+^N + \lambda_-^N = \lambda_+^N (1 + \tanh^N a)$, the subdominant
eigenvalue contributes the vanishing correction
$\tfrac1N \log(1 + \tanh^N a) \to 0$ (since $\tanh^N a \to 0$,
\texttt{tendsto\_transferEigenvalue\_ratio\_pow}), leaving the dominant
$\log\lambda_+$ (\texttt{tendsto\_log\_eigenvalueSum\_div\_nat}).  Since
$Z_N = \operatorname{Tr}(T(a)^N)$ for the cyclic chain, this is the transfer-matrix
solution of the 1D Ising free energy $f = -\beta^{-1}\log(2\cosh\beta J)$
(Glimm--Jaffe~\S17.1).

\paragraph{Closed-walk trace identity (\S17.1, Issue~\#3511 PR~4).}
The combinatorial core of $Z_N = \operatorname{Tr}(T^N)$, for an arbitrary finite
matrix $M : \mathrm{Matrix}\,\iota\,\iota\,R$ over a commutative semiring (Mathlib
has no such lemma, only the $0/1$ adjacency case). With the open-path weight
$\texttt{pathWeight}\,M\,\sigma = \prod_{i : \mathrm{Fin}\,n} M(\sigma\,i_{\mathrm{cast}})(\sigma\,i_{\mathrm{succ}})$
and the append rule
$\texttt{pathWeight}(M, \mathrm{snoc}\,\sigma\,x) = \texttt{pathWeight}(M,\sigma)\cdot M(\sigma(\mathrm{last}\,n))\,x$
(\texttt{pathWeight\_snoc}), the entry formula
$(M^n)_{ab} = \sum_{\sigma : \mathrm{Fin}(n{+}1)\to\iota} [\sigma_0 = a \wedge \sigma_{\mathrm{last}} = b]\,\texttt{pathWeight}\,M\,\sigma$
(\texttt{pow\_apply\_eq\_sum}) is proved by induction, peeling the last edge via
$\texttt{Fin.snocEquiv}$. Specialising to the diagonal and summing gives the
\emph{closed-walk trace identity}
$\operatorname{Tr}(M^n) = \sum_{\sigma : \mathrm{Fin}(n{+}1)\to\iota} [\sigma_0 = \sigma_{\mathrm{last}}]\,\texttt{pathWeight}\,M\,\sigma$
(\texttt{trace\_pow\_eq\_sum}), a weighted sum over vertex sequences returning to
their start. The cyclic form
$\texttt{closedWalkWeight}\,M\,\sigma = \prod_{i} M(\sigma\,i)(\sigma(i{+}1))$
(cyclic in $\mathrm{Fin}\,n$) factors as
$\texttt{closedWalkWeight}\,\tau = \texttt{pathWeight}\,\tau \cdot M(\tau(\mathrm{last}))(\tau\,0)$
(\texttt{closedWalkWeight\_succ}), underlying the cyclic 1D Ising
$Z_N = \operatorname{Tr}(T^N)$ (Glimm--Jaffe~\S17.1).

\paragraph{Cyclic trace identity and 1D evaluation (\S17.1, Issue~\#3511 PR~5).}
Reindexing the open-return form by the \texttt{init}/\texttt{snoc} bijection
($\texttt{Finset.sum\_bij'}$) between vertex sequences with $\sigma_0 = \sigma_{\mathrm{last}}$
and closed walks (using $\texttt{pathWeight\_eq\_closedWalkWeight\_init}$,
$\texttt{pathWeight}\,M\,\sigma = \texttt{closedWalkWeight}\,M\,(\mathrm{init}\,\sigma)$
when $\sigma_0 = \sigma_{\mathrm{last}}$) yields the purely cyclic identity
$\operatorname{Tr}(M^{m+1}) = \sum_{\tau : \mathrm{Fin}(m{+}1)\to\iota} \texttt{closedWalkWeight}\,M\,\tau$
(\texttt{trace\_pow\_eq\_sum\_cycle}). Specialising to the 1D Ising transfer
matrix $T(a)$ over the spin space $\mathrm{Fin}\,2$ and combining with the
spectral trace $\operatorname{Tr}(T(a)^{m+1}) = \lambda_+^{m+1} + \lambda_-^{m+1}$
gives the cyclic partition function as eigenvalue powers,
$\sum_{\sigma : \mathrm{Fin}(m{+}1)\to\mathrm{Fin}\,2} \texttt{closedWalkWeight}\,T(a)\,\sigma = \lambda_+^{m+1} + \lambda_-^{m+1}$
(\texttt{sum\_closedWalkWeight\_isingTransferMatrix1D}), i.e.\ the transfer-matrix
evaluation $Z_N = \operatorname{Tr}(T^N) = \lambda_+^N + \lambda_-^N$ of the cyclic
$N$-site zero-field Ising chain (Glimm--Jaffe~\S17.1).

\paragraph{Two-point function via the transfer matrix (\S17.1, Issue~\#3511 PR~6).}
The transfer-matrix expression for the two-point function is
$\operatorname{Tr}(S\,T^n S\,T^{N-n})/\operatorname{Tr}(T^N)$ (its identification with the
Gibbs $\langle\sigma_0\sigma_n\rangle$ via the cyclic config sum is a later step)
with the spin operator $S = \mathrm{diag}(1,-1)$ (\texttt{spinOperator}). The spin
operator swaps the Hadamard eigenvectors, $S\,H = H\,P$ with $P$ the $2\times2$
swap (\texttt{spinOperator\_mul\_hadamard}), hence $H^{-1}SH = P$ and
$P\,D(a)^k P = \mathrm{diag}(\lambda_-^k, \lambda_+^k)$. This gives the two-point
trace $\operatorname{Tr}(S\,T^n S\,T^m) = \lambda_-^n\lambda_+^m + \lambda_+^n\lambda_-^m$
(\texttt{twoPointTrace}), so
$\langle\sigma_0\sigma_n\rangle_N = (r^n + r^{N-n})/(1 + r^N)$ with $r = \lambda_-/\lambda_+ = \tanh a$
(\texttt{twoPointCorrelation\_eq\_ratio}). For $a = \beta J > 0$ the subdominant
ratio $r \in (0,1)$ makes the boundary terms vanish, giving the exact exponential
decay $\langle\sigma_0\sigma_n\rangle_N \to (\tanh\beta J)^n$ as $N\to\infty$
(\texttt{tendsto\_twoPointCorrelation}), with correlation length
$\xi = -1/\log\tanh\beta J$ (Glimm--Jaffe~\S17.1).

\paragraph{Cycle-graph edge enumeration and partition function (\S17.1, Issue~\#3511 PR~7).}
To bridge the project's graph-based Ising partition function on the cyclic chain
to the transfer-matrix's linear site product, the edges of the cycle graph are
enumerated (Mathlib provides only the adjacency):
$(\texttt{cycleGraph}(n{+}2)).\texttt{edgeFinset} = \{\,s(i, i{+}1) : i \in \mathrm{Fin}(n{+}2)\,\}$
(\texttt{cycleGraph\_edgeFinset\_eq\_image}, via $\texttt{Sym2.ind}$ and
$\texttt{cycleGraph\_adj}$). With the injectivity of $i \mapsto s(i, i{+}1)$ on
$\mathrm{Fin}(n{+}3)$ (\texttt{cyclePair\_injective}) this gives the product
reindexing $\prod_{e} f(e) = \prod_{i} f(s(i,i{+}1))$
(\texttt{prod\_cycleGraph\_edgeFinset}) and the edge count $n{+}3$
(\texttt{card\_cycleGraph\_edgeFinset}). Combined with the $h=0$ Boltzmann edge
product (\texttt{boltzmannWeight\_eq\_prod\_exp\_of\_h\_zero}), the partition
function of the cyclic $N$-site chain ($N = n{+}3$) becomes a linear site product
$Z_N = \sum_\sigma \prod_{i} e^{\beta J\,\sigma_i\sigma_{i+1}}$
(\texttt{partitionFunction\_cycleGraph\_eq\_sum\_prod}) -- the transfer-matrix
shape (Glimm--Jaffe~\S17.1).

\paragraph{Gibbs partition function $Z_N = \lambda_+^N + \lambda_-^N$ (\S17.1, Issue~\#3511 PR~8, capstone).}
Via the spin$\leftrightarrow\mathrm{Fin}\,2$ encoding $\texttt{spinEquivFin2}$
($\mathrm{up}\mapsto 0$, $\mathrm{down}\mapsto 1$), under which
$\texttt{spin1D}(\mathrm{encode}\,s) = \mathrm{sign}(s)$ and the Boltzmann edge
factor becomes a transfer-matrix entry
$T(\beta J)(\mathrm{encode}\,s)(\mathrm{encode}\,t) = e^{\beta J\,\mathrm{sign}(s)\mathrm{sign}(t)}$,
the per-configuration site product is a closed-walk weight of the encoded
configuration (\texttt{prod\_exp\_edgeSpin\_eq\_closedWalkWeight}). Reindexing the
configuration sum along $(\mathrm{Fin}\,N \to \mathrm{Spin}) \simeq (\mathrm{Fin}\,N \to \mathrm{Fin}\,2)$
and applying $\texttt{sum\_closedWalkWeight\_isingTransferMatrix1D}$ gives the
classic transfer-matrix evaluation of the Gibbs partition function,
$\texttt{partitionFunction}(\texttt{cycleGraph}\,N)\,\langle J,0,\beta\rangle = \lambda_+^N + \lambda_-^N = (2\cosh\beta J)^N + (2\sinh\beta J)^N$
(\texttt{partitionFunction\_cycleGraph\_eq\_eigenvaluePow}), completing the
transfer-matrix evaluation of $Z_N$ (Glimm--Jaffe~\S17.1).

\paragraph{Correlation length and mass (\S17.1, \S17.5).}
Writing the two-point decay $\langle\sigma_0\sigma_n\rangle_N \to (\tanh\beta J)^n$
as a pure exponential identifies the mass $m = -\log\tanh\beta J$
(\texttt{correlationMass}, the inverse correlation length, the \S17.5 mass) and
correlation length $\xi = 1/m$ (\texttt{correlationLength}), with $m, \xi > 0$ for
$a = \beta J > 0$ (\texttt{correlationMass\_pos}, \texttt{correlationLength\_pos}).
Then $(\tanh\beta J)^n = e^{-m n}$ (\texttt{tanh\_pow\_eq\_exp\_neg\_mass}) gives
$\langle\sigma_0\sigma_n\rangle_N \to e^{-m n}$
(\texttt{tendsto\_twoPointCorrelation\_exp\_neg\_mass}) -- exponential decay at the
inverse correlation length (Glimm--Jaffe~\S17.1, \S17.5).

\paragraph{Susceptibility (\S17.1).}
Summing the two-point decay rate over separations gives the magnetic
susceptibility. With $r = \tanh\beta J \in [0,1)$ the geometric series
$\sum_n r^n = (1-r)^{-1}$ converges (\texttt{summable\_tanh\_pow},
\texttt{tsum\_tanh\_pow}), and the two-sided lattice sum gives
$\chi = (1+\tanh\beta J)/(1-\tanh\beta J) = 2\sum_n r^n - 1$
(\texttt{isingSusceptibility1D}, \texttt{isingSusceptibility1D\_eq\_two\_tsum\_sub\_one}),
strictly positive and finite for $\beta J > 0$ (\texttt{isingSusceptibility1D\_pos}),
diverging only as $\beta J \to \infty$ -- the 1D Ising chain has no
finite-temperature phase transition (Glimm--Jaffe~\S17.1).

\paragraph{Gibbs free-energy density (\S17.1).}
Combining the eigenvalue free-energy limit with the Gibbs $Z_N = \lambda_+^N + \lambda_-^N$
identity gives the free-energy density of the project's \emph{Gibbs} partition
function on the cyclic chain:
$\tfrac1N \log \texttt{partitionFunction}(\texttt{cycleGraph}\,N)\langle J,0,\beta\rangle \to \log(2\cosh\beta J)$
(\texttt{tendsto\_log\_partitionFunction\_cycleGraph\_div\_nat}, over $N = n{+}3$),
and the physical Helmholtz density
$-(\beta N)^{-1}\log Z_N \to -\beta^{-1}\log(2\cosh\beta J)$
(\texttt{tendsto\_gibbs\_freeEnergy\_density}) -- the Gibbs (not merely
transfer-trace) 1D Ising free-energy density (Glimm--Jaffe~\S17.1).

\paragraph{Trace with diagonal insertions (\S17.1).}
Toward the Gibbs two-point function $\langle\sigma_0\sigma_n\rangle = \operatorname{Tr}(S\,T^n S\,T^{N-n})/\operatorname{Tr}(T^N)$,
the trace with two diagonal insertions $D = \mathrm{diagonal}\,d$ expands as
$\operatorname{Tr}(D\,M^a D\,M^b) = \sum_{i,j} d_i\,d_j\,(M^a)_{ij}(M^b)_{ji}$
(\texttt{trace\_diagonal\_pow\_diagonal\_pow\_expand}), with the single-insertion
form $\operatorname{Tr}(D\,M^a) = \sum_i d_i (M^a)_{ii}$
(\texttt{trace\_diagonal\_mul\_pow}) and the cyclic symmetry
$\operatorname{Tr}(D\,M^a D\,M^b) = \operatorname{Tr}(D\,M^b D\,M^a)$
(\texttt{trace\_diagonal\_pow\_diagonal\_pow\_comm}); the marked closed-walk form
$\sum_\tau d(\tau_0)d(\tau_a)\,\texttt{closedWalkWeight}\,M\,\tau$ (gluing two open
paths) is the next step (Glimm--Jaffe~\S17.1).

\paragraph{Cyclic-to-open closed-walk conversion (\S17.1).}
The technical key for the marked closed-walk identity, avoiding all cyclic
$\mathrm{Fin}$-arithmetic, is
$\texttt{closedWalkWeight}\,M\,\tau = \texttt{pathWeight}\,M\,(\mathrm{snoc}\,\tau\,(\tau\,0))$
(\texttt{closedWalkWeight\_eq\_pathWeight\_snoc\_zero}): appending the first vertex
turns the cyclic closed walk into an open path returning to its start, whose
open-path weight is the cyclic weight. The marked summand
$\texttt{markedClosedWalkWeight}\,M\,d\,\tau = d(\tau_0)d(\tau_a)\,\texttt{closedWalkWeight}\,M\,\tau$
then has the open-path form \texttt{markedClosedWalkWeight\_eq} (Glimm--Jaffe~\S17.1).

\paragraph{Path gluing (\S17.1).}
Gluing two open paths $\sigma : \mathrm{Fin}(a{+}1)\to\iota$,
$\rho : \mathrm{Fin}(b{+}1)\to\iota$ at a shared vertex ($\rho_0 = \sigma_{\mathrm{last}}$)
by appending the interior of $\sigma$ to $\rho$ multiplies their path weights,
$\texttt{pathWeight}\,M\,(\mathrm{append}\,(\mathrm{init}\,\sigma)\,\rho) = \texttt{pathWeight}\,M\,\sigma \cdot \texttt{pathWeight}\,M\,\rho$
(\texttt{pathWeight\_append}, by induction on $b$ peeling $\rho$ via
$\texttt{Fin.append\_snoc}$). The marked glue
$\texttt{markedGlue}\,\sigma\,\rho = \mathrm{append}\,(\mathrm{init}\,\sigma)\,(\mathrm{init}\,\rho)$
is the closed walk with marks $\texttt{markedGlue}\,\sigma\,\rho\,0 = \sigma_0$ and at
site $a$ equal to $\rho_0$ (Glimm--Jaffe~\S17.1).
Under the gluing conditions $\rho_0 = \sigma_{\mathrm{last}}$ and
$\rho_{\mathrm{last}} = \sigma_0$, its cyclic closed-walk weight factors as
$\texttt{closedWalkWeight}\,M\,(\texttt{markedGlue}\,\sigma\,\rho) = \texttt{pathWeight}\,M\,\sigma \cdot \texttt{pathWeight}\,M\,\rho$
(\texttt{closedWalkWeight\_markedGlue}), via the open-path conversion and
$\texttt{pathWeight\_append}$.

\paragraph{Marked trace as a sum over glued pairs (\S17.1).}
Reordering four nested sums (\texttt{sum\_reorder\_4}, a \texttt{calc} chain of
$\texttt{Finset.sum\_comm}$) and collapsing the endpoint sums, the trace with two
diagonal insertions becomes a sum over glued open-path pairs:
$\operatorname{Tr}(D\,M^a D\,M^b) = \sum_{\sigma,\rho} [\sigma_{\mathrm{last}}=\rho_0 \wedge \rho_{\mathrm{last}}=\sigma_0]\, d(\sigma_0)d(\rho_0)\,\texttt{pathWeight}\,M\,\sigma\cdot\texttt{pathWeight}\,M\,\rho$
(\texttt{trace\_diagonal\_pow\_diagonal\_pow\_eq\_sum\_glued}). With
\texttt{closedWalkWeight\_markedGlue} this is the marked closed-walk trace identity
up to the path-pair bijection (Glimm--Jaffe~\S17.1).

\paragraph{Marked closed-walk trace identity (\S17.1).}
The reconstruction maps $\texttt{σOf}\,\tau = \mathrm{snoc}\,(\tau\circ\mathrm{castAdd}\,b)\,(\tau\,a)$,
$\texttt{ρOf}\,\tau = \mathrm{snoc}\,(\tau\circ\mathrm{natAdd}\,a)\,(\tau\,0)$ satisfy
$\texttt{markedGlue}\,(\texttt{σOf}\,\tau)\,(\texttt{ρOf}\,\tau) = \tau$
(\texttt{markedGlue\_σOf\_ρOf}, via $\texttt{Fin.append\_castAdd\_natAdd}$). The
$\texttt{Finset.sum\_bij'}$ bijection between closed walks and glued path pairs then
yields the marked closed-walk trace identity
$\operatorname{Tr}(D\,M^a D\,M^b) = \sum_{\tau : \mathrm{Fin}(a{+}b)\to\iota} d(\tau_0)\,d(\tau_a)\,\texttt{closedWalkWeight}\,M\,\tau$
(\texttt{trace\_diagonal\_pow\_diagonal\_pow\_eq\_sum\_markedClosedWalk}) -- the
combinatorial core of the Gibbs two-point function (Glimm--Jaffe~\S17.1).

\paragraph{1D marked closed-walk sum (\S17.1).}
Specialising to the 1D Ising transfer matrix $T(a)$ with marks
$\texttt{spin1D} = (1,-1)$ (so the marking matrix is $S = \mathrm{diagonal}\,\texttt{spin1D}$,
\texttt{spinOperator\_eq\_diagonal\_spin1D}) and combining with \texttt{twoPointTrace},
the closed-walk form of the two-point numerator is
$\sum_{\tau : \mathrm{Fin}(n{+}m)\to\mathrm{Fin}\,2} \texttt{markedClosedWalkWeight}\,T(a)\,\texttt{spin1D}\,\tau = \lambda_-^n\lambda_+^m + \lambda_+^n\lambda_-^m$
(\texttt{sum\_markedClosedWalkWeight\_isingTransferMatrix1D}, $= \operatorname{Tr}(S\,T^n S\,T^m)$),
the bridge toward the Gibbs two-point function (Glimm--Jaffe~\S17.1).

\paragraph{Gibbs two-point function as the transfer-matrix ratio (\S17.1).}
This is the standard cyclic transfer-matrix computation of the 1D Ising
two-point function, providing the Gibbs reading of the mass and exponential-decay
discussion of Glimm--Jaffe~\S17.1.
The previous paragraphs supply the transfer-matrix numerator
$\operatorname{Tr}(S\,T^n S\,T^m)$ in closed-walk form; it remains to identify the
project's \emph{Gibbs} correlation
$\langle\sigma_0\sigma_n\rangle = \texttt{correlation}\,(\texttt{cycleGraph}\,N)\,\langle J,0,\beta\rangle\,\{0,n\}$
of the two endpoint spins with the transfer-matrix two-point ratio
$\texttt{twoPointCorrelation}\,(\beta J)\,n\,N
= \operatorname{Tr}(S\,T^n S\,T^{N-n})/\operatorname{Tr}(T^N)$.
Using the same spin$\leftrightarrow\mathrm{Fin}\,2$ encoding that evaluates the
partition function (\texttt{partitionFunction\_cycleGraph\_eq\_eigenvaluePow}), the
Gibbs two-point numerator becomes the site-marked closed-walk sum,
\[
  \sum_\sigma \texttt{spinProduct}\,\{0,n\}\,\sigma\,\cdot\,
    \texttt{boltzmannWeight}\,(\texttt{cycleGraph}\,N)\,\langle J,0,\beta\rangle\,\sigma
  \;=\; \sum_{\tau:\mathrm{Fin}\,N\to\mathrm{Fin}\,2}
    \texttt{spin1D}(\tau_0)\,\texttt{spin1D}(\tau_n)\,\texttt{closedWalkWeight}\,T(\beta J)\,\tau,
\]
the two spin insertions $\sigma_0\sigma_n$ turning into the marks
$\texttt{spin1D}(\tau_0)\,\texttt{spin1D}(\tau_n)$
(\texttt{twoPointNumerator\_cycleGraph\_eq\_sum\_siteMarked}).  Unfolding the marked
closed-walk weight, this site-marked sum evaluates (for $N = n+m$, $0<n$, $0<m$) to
$\lambda_-^n\lambda_+^m + \lambda_+^n\lambda_-^m$
(\texttt{sum\_siteMarkedClosedWalkWeight\_isingTransferMatrix1D}).  Dividing by the
partition function $\lambda_+^N + \lambda_-^N$ and matching
\texttt{twoPointCorrelation\_eq} gives the headline identity
\[
  \texttt{correlation}\,(\texttt{cycleGraph}\,N)\,\langle J,0,\beta\rangle\,\{0,n\}
  \;=\; \texttt{twoPointCorrelation}\,(\beta J)\,n\,N
  \qquad (N = k+3 \ge 3,\ 0 < n < N)
\]
(\texttt{correlation\_cycleGraph\_eq\_twoPointCorrelation}).  This turns the abstract
transfer-matrix two-point analysis -- in particular the exact geometric decay
$\langle\sigma_0\sigma_n\rangle \to (\tanh\beta J)^n$ -- into a statement about the
project's Gibbs correlation $\langle\sigma_0\sigma_n\rangle$ (Glimm--Jaffe~\S17.1).

\paragraph{Gibbs-form exponential decay (\S17.1).}
Composing the bridge with the transfer-matrix limits gives the exact decay of the
1D Ising correlation directly for the project's Gibbs \texttt{correlation}.  The
closed forms \texttt{correlation\_cycleGraph\_eq\_eigenvaluePow}
($= (\lambda_-^n\lambda_+^{N-n}+\lambda_+^n\lambda_-^{N-n})/(\lambda_+^N+\lambda_-^N)$)
and \texttt{correlation\_cycleGraph\_eq\_ratio}
($= (r^n+r^{N-n})/(1+r^N)$, $r = \tanh\beta J$) restate
\texttt{twoPointCorrelation\_eq} / \texttt{twoPointCorrelation\_eq\_ratio} in Gibbs
terms.  Parametrising the volume as $N = n+k+3$ (so the sites $0$, $n$ stay valid as
$k\to\infty$ with the endpoint distance $n$ fixed), the headline
\texttt{tendsto\_correlation\_cycleGraph} gives, for $\beta J > 0$ and $0 < n$,
\[
  \texttt{correlation}\,(\texttt{cycleGraph}\,(n{+}k{+}3))\,\langle J,0,\beta\rangle\,\{0,n\}
  \;\xrightarrow[k\to\infty]{}\; (\tanh\beta J)^n,
\]
and \texttt{tendsto\_correlation\_cycleGraph\_exp\_neg\_mass} writes the same limit as
the pure exponential $e^{-m n}$ at the inverse correlation length
$m = \texttt{correlationMass}\,\beta J = -\log\tanh\beta J$ (Glimm--Jaffe~\S17.1).

\paragraph{Exact open 1D chain two-point function (\S17.1).}
Toward the infinite-volume 1D two-point function, the first step is the exact
finite-volume two-point function of the \emph{open} chain $\texttt{pathGraph}\,(n{+}1)$.
Mathlib provides only the adjacency $\texttt{pathGraph\_adj}$, so we enumerate the
$n$ edges as the consecutive pairs $s(i.\mathrm{castSucc}, i.\mathrm{succ})$
($i:\mathrm{Fin}\,n$): $\texttt{pathGraph\_edgeFinset\_eq\_image}$,
$\texttt{card\_pathGraph\_edgeFinset} = n$.  Applying the FV~(3.46) high-temperature
closed form $\texttt{correlation\_high\_temp\_expansion\_h\_zero\_closed}$, the open
path is a \emph{tree}, so the two filtered subgraph sums collapse: the even-degree
denominator is the single empty subgraph ($\texttt{even\_subgraph\_eq\_empty}$, sum
$1$) and the $\{0,n\}$-odd-boundary numerator is the single full edge set
($\texttt{endpoint\_subgraph\_eq\_edgeFinset}$, sum $\tanh^n$).  Both are proved by an
index-set degree formula ($\texttt{deg\_eq\_of\_subset}$: the degree of vertex $k$
splits into the left ($i = k$) and right ($i = k-1$) index contributions) and a
minimal-index counterexample argument.  Hence
\[
  \texttt{correlation}\,(\texttt{pathGraph}\,(n{+}1))\,\langle J,0,\beta\rangle\,
    \{0,\,\mathrm{last}\} \;=\; (\tanh\beta J)^n
  \qquad (0 < n)
\]
($\texttt{correlation\_pathGraph\_eq\_tanh\_pow}$; $0 < n$ keeps the endpoints
$0$, $\mathrm{last}$ distinct).  Unlike the cyclic chain there is
\emph{no boundary correction}, making this the clean finite-volume input to the
infinite-volume limit $\texttt{twoPointFunction}\,1 = (\tanh\beta J)^{\mathrm{dist}}$
(Glimm--Jaffe~\S17.1).

\paragraph{General-pair open 1D chain two-point function (\S17.1).}
Generalising the endpoint result to an arbitrary distinct pair $\{a,b\}$ of
$\texttt{pathGraph}\,(m{+}1)$ vertices is needed for the infinite-volume lift, since
under the box$\cong$path relabelling the lattice endpoints $\{0,r\}$ map to interior
path vertices.  Define the segment index set
$\texttt{segmentIdx}\,m\,a\,b = \{\, i : \mathrm{Fin}\,m \mid a \le i < b \,\}$ (the
edges connecting $a$ to $b$), with $\texttt{card\_segmentIdx} = b - a$.  The key
combinatorial fact $\texttt{idx\_eq\_segmentIdx}$ is that an edge subset $X$ with odd
degree exactly at $\{a,b\}$ ($a < b$, even elsewhere) has index set the segment; it is
proved by the discrete-derivative parity recurrence
$\deg(\text{vertex }k) = [\,\text{edge }(k{-}1)\in S\,] + [\,\text{edge }k\in S\,]$ via
strong induction on $k$ (the boundary $\{a,b\}$ flips force exactly the segment).
With the segment-parity check $\texttt{segment\_parity}$ (degree $1$ at the endpoints,
$2$ inside, $0$ outside), the FV~(3.46) closed form collapses to the single segment
subgraph of cardinality $b-a$ in the numerator and $\{\emptyset\}$ in the denominator,
giving
\[
  \texttt{correlation}\,(\texttt{pathGraph}\,(m{+}1))\,\langle J,0,\beta\rangle\,
    \{a,b\} \;=\; (\tanh\beta J)^{\lvert a - b\rvert} \qquad (a \ne b)
\]
($\texttt{correlation\_pathGraph\_pair\_eq\_tanh\_pow}$) -- the exact geometric decay in
the path distance between the two sites (Glimm--Jaffe~\S17.1).

\paragraph{Infinite-volume 1D two-point function (\S17.1, capstone).}
The capstone of the infinite-volume 1D programme identifies the project's
infinite-volume Gibbs two-point function with the exact geometric decay,
\[
  \texttt{twoPointFunction}\,1\,\langle J,0,\beta\rangle\,r
    \;=\; (\tanh\beta J)^{\mathrm{latticeDistance}\,1\,0\,r}
  \qquad (r \ne 0,\ 0 \le J,\ 0 < \beta),
\]
the project's most-important long-term goal realised in one dimension.  The induced
subgraph of $\texttt{latticeGraph}\,1$ on the centred box $[-N,N]$ is an open chain:
the relabelling $k \mapsto k-N$ gives an equivalence
$\texttt{boxEquiv} : \mathrm{Fin}\,(2N{+}1) \simeq \mathord{\uparrow}(\texttt{cubicBox}\,1\,N)$
with the graph-map equality
$(\texttt{pathGraph}\,(2N{+}1))\mathbin{.}\mathrm{map}\,\texttt{boxEquiv}
   = \texttt{inducedGraph}\,(\texttt{latticeGraph}\,1)\,(\texttt{cubicBox}\,1\,N)$
($\texttt{pathGraph\_map\_boxEquiv}$; adjacency on both sides reduces to
$\lvert u_0 - v_0\rvert = 1$).  By $\texttt{correlation\_map\_equiv}$ the finite-volume
$\texttt{correlationAlongExhaustion}$ at stage $N \ge \lvert r_0\rvert$ equals the
open-chain pair two-point $(\tanh\beta J)^{\lvert r\rvert}$ (the lattice pair $\{0,r\}$
lifting to the $\texttt{boxEquiv}$-image of the path pair $\{N, r_0{+}N\}$).  The
sequence is eventually constant in $N$, so the thermodynamic-limit supremum collapses
to it ($\texttt{twoPointFunction\_one\_eq\_tanh\_pow}$, via
$\texttt{correlationAlongExhaustion\_tendsto\_ciSup}$ and $\texttt{tendsto\_nhds\_unique}$).
This is the first genuinely infinite-volume Gibbs correlation result in the project
(Glimm--Jaffe~\S17.1).

\paragraph{Infinite-volume 1D susceptibility and mass form (\S17.1).}
Two infinite-volume observables follow from the two-point capstone
$\texttt{twoPointFunction}\,1\,r = (\tanh\beta J)^{\lvert r\rvert}$ (for $r \ne 0$).  The
\emph{susceptibility} is the bilateral lattice sum of the two-point function: using
the equivalence $(\mathrm{Fin}\,1 \to \mathbb Z) \simeq \mathbb Z$
($\texttt{Equiv.funUnique}$) and the split $\mathbb Z = \mathbb N \sqcup \mathbb N$
($\texttt{HasSum.int\_rec}$: the non-negative branch contributes
$\sum_k (\tanh\beta J)^k$ and the negative branch
$\sum_k (\tanh\beta J)^{k+1}$),
\[
  \sum_{r : \mathrm{Fin}\,1 \to \mathbb Z}
    (\tanh\beta J)^{\mathrm{latticeDistance}\,1\,0\,r}
  \;=\; \texttt{isingSusceptibility1D}\,(\beta J)
  \;=\; \frac{1+\tanh\beta J}{1-\tanh\beta J}
\]
($\texttt{tsum\_tanh\_pow\_latticeDistance\_eq\_susceptibility}$), the $r = 0$
diagonal contributing $(\tanh\beta J)^0 = 1 = \langle\sigma_0^2\rangle$.  The
\emph{mass form} writes the two-point function as a pure exponential at the inverse
correlation length, $\texttt{twoPointFunction}\,1\,\langle J,0,\beta\rangle\,r
   = e^{-m\lvert r\rvert}$ with $m = \texttt{correlationMass}\,(\beta J)
   = -\log\tanh\beta J$
($\texttt{twoPointFunction\_one\_eq\_exp\_neg\_mass}$, for $r \ne 0$)
(Glimm--Jaffe~\S17.1).

\paragraph{Sharp 1D lattice mass and cluster property (Glimm--Jaffe~\S17.1,~\S17.5).}
The exact two-point identity pins down the abstract \S17.5 \emph{lattice mass}
$\mathrm{latticeMass}\,1\,(\mathrm{cubicExhaustion}\,1)\,\langle J,0,\beta\rangle$
--- the $\mathrm{ENNReal}$ supremum of admissible exponential-decay rates of the
truncated two-point function --- exactly.  At zero external field the spontaneous
magnetization vanishes
($\texttt{magnetizationInfinite\_latticeGraph\_zero\_at\_h\_zero}$), so the
truncated (Ursell) two-point function coincides with the full two-point function
($\texttt{truncated2Infinite\_one\_zero\_eq\_twoPointFunction}$); by translation
invariance the truncated function at any distinct pair $i \ne j$ equals the exact
geometric value
\[
  \texttt{truncated2Infinite}\,(\mathrm{latticeGraph}\,1)\,
    (\mathrm{cubicExhaustion}\,1)\,\langle J,0,\beta\rangle\,i\,j
  \;=\; (\tanh\beta J)^{\mathrm{dist}(i,j)}
  \;=\; e^{-m\,\mathrm{dist}(i,j)}
\]
($\texttt{truncated2Infinite\_one\_eq\_tanh\_pow}$,
$\texttt{truncated2Infinite\_one\_eq\_exp\_neg\_mass}$; helper
$\texttt{latticeDistance\_zero\_sub}$, $\mathrm{dist}(i,j) =
\mathrm{latticeDistance}\,1\,0\,(j-i)$).  With constant $C = 1$ this is
exponential decay at the mass rate $m$
($\texttt{HasExponentialDecay\_one\_correlationMass}$), so $\mathrm{ofReal}\,m \le
\mathrm{latticeMass}$
($\texttt{latticeMass\_one\_ge\_correlationMass}$).  Conversely no rate $\alpha >
m$ can work: the \emph{exact} value $e^{-m n}$ at distance $n$ forces
$e^{(\alpha-m)n} \le C$ for all $n$, impossible since $e^{(\alpha-m)n}\to\infty$
($\texttt{le\_correlationMass\_of\_HasExponentialDecay}$), giving
$\mathrm{latticeMass} \le \mathrm{ofReal}\,m$
($\texttt{latticeMass\_one\_le\_correlationMass}$).  Hence the \emph{sharp}
equality
\[
  \mathrm{latticeMass}\,1\,(\mathrm{cubicExhaustion}\,1)\,\langle J,0,\beta\rangle
  \;=\; \mathrm{ENNReal.ofReal}\,(\texttt{correlationMass}\,(\beta J))
  \qquad (J \ge 0,\ \beta > 0,\ \beta J > 0)
\]
($\texttt{latticeMass\_one\_eq\_correlationMass}$; correlation-length reading
$\texttt{latticeMass\_one\_eq\_ofReal\_inv\_correlationLength}$).  Positivity of
$m$ yields the infinite-volume cluster property: the truncated two-point function
tends to $0$ along the cofinite filter
($\texttt{clusterProperty\_one}$), and so does the full two-point function
($\texttt{twoPointFunction\_one\_tendsto\_cofinite\_zero}$).  This closes the 1D
\S17.5 mass picture end-to-end (Issue~\#3511).
...
The concrete \(\mathbb Z^d\) non-real wrappers now live in
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/VitaliBridge/Bridge/OpenSet.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/VitaliBridge/Bridge/LeeYangDomain.lean
\end{Verbatim}
and the real-axis wrappers live in
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/VitaliBridge/Real/Limit.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/VitaliBridge/Real/Identified.lean
\end{Verbatim}
with the old Vitali bridge and branches import paths kept as re-exports.
\end{theorem}
...
The all-stages form
\begin{Verbatim}
freeEnergyComplexAlongExhaustion_analyticOnNhd_branch_ball_all_stages
\end{Verbatim}
packages this for every nonempty stage.  The concrete $\mathbb Z^d$
specializations are recorded in
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/Stage/Ball/Basic.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/AllStages/Ball.lean
\end{Verbatim}
with the old ball, all-stage, stage-Lee--Yang, and branches import paths
kept as re-exports.
...
The concrete $\mathbb Z^d$ stage wrapper now lives in
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/Stage/Ball/Strong.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/AllStages/Strong.lean
\end{Verbatim}
with the old ball, all-stage, stage-Lee--Yang, and branches import paths
kept as re-exports.
...
Branches/LocalCoverPatch/Vitali/Ball/Real.lean
\end{Verbatim}
and are re-exported by the old
\begin{Verbatim}[fontsize=\footnotesize]
Branches/LocalCoverPatch/Vitali/Ball.lean
Branches/LocalCoverPatch/Vitali.lean
Branches/LocalCoverPatch.lean
\end{Verbatim}
paths.
...
\begin{Verbatim}[breaklines=true,breakanywhere=true]
freeEnergyComplexAlongExhaustion_branchFamily_vitali_localCover_real
\end{Verbatim}
...
wrapper lives in
\begin{Verbatim}[breaklines=true,breakanywhere=true]
Branches/LocalCoverPatch/Vitali/LocalCover.lean
\end{Verbatim}
It is re-exported by the old
\begin{Verbatim}[breaklines=true,breakanywhere=true]
Branches/LocalCoverPatch/Vitali.lean
Branches/LocalCoverPatch.lean
\end{Verbatim}
paths.  The real wrapper lives in
\begin{Verbatim}[breaklines=true,breakanywhere=true]
Branches/RealCompact/Vitali.lean
\end{Verbatim}
and is re-exported by the old
\begin{Verbatim}[breaklines=true,breakanywhere=true]
Branches/RealCompact.lean
\end{Verbatim}
path.
...
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/CompactOpenVitali/Ball/Bridge.lean
SubseqCompactOpen/CompactOpenVitali/Ball/Real.lean
\end{Verbatim}
under \texttt{Concrete/LatticeGraphCorrelation/PerStageComplex}, and are
re-exported by the old paths
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/CompactOpenVitali/Ball.lean
SubseqCompactOpen/CompactOpenVitali.lean
\end{Verbatim}
...
The concrete \(\mathbb Z^d\) wrapper now lives in
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/CompactOpenVitali/FiniteDiagonal/TwoBall.lean
\end{Verbatim}
under
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex
...
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/CompactOpenVitali/FiniteDiagonal/FinBall.lean
\end{Verbatim}
...
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteSubseqCompactOpen/CompactOpen/FiniteFamily.lean
\end{Verbatim}
The patch wrapper now lives in
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteSubseqCompactOpen/Patch/Family.lean
\end{Verbatim}
...
SubseqCompactOpen/FiniteSubseqCompactOpen.lean
\end{Verbatim}
...
It now lives in
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteSubseqCompactOpen/Patch/Real.lean
\end{Verbatim}
under \texttt{Concrete/LatticeGraphCorrelation/PerStageComplex}, and is
re-exported by the old paths
\begin{Verbatim}[fontsize=\footnotesize]
...
It now lives in
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteCoverPatches/CompactOpen/FiniteFamily.lean
\end{Verbatim}
under the concrete per-stage complex directory, and is re-exported by the old
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteCoverPatches/CompactOpen.lean
SubseqCompactOpen/FiniteCoverPatches.lean
\end{Verbatim}
path.
...
\(\texttt{freeEnergyInfinite}\).  The concrete \(\mathbb Z^d\) direct patch
wrappers and compact-open package/patch wrappers are implemented in
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/
  SubseqCompactOpen/FiniteCoverPatches/Patch/FiniteCover.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  BranchAscoliCompactOpen/RealCoverPatches/FiniteCover.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  BranchAscoliCompactOpen/RealCoverPatches/Patch.lean
\end{Verbatim}
and are re-exported through the legacy finite-cover patch and real-cover
paths.  The compact-open
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/
  SubseqCompactOpen/FiniteCoverPatches/Patch/RealCover.lean
\end{Verbatim}
and is re-exported by the old
\begin{Verbatim}
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/
  SubseqCompactOpen/FiniteCoverPatches/Patch/CompactTarget.lean
\end{Verbatim}
and is re-exported by the old
\begin{Verbatim}
SubseqCompactOpen/FiniteCoverPatches/Patch.lean
...
\texttt{OpenPatch/OpenCover.lean} child, re-exported by the legacy
\texttt{OpenPatch.lean} and \texttt{LocalCoverPatch.lean} paths.
...
in
\begin{Verbatim}
Branches/LocalCoverPatch/OpenPatch/LocalCover.lean
\end{Verbatim}
and is re-exported by the legacy paths
\begin{Verbatim}
Branches/LocalCoverPatch/OpenPatch.lean
Branches/LocalCoverPatch.lean
\end{Verbatim}
.
...
in the patch child
\begin{Verbatim}
StructuredPatch/Patch/Data.lean
\end{Verbatim}
and re-exported by the legacy chain
\begin{Verbatim}
StructuredPatch/Patch.lean
...
\end{Verbatim}
in the real-axis child
\begin{Verbatim}
StructuredPatch/Real/Data.lean
\end{Verbatim}
and re-exported by the legacy chain
\begin{Verbatim}
StructuredPatch/Real.lean
StructuredPatch.lean
LocalCoverPatch.lean
\end{Verbatim}
...
\begin{Verbatim}
StructuredPatch/Patch/Family.lean
StructuredPatch/Real/Family.lean
\end{Verbatim}
and re-exported by the legacy chain
\begin{Verbatim}
StructuredPatch/Patch.lean
...
in the real-axis child
\begin{Verbatim}
StructuredPatch/Real/RealFamily.lean
\end{Verbatim}
and re-exported by the legacy chain
\begin{Verbatim}
StructuredPatch/Real.lean
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/BranchData.lean
\end{Verbatim}
with related stage and all-stage branch wrappers split into
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/Stage/Point/Exists.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/Stage/Point/Family.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/Stage/Ball/Basic.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/Stage/Ball/Strong.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/AllStages/Pointwise.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/AllStages/Ball.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  Branches/StageLeeYang/AllStages/Strong.lean
\end{Verbatim}
and re-exported through the legacy
\begin{Verbatim}[fontsize=\footnotesize]
Branches/StageLeeYang/Stage/Point.lean
Branches/StageLeeYang/Stage/Ball.lean
Branches/StageLeeYang/AllStages.lean
Branches/StageLeeYang.lean
PerStageComplex/Branches.lean
\end{Verbatim}
paths.
...
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteSubseqCompactOpen/CompactOpen/Pointwise.lean
\end{Verbatim}
and is re-exported by the old paths
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteSubseqCompactOpen/CompactOpen.lean
SubseqCompactOpen/FiniteSubseqCompactOpen.lean
\end{Verbatim}
The patch wrapper lives in
\begin{Verbatim}[fontsize=\footnotesize]
SubseqCompactOpen/FiniteCoverPatches/CompactOpen/Pointwise.lean
\end{Verbatim}
under
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  SubseqCompactOpen/PointwiseAllStage/FinCover/Package.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  SubseqCompactOpen/PointwiseAllStage/FinCover/Patch.lean
\end{Verbatim}
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  SubseqCompactOpen/PointwiseAllStage/COpenData/Patch.lean
\end{Verbatim}
and
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  SubseqCompactOpen/PointwiseAllStage/COpenData/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy
\begin{Verbatim}[fontsize=\footnotesize]
...
\end{Verbatim}
in the split child modules
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/RangeCompactOpen/RangeClosure/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/RangeCompactOpen/RangeClosure/CompactTarget.lean
\end{Verbatim}
and re-exported by the legacy paths
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/RangeCompactOpen/RangeClosure.lean
RangeAscoliPatches/RangeCompactOpen.lean
\end{Verbatim}
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/RangeCompactOpen/RelCompact/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/RangeCompactOpen/RelCompact/CompactTarget.lean
\end{Verbatim}
and re-exported by the legacy paths
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/RangeCompactOpen/RelCompact.lean
RangeAscoliPatches/RangeCompactOpen.lean
\end{Verbatim}
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/RangeNormBounded/RelCompact/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/RangeNormBounded/RelCompact/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/AllStageAscoli/RangeNormBounded/RelCompact.lean
RangeAscoliPatches/AllStageAscoli/RangeNormBounded.lean
\end{Verbatim}
and
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/AllStageAscoli.lean
\end{Verbatim}
paths.
...
\end{Verbatim}
and the concrete \(\mathbb Z^d\) norm-bounded wrappers now live in
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchRelCompact/NormBounded/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchRelCompact/NormBounded/CompactTarget.lean
\end{Verbatim}
and the constant norm-bounded wrappers now live in
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchRelCompact/ConstNormBounded/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchRelCompact/ConstNormBounded/CompactTarget.lean
\end{Verbatim}
and the locally bounded wrappers now live in
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchRelCompact/LocallyBounded/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchRelCompact/LocallyBounded/CompactTarget.lean
\end{Verbatim}
The wrapper layers are re-exported by
\begin{Verbatim}[fontsize=\footnotesize]
BranchRelCompact/NormBounded.lean
BranchRelCompact/ConstNormBounded.lean
BranchRelCompact/LocallyBounded.lean
BranchRelCompact.lean
\end{Verbatim}
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    LocallyBounded/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    LocallyBounded/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy modules
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/BranchLocalDirectRange/LocallyBounded.lean
RangeAscoliPatches/BranchLocalDirectRange.lean
\end{Verbatim}
This is a
...
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    EventualOverlap/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    EventualOverlap/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy modules
\begin{Verbatim}[fontsize=\footnotesize]
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchDeviationDirectRange/RelCompact/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchDeviationDirectRange/RelCompact/CompactTarget.lean
\end{Verbatim}
and re-exported by the legacy paths
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/BranchDeviationDirectRange/RelCompact.lean
RangeAscoliPatches/BranchDeviationDirectRange.lean
\end{Verbatim}
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchDeviationDirectRange/DirectRange/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchDeviationDirectRange/DirectRange/CompactTarget.lean
\end{Verbatim}
and re-exported by the legacy paths
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/BranchDeviationDirectRange/DirectRange.lean
RangeAscoliPatches/BranchDeviationDirectRange.lean
\end{Verbatim}
This is a composition bridge;
...
in
\begin{Verbatim}[fontsize=\scriptsize]
RangeAscoliPatches/BranchDeviationDirectRange/ViaLocal/Core.lean
RangeAscoliPatches/BranchDeviationDirectRange/ViaLocal/CompactTarget.lean
\end{Verbatim}
and are re-exported by the legacy
\texttt{BranchDeviationDirectRange/ViaLocal.lean} module and the umbrella
module.
...
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    ViaDeviation/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    ViaDeviation/CompactTarget.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    EventualOverlapViaDeviation/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/BranchLocalDirectRange/
    EventualOverlapViaDeviation/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy modules
\begin{Verbatim}[fontsize=\footnotesize]
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/Ascoli/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/Ascoli/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy
\texttt{RangeAscoliPatches/AllStageAscoli/Ascoli.lean} and
\texttt{RangeAscoliPatches/AllStageAscoli.lean} paths.
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/ClosedProduct/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/ClosedProduct/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy
\texttt{RangeAscoliPatches/AllStageAscoli/ClosedProduct.lean} and
\texttt{RangeAscoliPatches/AllStageAscoli.lean} paths.
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/NormBounded/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/NormBounded/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy
\texttt{RangeAscoliPatches/AllStageAscoli/NormBounded.lean} and
\texttt{RangeAscoliPatches/AllStageAscoli.lean} paths.
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/RangeNormBounded/Ascoli/Core.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/
  RangeAscoliPatches/AllStageAscoli/RangeNormBounded/Ascoli/CompactTarget.lean
\end{Verbatim}
and are re-exported through the legacy
\begin{Verbatim}[fontsize=\footnotesize]
RangeAscoliPatches/AllStageAscoli/RangeNormBounded/Ascoli.lean
RangeAscoliPatches/AllStageAscoli/RangeNormBounded.lean
\end{Verbatim}
and
\begin{Verbatim}[fontsize=\footnotesize]
...
returns a real-centred branch-limit family and a differentiable global patch
on the Lee-Yang domain.  The patch agrees directly with each raw local limit
\(f_{h_0}\) on \(B(h_0,r(h_0))\), agrees with the packaged local limits, and
has value \texttt{freeEnergyInfinite} at \(p.h\).
...
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  RawBranchData/LocalFamily.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  RawBranchData/RealFamily.lean
\end{Verbatim}
and are re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  RawBranchData.lean
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  EventuallyEqOn/LocalFamily.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  EventuallyEqOn/RealFamily.lean
\end{Verbatim}
and are re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  StructuredFamily/Structured.lean
\end{Verbatim}
and are re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  StructuredPatch/Structured.lean
\end{Verbatim}
and is re-exported by the old
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  StructuredFamily/Pointwise.lean
\end{Verbatim}
and is re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  StructuredFamily.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap.lean
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch/Patch/Structured.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch/Patch/Pointwise.lean
\end{Verbatim}
and are re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch/Patch.lean
\end{Verbatim}
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch.lean
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch/FamilyPatch/Structured.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch/FamilyPatch/Pointwise.lean
\end{Verbatim}
and are re-exported by the old
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch/FamilyPatch.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/EventualOverlap/
  LocalCoverPatch.lean
\end{Verbatim}
and
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry/EventualDataGeometry/Structured.lean
\end{Verbatim}
and is re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry/EventualDataGeometry.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry/EventualDataGeometry/Pointwise.lean
\end{Verbatim}
and is re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  StructuredPatch/Pointwise.lean
\end{Verbatim}
and is re-exported by the old
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry/FinsetCover/LocalFamily.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry/FinsetCover/RealFamily.lean
\end{Verbatim}
and are re-exported by the legacy chain
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry/FinsetCover.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact.lean
\end{Verbatim}
...
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact/
  CompactGeometry/FinGeometry.lean
\end{Verbatim}
and is re-exported by the old
\begin{Verbatim}
Concrete/LatticeGraphCorrelation/PerStageComplex/Branches/RealCompact.lean
...
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  PolynomialLower/PolyLocalBounded.lean
\end{Verbatim}
and is re-exported through the legacy
\begin{Verbatim}[fontsize=\footnotesize]
PerStageComplex/Bounds/PolynomialLower.lean
PerStageComplex/Bounds.lean
\end{Verbatim}
paths.
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  PolynomialLower/PolyBallAround.lean
\end{Verbatim}
and is re-exported through the legacy
\begin{Verbatim}[fontsize=\footnotesize]
PerStageComplex/Bounds/PolynomialLower.lean
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  LeeYangLower/LowerLog.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  LeeYangLower/LocalBounded.lean
\end{Verbatim}
and are re-exported through the legacy paths
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  LeeYangLower.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds.lean
\end{Verbatim}
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  PolynomialLower/LeeYangUpperLower.lean
\end{Verbatim}
and is re-exported through the legacy
\begin{Verbatim}[fontsize=\footnotesize]
PerStageComplex/Bounds/PolynomialLower.lean
...
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  UpperLog/AbsLogFreeEnergy.lean
\end{Verbatim}
and are re-exported through the legacy paths
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
...
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  UpperLog/AbsLogFreeEnergy.lean
\end{Verbatim}
and is re-exported through the legacy paths
\begin{Verbatim}[fontsize=\footnotesize]
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds/
  UpperLog.lean
Concrete/LatticeGraphCorrelation/PerStageComplex/Bounds.lean
...
\begin{theorem}[\texttt{correlation\_high\_temp\_expansion\_general\_h\_subset\_form}; GJ~\S18.3/\S18.5]
For any $A \subseteq \iota$, any finite simple graph $G = (\iota, E)$, and
any Ising parameter $p = (J, h, \beta)$,
\[
\langle \sigma_A \rangle_p =
  \frac{\sum_{X \subseteq E} \tanh(\beta J)^{|X|}
       ...
       {\sum_{X \subseteq E} \tanh(\beta J)^{|X|}
        \sum_\sigma \bigl(\prod_{e \in X} \sigma_e\bigr)
          e^{\beta h \sum_i \sigma_i}}.
\]
\end{theorem}

\textit{Proof.} The numerator
$N := \sum_\sigma \sigma_A\, e^{-\beta H(\sigma)}$ expands by the same edge
decomposition $e^{\beta J \sigma_e} = \cosh(\beta J)\,(1 + \tanh(\beta J)\sigma_e)$,
subset expansion $\prod_{e \in E}(1 + x_e) = \sum_{X \subseteq E} \prod_{e \in X} x_e$,
and $\sigma \leftrightarrow X$ Fubini swap used for $Z$, but with the spin
insertion $\sigma_A$ carried through the $\sigma$-sum (the
$F = \sigma_A$ case of
\texttt{numerator\_observable\_high\_temp\_expansion\_subset\_form}):
\[
N = (\cosh \beta J)^{|E|}
   \sum_{X \subseteq E} \tanh(\beta J)^{|X|}
   \sum_\sigma \sigma_A \Bigl(\prod_{e \in X} \sigma_e\Bigr) e^{\beta h \sum_i \sigma_i}.
\]
At general $h$ the field factor $e^{\beta h \sum_i \sigma_i}$ blocks the
$\{\pm 1\}$ parity cancellation that, at $h = 0$, reduces the inner
$\sigma$-sum to the even-subgraph boundary indicator $\partial X = A$.
The matching denominator is
\texttt{partitionFunction\_high\_temp\_expansion\_subset\_form}; dividing
cancels the common prefactor $(\cosh \beta J)^{|E|}$ via
\texttt{inv\_mul\_cancel}$_0$, using only $\cosh(\beta J) \neq 0$ (no
nonvanishing hypothesis on the denominator). At $h = 0$ this specialises to
FV~(3.46) above.
\hfill$\square$

\paragraph{Wrappers.} Lifted to the \(\Lambda\)-layer
(\texttt{correlation\LeanLambda\_high\_temp\_expansion\_general\_h\_subset\_form}
in \texttt{AmbientLattice/Defs/HighTempPartition/ClosedForms.lean}) and the
along-exhaustion layer
(\texttt{correlationAlongExhaustion\_high\_temp\_expansion\_general\_h\_subset\_form}
in \texttt{AmbientLattice/SpecialCases/HighTemperatureBoundsExpansionClosedCorrelation.lean})
through \(\langle\sigma_A\rangle^\Lambda = \langle\sigma_A\rangle\) on the
induced subgraph.

\paragraph{Arbitrary-observable master form.} The same expansion holds for an
arbitrary observable \(F : \mathrm{Config}~\iota \to \mathbb{R}\)
(\texttt{gibbsExpectation\_high\_temp\_expansion\_general\_h\_subset\_form}):
\[
\langle F \rangle_p =
  \frac{\sum_{X \subseteq E} \tanh(\beta J)^{|X|}
...
\]
This is the canonical GJ~\S18.3 high-temperature representation of thermal
averages; the proof is identical, carrying \(F(\sigma)\) through the
\(\sigma\)-sum in the public numerator lemma
\texttt{numerator\_observable\_high\_temp\_expansion\_subset\_form} (whose
\(F = 1\) case is the partition-function subset form). The correlation theorem
above is the one-line specialisation \(F = \sigma_A\), and the magnetization
\(\langle\sigma_i\rangle\) is \(F = \sigma_{\{i\}}\). The along-exhaustion lift
\texttt{gibbsExpectationAlongExhaustion\_high\_temp\_expansion\_general\_h\_subset\_form}
(\texttt{AmbientLattice/GibbsExpectationHighTempExpansion.lean}) carries it to
the infinite-volume exhaustion.

\paragraph{External-field response in subset form (GJ~\S17.6, pp.~348--351;
\S18.3, pp.~378--386).}
The GJ~\S17.6 fluctuation-response derivative
\[
\frac{\partial}{\partial h}\langle\sigma_A\rangle_h
  ...
  \qquad M(\sigma) = \textstyle\sum_i \sigma_i,
\]
combined with the master subset form, expresses the response coefficient
entirely as ratios of subset sums over the common denominator
\(D = \sum_{X\subseteq E}\tanh(\beta J)^{|X|}\sum_\sigma(\prod_{e\in X}\sigma_e)e^{\beta h\sum_i\sigma_i}\):
each thermal average \(\langle\sigma_A M\rangle\), \(\langle\sigma_A\rangle\),
\(\langle M\rangle\) is rewritten through
\texttt{gibbsExpectation\_high\_temp\_expansion\_general\_h\_subset\_form}.
This is \texttt{hasDerivAt\_correlation\_field\_high\_temp\_subset\_form}
(\texttt{PhaseTransition/FieldResponseHighTemp.lean}); the magnetization
specialisation \(A=\{i\}\),
\texttt{hasDerivAt\_magnetization\_field\_high\_temp\_subset\_form}, gives the
local field susceptibility \(\partial\langle\sigma_i\rangle/\partial h\), and
connects the \S5.3/\S17.6 susceptibility machinery to the \S18.3 expansion.

\paragraph{Connected correlations in subset form (GJ~\S18.5, pp.~313--316;
\S18.7, pp.~319--322).}
Writing
\(N[F] = \sum_{X\subseteq E}\tanh(\beta J)^{|X|}\sum_\sigma F(\sigma)(\prod_{e\in X}\sigma_e)e^{\beta h\sum_i\sigma_i}\)
(\texttt{highTempSubsetNumerator}) and \(D = N[1]\)
(\texttt{highTempSubsetDenominator}, \(= Z/\cosh(\beta J)^{|E|}\)), the master
form reads \(\langle F\rangle_p = N[F]/D\). The connected (truncated)
correlations — the objects whose decay the cluster expansion controls in
GJ~\S18.7 — then take the general-\(h\) subset form
\[
\langle\sigma_i;\sigma_j\rangle_p
  = \frac{N[\sigma_{\{i,j\}}]}{D} - \frac{N[\sigma_{\{i\}}]}{D}\frac{N[\sigma_{\{j\}}]}{D}
\]
(\texttt{truncated2\_high\_temp\_expansion\_general\_h\_subset\_form}), and
likewise the three-point Ursell function
(\texttt{truncated3\_high\_temp\_expansion\_general\_h\_subset\_form}). These
are finite-volume lattice Ising identities (no continuum limit) furnishing the
general-\(h\) foundation for connected-correlation decay.

\paragraph{General-\(h\) exponential decay of the connected 2-point function
(GJ~\S18.7, pp.~319--322).} On \(\mathbb{Z}^d\), for a ferromagnetic field
\(\langle J,h,\beta\rangle\) with \(h\ge 0\) and distinct sites \(i\neq j\), the
\emph{connected} two-point function \(\langle\sigma_i;\sigma_j\rangle =
\texttt{truncated2Infinite}\) decays (unlike the full two-point function, which
tends to \(\langle\sigma_i\rangle\langle\sigma_j\rangle\neq 0\)):
\[
\langle\sigma_i;\sigma_j\rangle_{\langle J,h,\beta\rangle}
  \le (\beta J\cdot 2d)^n,\qquad n+1\le d_{\mathbb{Z}^d}(i,j),
\]
with high-temperature exponential forms
\(\le e^{-(-\log(\beta J\cdot 2d))\,n}\) and the half-rate full-distance form
\(\le e^{-\tfrac12(-\log(\beta J\cdot 2d))\,d_{\mathbb{Z}^d}(i,j)}\) for
\(d_{\mathbb{Z}^d}(i,j)\ge 2\). The proof composes three existing facts: GHS
field-monotonicity (\texttt{truncated2Infinite\_antitoneOn\_h\_of\_ne}: the
connected 2-point is antitone in \(h\) on \([0,\infty)\), so
\(\le\) its \(h=0\) value), the \(h=0\) collapse
\(\texttt{truncated2Infinite\_h\_zero}\) (\(=\langle\sigma_i\sigma_j\rangle_{h=0}\)
since \(\langle\sigma_i\rangle=0\) by \(\mathbb{Z}_2\) symmetry), and the
existing \(h=0\) Simon--Lieb decay of the full two-point function
(\texttt{SimonLiebDistanceDecay.lean}). The three general-\(h\) decay theorems
live in \texttt{Truncated2GeneralFieldDecay.lean}. Lattice Ising, no continuum
limit.

\paragraph{General-\(h\) cluster property (GJ~\S18.7).} For \(0<J\),
\(0\le h\), \(0<\beta\) and \(\beta J\cdot 2d<1\) on \(\mathbb{Z}^d\), the
connected two-point function clusters: \(j\mapsto\langle\sigma_i;\sigma_j\rangle
\to 0\) along the cofinite filter (as \(d_{\mathbb{Z}^d}(i,j)\to\infty\)). The
existing \(h=0\) exponential-decay witness
(\texttt{hasExponentialDecay\_latticeGraph\_of\_betaJ\_two\_d\_lt\_one}) is
lifted to general \(h\) by GHS field-monotonicity — the same constant and rate
\(-\log(\beta J\cdot 2d)\) work since
\(0\le\langle\sigma_i;\sigma_j\rangle_h\le\langle\sigma_i;\sigma_j\rangle_0\)
for \(h\ge 0\) and \(i\neq j\) — and fed into the existing squeeze-based
\texttt{clusterProperty\_latticeGraph\_of\_HasExponentialDecay}
(\texttt{Truncated2GeneralFieldCluster.lean}). Lattice Ising, no continuum
limit.
...
is being established by the \emph{root-component recurrence}, which avoids the
chromatic-polynomial / matrix-tree machinery absent from Mathlib. Classifying
every spanning edge-subset by the connected component of vertex $0$ gives
$D_n = \sum_{C \ni 0} c_{|C|}\, D_{n-|C|}$, where
$c_m = \mathrm{alternatingConnectedSubgraphSum}(K_m)$ and
$D_m = \mathrm{allSignedSubgraphSum}(K_m) := \sum_{S \subseteq E(K_m)} (-1)^{|S|}$
is the signed sum over \emph{all} (not necessarily connected) spanning
edge-subsets. Since $D_m = 0$ for $m \ge 2$ and $D_0 = D_1 = 1$, the recurrence
collapses to $c_n + (n-1)\,c_{n-1} = 0$, whence
$c_n = (-1)^{n-1}(n-1)!$. The $D_n$ foundation
(\texttt{allSignedSubgraphSum\_eq\_zero\_of\_edgeFinset\_nonempty} via
\texttt{Finset.sum\_powerset\_neg\_one\_pow\_card\_of\_nonempty},
\texttt{\_eq\_one\_of\_edgeFinset\_empty}, and the complete-graph specialisations
$D_{n \ge 2} = 0$, $D_{n \le 1} = 1$) is established in
\texttt{AlternatingCompleteGraph.lean} (Part of \#1499). The
\emph{iso-invariance} of $c$ and $D$
(\texttt{alternatingConnectedSubgraphSum\_iso},
\texttt{allSignedSubgraphSum\_iso}: for $\varphi : G \simeq_g H$ the relabelling
$S \mapsto S.\mathrm{map}(\mathrm{Sym2.map}\,\varphi)$ is a cardinality- and
connectivity-preserving bijection on edge-subsets, via
\texttt{SimpleGraph.Iso.connected\_iff}) supplies the
$c_{|C|} = c(K_{|C|})$, $D_{n-|C|} = D(K_{n-|C|})$ ingredients through
$K_n|_C \cong K_{|C|}$. The \emph{iso-invariance corollaries} \texttt{alternatingConnectedSubgraphSum\_completeGraph\_card}, \texttt{allSignedSubgraphSum\_completeGraph\_card} give that $c$, $D$ of $K_V$ depend only on $|V|$, and \texttt{connectedComponentMk\_eq\_of\_mem} establishes the crossing-edge-free property (no edge of $S$ joins distinct components of $\mathrm{fromEdgeSet}\,S$). The vertex set \texttt{rootComponentFinset}\,S\,r (the component of the root $r$) and its membership/crossing-free characterisations (\texttt{mem\_rootComponentFinset\_iff\_of\_mem\_edge}: both endpoints of an edge of $S$ lie in the root component together) set up the within-$C$/outside-$C$ split. The crux \texttt{induce\_fromEdgeSet\_inside\_connected} shows the within-$C$ edges induce a \emph{connected} graph on $C$ (since $C$ is exactly $r$'s component support, the induced graph is the component's \texttt{toSimpleGraph}, connected by \texttt{connected\_toSimpleGraph}), and \texttt{rootComponent\_edge\_card\_split} gives $|S|=|S_{\mathrm{in}}|+|S_{\mathrm{out}}|$. The root-component bijection and closed form follow. The \emph{fibre product split} (lemma 6) is now formalised: \texttt{reachable\_stays\_in\_of\_no\_cross} (with no crossing edge, reachability from the root stays inside $C$, by \texttt{ReflTransGen} induction); \texttt{rootComponentFinset\_eq\_iff} characterises the fibre $\{S : \mathrm{rootComponentFinset}\,S\,r = C\}$ as $r \in C$, no crossing edge, and $S \cap C.\mathrm{sym2}$ inducing a connected graph on $C$; \texttt{mem\_sym2\_and\_compl\_sym2\_false} records the disjointness $C.\mathrm{sym2} \cap C^c.\mathrm{sym2} = \varnothing$; and the inside/outside ambient signed-subgraph sets \texttt{insideConnectedEdgeSubsets}, \texttt{outsideEdgeSubsets} carry the two factors. The capstone \texttt{fiber\_signed\_sum\_eq\_product} proves $\sum_{S : \mathrm{rootComponentFinset}\,S\,r = C} (-1)^{|S|} = \bigl(\sum_{A} (-1)^{|A|}\bigr)\bigl(\sum_{B} (-1)^{|B|}\bigr)$ over the inside/outside sets, via the crossing-free bijection $S \mapsto (S \cap C.\mathrm{sym2}, S \cap C^c.\mathrm{sym2})$ with inverse $(A,B) \mapsto A \cup B$ (\texttt{Finset.sum\_bij'}), giving the ambient multiplicative core of $D_n = \sum_{C \ni 0} c_{|C|} D_{n-|C|}$. The fibrewise summation over $C$ and the reindex $\mathrm{inside} \cong c(K_C)$, $\mathrm{outside} \cong D(K_{C^c})$ collapsing to the closed form remain. The \emph{fibrewise recurrence} (lemma 7) is now formalised: \texttt{real\_signed\_sum\_powerset} ($\sum_{B \subseteq X}(-1)^{|B|} = [X = \varnothing]$, a real cast of \texttt{Finset.sum\_powerset\_neg\_one\_pow\_card}), \texttt{allSignedSubgraphSum\_eq\_ite} ($D(G) = [G.\mathrm{edgeFinset} = \varnothing]$), and the capstone \texttt{allSignedSubgraphSum\_eq\_sum\_fiber\_product}: $D(G) = \sum_{C : r \in C} \Sigma_{\mathrm{in}}(C)\cdot\Sigma_{\mathrm{out}}(C)$ via \texttt{Finset.sum\_fiberwise\_of\_maps\_to} on the root-component map $S \mapsto \mathrm{rootComponentFinset}\,S\,r$ combined with the per-fibre product split. The outside factor is evaluated in ambient terms by \texttt{outsideEdgeSubsets\_eq\_powerset} (the outside subsets are the powerset of $G.\mathrm{edgeFinset} \cap C^c.\mathrm{sym2}$) and \texttt{outsideEdgeSubsets\_signed\_sum\_eq\_ite} ($\Sigma_{\mathrm{out}}(C) = [G.\mathrm{edgeFinset} \cap C^c.\mathrm{sym2} = \varnothing]$). The ambient recurrence $D_n = \sum_{C \ni 0} \Sigma_{\mathrm{in}}(C)\cdot\Sigma_{\mathrm{out}}(C)$ is in place; the remaining reindex identifies $\Sigma_{\mathrm{in}}(C) = c(K_C)$, $\Sigma_{\mathrm{out}}(C) = D(K_{C^c})$ on the subtype complete graphs and collapses to $c_n + (n-1)c_{n-1} = 0$. The \emph{outside half} of the reindex (lemma 8) is now formalised by reduction to the complement cardinality: \texttt{completeGraph\_edgeFinset\_inter\_compl\_sym2\_empty\_iff} ($(\top).\mathrm{edgeFinset} \cap C^c.\mathrm{sym2} = \varnothing \iff |C^c| \le 1$, since an edge inside $C^c$ needs two distinct complement vertices), \texttt{allSignedSubgraphSum\_completeGraph\_subtype\_eq\_ite} ($D(K_C) = [\,|C| \le 1\,]$ via \texttt{allSignedSubgraphSum\_completeGraph\_card} and the $\mathrm{Fin}$ boundary lemmas), and \texttt{outsideEdgeSubsets\_completeGraph\_signed\_sum}: $\Sigma_{\mathrm{out}}(C) = \mathrm{allSignedSubgraphSum}(\top : \mathrm{SimpleGraph}\,\uparrow\!C^c) = D(K_{C^c})$, both sides collapsing to $[\,|C^c| \le 1\,]$ (avoiding subtype edge-set rewrites). The \emph{inside half} of the reindex, and with it the full root-component recurrence, are now formalised. The graph-equality crux \texttt{induce\_fromEdgeSet\_map\_subtype} ($(\mathrm{fromEdgeSet}\,\uparrow\!(T.\mathrm{map}\,e)).\mathrm{induce}\,\uparrow\!C = \mathrm{fromEdgeSet}\,\uparrow\!T$ on $\uparrow\!C$, $e$ the subtype $\mathrm{sym2}$-embedding) transfers connectivity; the roundtrip helpers \texttt{inside\_mem\_range\_sym2Map} (every inside edge is $e\,z$) and \texttt{inside\_preimage\_map\_eq} ($(A.\mathrm{preimage}\,e).\mathrm{map}\,e = A$) handle the bijection bookkeeping; \texttt{insideConnectedEdgeSubsets\_completeGraph\_signed\_sum} proves $\Sigma_{\mathrm{in}}(C) = \mathrm{alternatingConnectedSubgraphSum}(\top : \mathrm{SimpleGraph}\,\uparrow\!C) = c(K_C)$ via the connectivity-preserving bijection $T \mapsto T.\mathrm{map}\,e$ / $A \mapsto A.\mathrm{preimage}\,e$ (\texttt{Finset.sum\_bij'}, \texttt{Finset.preimage\_map}). The capstone \texttt{allSignedSubgraphSum\_completeGraph\_root\_recurrence} assembles lemma 7 with both reindexes into the full recurrence $D_n = \sum_{C \ni r} c(K_C)\cdot D(K_{C^c}) = \sum_{C \ni 0} c_{|C|} D_{n-|C|}$. The collapse and closed form are now formalised, completing the programme. \texttt{mayer\_surviving\_set} identifies the $C \ni 0$ with $|C^c| \le 1$ as exactly $\mathrm{univ}$ together with the $n-1$ cofinite singletons $\{j\}^c$ ($j \ne 0$); \texttt{alternatingConnectedSubgraphSum\_completeGraph\_recurrence} collapses the root-component recurrence to $c_n + (n-1)c_{n-1} = 0$ ($n \ge 2$), using $D(K_{C^c}) = 0$ unless $|C^c| \le 1$, the cardinality-invariance of $c$, and $D_n = 0$; and the capstone \texttt{alternatingConnectedSubgraphSum\_completeGraph\_closed\_form} proves $\mathrm{alternatingConnectedSubgraphSum}\,K_n = (-1)^{n-1}(n-1)!$ for $n \ge 1$ by induction from the recurrence (base $c_1 = 1$, step via $m! = m\,(m-1)!$ and $(-1)^m = -(-1)^{m-1}$). This is the general-$n$ Mayer/Ursell coefficient of the complete-graph cluster expansion, obtained through the root-component recurrence without any chromatic-polynomial or matrix-tree machinery. As a corollary, the closed form is connected to the actual Ursell coefficient: \texttt{ursellCoefficient\_eq\_alternatingConnectedSubgraphSum\_div} ($\phi^T(\omega) = c(G(\omega))/n!$), \texttt{polymerSeqIncompatibilityGraph\_eq\_top\_of\_pairwise} (pairwise incompatibility gives $G(\omega) = \top$), \texttt{alternatingConnectedSubgraphSum\_polymerSeq\_complete} (the complete cluster's connected-spanning sum is $(-1)^{n-1}(n-1)!$, via the identity isomorphism $G(\omega) \simeq_g \top$), and \texttt{ursellCoefficient\_complete\_eq}: the classic single-cluster Mayer value $\phi^T(\omega) = (-1)^{n-1}(n-1)!/n! = (-1)^{n-1}/n$ for $n$ pairwise incompatible polymers. The companion file \texttt{MayerCompleteContribution.lean} records $|\phi^T(\omega)| = 1/n$ (\texttt{ursellCoefficient\_complete\_abs\_eq}), the $n=2$ consistency $\phi^T = -1/2$ (\texttt{ursellCoefficient\_complete\_eq\_two}, matching the pair Ursell value), and \texttt{mayerExpansionTerm\_completeClusterSubsum\_eq}: the Mayer-term contribution of the fully-incompatible sequences factors as $\bigl((-1)^{n-1}/n\bigr)\sum_\omega z(\omega)$, the constant Ursell coefficient pulled out of the activity sum. The capstone \texttt{hasSum\_singlePolymer\_ursell\_eq\_log} / \texttt{tsum\_singlePolymer\_ursell\_eq\_log} proves the classic identity that a single polymer $P$ contributes $\log(1+t^{|P|})$ to $\log\Xi$: summing the repeated-polymer term $\phi^T(P,\dots,P)\cdot(t^{|P|})^m = \bigl((-1)^{m-1}/m\bigr)(t^{|P|})^m$ over the multiplicity gives $\sum_{m\ge 1}\phi^T(P,\dots,P)\,(t^{|P|})^m = \log(1+t^{|P|})$ for $|t^{|P|}|<1$, via the closed form (a repeated polymer is a self-incompatible complete cluster) and the logarithm power series \texttt{Real.hasSum\_pow\_div\_log\_of\_abs\_lt\_one}. This is the log structure at the heart of why the cluster expansion exponentiates. The convergence side (GJ §18.5) is recorded by \texttt{hasSum\_abs\_singlePolymer\_ursell} / \texttt{tsum\_abs\_singlePolymer\_ursell}: for $0 \le t$ and $t^{|P|} < 1$ the absolute values sum to $\sum_m (t^{|P|})^{m+1}/(m+1) = -\log(1 - t^{|P|})$ (the explicit convergence radius $t^{|P|} < 1$), together with \texttt{summable\_singlePolymer\_ursell} and the domination \texttt{singlePolymer\_log\_le\_neg\_log} ($\log(1+z) \le -\log(1-z)$ for $0 \le z < 1$). The independent-polymer upper bound (GJ §18.5) is \texttt{vdPolymerFamilies\_sum\_le\_prod\_one\_add} ($\sum_\Gamma \prod_{P\in\Gamma} t^{|P|} \le \prod_P (1+t^{|P|})$ for $t\ge 0$, the compatible families being a subset of all subsets, \texttt{Finset.prod\_one\_add}), \texttt{polymerFreeEnergy\_le\_sum\_log\_one\_add} ($\mathrm{polymerFreeEnergy}\,G\,t \le \sum_P \log(1+t^{|P|})$ by $\log$-monotonicity and \texttt{Real.log\_prod}), and \texttt{polymerFreeEnergy\_le\_sum\_singlePolymer\_contribution} (for $t^{|P|}<1$ the free energy is dominated by the sum of independent single-polymer Mayer contributions $\sum_P\sum'_m \phi^T(P,\dots,P)\,z$). The matching lower bound uses the empty and single-polymer families (\texttt{IsCompatiblePolymerFamilyVertexDisjoint.singleton}): \texttt{one\_add\_sum\_le\_vdPolymerFamilies\_sum} ($1 + \sum_P t^{|P|} \le \sum_\Gamma \prod_{P\in\Gamma} t^{|P|}$) gives \texttt{log\_one\_add\_sum\_le\_polymerFreeEnergy}, and \texttt{polymerFreeEnergy\_sandwich\_independent} bundles the two-sided estimate $\log(1+\sum_P t^{|P|}) \le \mathrm{polymerFreeEnergy}\,G\,t \le \sum_P \log(1+t^{|P|})$. Substituting into the polymer decomposition \texttt{freeEnergy\_eq\_polymerFreeEnergy} gives the Ising free energy sandwich \texttt{freeEnergy\_sandwich\_independent\_polymer}: for $0 \le \beta J$ and $|\iota|>0$, $\log 2 + \frac{|E|}{|\iota|}\log\cosh(\beta J) + \frac{1}{|\iota|}\log(1+\sum_P \tanh(\beta J)^{|P|}) \le \mathrm{freeEnergy}\,G\,\langle J,0,\beta\rangle \le \dots + \frac{1}{|\iota|}\sum_P \log(1+\tanh(\beta J)^{|P|})$, with the coarser explicit upper bound \texttt{freeEnergy\_le\_log\_two\_plus\_sum\_tanh\_pow} (via $\log(1+x)\le x$).
...
\section{Random-walk representation (FFS Ch 12 / GJ \S18)}

\paragraph{Neighbour-vertex Simon-Lieb transfer kernel (Issue~\#3543 PR~1).}
The Simon-Lieb edge-peeling inequality
$\langle\sigma_i\sigma_j\rangle \le \beta J \sum_{e \ni i}
  \langle\sigma^{\{i,j\}\,\triangle\,\partial e}\rangle$ is recast as a one-step
transfer over \emph{neighbour vertices}, the foundation of the random-walk
representation of the two-point function. Define the kernel
\[
  K(j,u) = \begin{cases} 1 & u = j \\ \langle\sigma_u\sigma_j\rangle & u \ne j
  \end{cases}
  \qquad (\texttt{simonLiebKernel}),
\]
where the $u=j$ value $1 = \langle\sigma_\emptyset\rangle$ collects the
diagonal-edge contribution $\{i,j\}\triangle\{i,j\} = \emptyset$. Its basic
properties (\texttt{simonLiebKernel\_\{self,of\_ne,nonneg,le\_one\}}) follow from
GKS-I and $|\langle\sigma_A\rangle| \le 1$. For an edge $e$ incident to $i$ with
other endpoint $u = \texttt{Sym2.Mem.other}$, the per-edge identity
$\langle\sigma^{\{i,j\}\triangle\,\mathrm{toFinset}\,e}\rangle = K(j,u)$
(\texttt{correlation\_symmDiff\_eq\_simonLiebKernel}) uses
$\{i,j\}\triangle\{i,u\} = \{u,j\}$ for $u \ne j$. Reindexing the incident-edge
sum to a neighbour sum via \texttt{Finset.sum\_bij'} and \texttt{Sym2.other\_spec}
gives the headline one-step inequality
\[
  \langle\sigma_i\sigma_j\rangle
    \le \beta J \sum_{u \in \mathcal N(i)} K(j,u)
  \qquad (\texttt{correlation\_inducedGraph\_simon\_lieb\_neighbor}),
\]
for ferromagnetic $\langle J,0,\beta\rangle$ and $i \ne j$. Iterating this
transfer kernel (later PRs) yields the random-walk bound
$\langle\sigma_i\sigma_j\rangle \le \sum_{\text{walks } i \to j}(\beta J)^{\text{length}}$.
File: \texttt{Inequalities/SimonLiebKernel.lean}. Issue~\#3543 PR~1.

\paragraph{Walk-weight and fixed-length walk-sum framework (Issue~\#3543 PR~2).}
The walk-side companion of the transfer kernel. Define the geometric weight
$\texttt{walkWeight}\,z\,w = z^{w.\mathrm{length}}$ and the walk sum
\[
  \texttt{walkSum}\,G\,z\,i\,j\,n
    = \sum_{w : \text{length-}n\text{ walk } i \to j} z^{w.\mathrm{length}}
  \qquad (z = \beta J).
\]
Every length-$n$ walk has weight $z^n$, so
$\texttt{walkSum} = z^n \cdot \#\{\text{length-}n\text{ walks}\}
  = z^n \cdot (A^n)_{ij}$ (\texttt{walkSum\_eq\_pow\_mul\_card},
\texttt{walkSum\_eq\_pow\_mul\_adjMatrix\_pow}, via
\texttt{adjMatrix\_pow\_apply\_eq\_card\_walk}), nonnegative for $z \ge 0$, and
$\texttt{walkSum}\,z\,i\,j\,0 = [i = j]$ (\texttt{walkSum\_zero}). The headline is
the \emph{neighbour recurrence}
\[
  \texttt{walkSum}\,z\,i\,j\,(n+1)
    = z \cdot \sum_{u \in \mathcal N(i)} \texttt{walkSum}\,z\,u\,j\,n
  \qquad (\texttt{walkSum\_succ}),
\]
from the adjacency-matrix step $(A \cdot A^n)_{ij} = \sum_{u \sim i}(A^n)_{uj}$
(\texttt{adjMatrix\_mul\_apply}). This mirrors the one-step Simon-Lieb transfer
kernel; iterating the kernel against this recurrence (next PR) yields the
random-walk bound. File: \texttt{Inequalities/WalkSum.lean}. Issue~\#3543 PR~2.

\paragraph{Iterated Simon-Lieb transfer kernel (Issue~\#3543 PR~3).}
Iterating the one-step neighbour-vertex kernel $n$ times produces
\[
  \texttt{simonLiebIterate}\,p\,j\,0\,i = K(j,i), \qquad
  \texttt{simonLiebIterate}\,p\,j\,(n{+}1)\,i =
    \begin{cases} 1 & i = j \\
      \beta J \sum_{u \sim i} \texttt{simonLiebIterate}\,p\,j\,n\,u & i \ne j,
    \end{cases}
\]
...
$\langle\sigma_\emptyset\rangle = 1$ factor). The key estimate is the uniform upper bound
\[
  K(j,i) \le \texttt{simonLiebIterate}\,\langle J,0,\beta\rangle\,j\,n\,i
  \qquad \forall\, n, i
  \quad (\texttt{simonLiebKernel\_le\_simonLiebIterate}),
\]
by induction on $n$ (generalising $i$): base reflexivity; for $i=j$ both sides are
$1$; for $i \ne j$ the one-step kernel $\langle\sigma_i\sigma_j\rangle \le \beta J
\sum_{u \sim i} K(j,u)$ combines with the induction hypothesis via $\beta J \ge 0$
and \texttt{Finset.sum\_le\_sum}. For distinct $i \ne j$ this reads
$\langle\sigma_i\sigma_j\rangle \le \texttt{simonLiebIterate}\,\langle
J,0,\beta\rangle\,j\,n\,i$
(\texttt{correlation\_inducedGraph\_le\_simonLiebIterate}). This is the iteration
mechanism of the random-walk representation; a later PR connects the iterated
bound to the walk sum and the high-temperature exponential decay. File:
\texttt{Inequalities/SimonLiebIterate.lean}. Issue~\#3543 PR~3.

\paragraph{High-temperature uniform bound on the iterate (Issue~\#3543 PR~4).}
In the high-temperature regime $\beta J \cdot D \le 1$ ($D$ a vertex-degree
bound) the iterated Simon-Lieb kernel stays uniformly bounded:
\[
  \texttt{simonLiebIterate}\,\langle J,0,\beta\rangle\,j\,n\,i \le 1
  \qquad \forall\, n, i
  \quad (\texttt{simonLiebIterate\_le\_one}),
\]
by induction on $n$: the base is $K(j,i) \le 1$; for $i=j$ the value is $1$; for
$i \ne j$ the one-step transfer is $\beta J \sum_{u \sim i}(\cdots) \le \beta J
\cdot D \cdot 1 \le 1$ using the induction hypothesis, $\beta J \ge 0$, the degree
bound, and $\beta J \cdot D \le 1$. This is the boundedness input for the
random-walk representation: the iterated upper bound for
$\langle\sigma_i\sigma_j\rangle$ does not blow up as the number of transfer steps
grows (the prerequisite for the first-passage walk-sum limit). The corollary
$\langle\sigma_i\sigma_j\rangle \le 1$ for $i \ne j$
(\texttt{correlation\_inducedGraph\_le\_one\_of\_high\_temp}) follows by composing
with \texttt{correlation\_inducedGraph\_le\_simonLiebIterate}. File:
\texttt{Inequalities/SimonLiebIterateHighTemp.lean}. Issue~\#3543 PR~4.

\paragraph{Exponential decay via Simon-Lieb iteration (Issue~\#3543 PR~5).}
The iterated kernel yields exponential decay in the graph distance. If the target
$j$ is more than $n$ steps from $i$ ($n < \mathrm{dist}(i,j)$), the $n$-step
transfer never reaches the absorbing diagonal $K(j,j)=1$, so
\[
  \texttt{simonLiebIterate}\,\langle J,0,\beta\rangle\,j\,n\,i
    \le (\beta J \cdot D)^n
  \quad (\texttt{simonLiebIterate\_le\_pow\_of\_lt\_dist}),
\]
by induction on $n$: base $0 < \mathrm{dist}(i,j)$ (kernel $K(j,i)\le 1$); step
$\mathrm{dist}(i,j) > n+1$ forces $i \ne j$, and every neighbour $u \sim i$ still
has $\mathrm{dist}(u,j) > n$ (the edge moves the distance by at most one,
\texttt{Adj.diff\_dist\_adj}), so the induction hypothesis bounds each branch by
$(\beta J D)^n$; summing over $\le D$ neighbours times $\beta J$ gives $(\beta J
D)^{n+1}$. Specialising $n = \mathrm{dist}(i,j)-1$ for reachable distinct $i,j$
gives the exponential decay
\[
  \langle\sigma_i\sigma_j\rangle \le (\beta J \cdot D)^{\mathrm{dist}(i,j)-1}
  \quad (\texttt{correlation\_inducedGraph\_le\_pow\_dist}),
\]
which tends to $0$ with the distance in the high-temperature regime $\beta J
\cdot D < 1$ --- the random-walk (Simon-Lieb) derivation of high-temperature
correlation decay. File: \texttt{Inequalities/SimonLiebIterateDecay.lean}.
Issue~\#3543 PR~5.

\paragraph{$\mathbb Z^d$ wrapper of the Simon-Lieb decay (Issue~\#3543 PR~6).}
On the cubic lattice $\texttt{latticeGraph}\,d$ every induced-subgraph vertex has
degree $\le 2d$ (\texttt{inducedLatticeGraph\_degree\_le}), so the
high-temperature parameter is $\beta J \cdot 2d$ and
\[
  \langle\sigma_i\sigma_j\rangle
    \le (\beta J \cdot 2d)^{\mathrm{dist}(i,j)-1}
  \quad (\texttt{correlation\_inducedLatticeGraph\_le\_pow\_dist}),
\]
for reachable distinct sites $i,j$ of the induced subgraph on $\Lambda$
($0 < \mathrm{dist}(i,j)$), decaying exponentially in the induced-graph distance
in the high-temperature regime $\beta J \cdot 2d < 1$. The $\mathbb Z^d$ concrete
reading of the random-walk (Simon-Lieb) high-temperature correlation decay.
File: \texttt{Concrete/LatticeGraphCorrelation/SimonLiebDecayLatticeGraph.lean}.
Issue~\#3543 PR~6.

\paragraph{$\ell^1$ lattice-distance form of the decay (Issue~\#3543 PR~7).}
The induced-graph-distance decay refines to the physical $\ell^1$ lattice
distance. A minimal induced-graph walk maps (via the inclusion
$\uparrow\!\Lambda \hookrightarrow \mathbb Z^d$,
\texttt{SimpleGraph.Embedding.induce}) to a $\texttt{latticeGraph}\,d$ walk of the
same length, so $\mathrm{latticeDistance}\,i\,j \le \mathrm{dist}\,i\,j$
(\texttt{latticeDistance\_le\_inducedGraph\_dist}), the ambient lattice-graph
distance being the $\ell^1$ distance (\texttt{latticeGraph\_dist\_eq\_latticeDistance}).
In the high-temperature regime $\beta J \cdot 2d \le 1$ the base $\le 1$ makes the
geometric bound monotone (\texttt{pow\_le\_pow\_of\_le\_one}), giving
\[
  \langle\sigma_i\sigma_j\rangle
    \le (\beta J \cdot 2d)^{\mathrm{latticeDistance}(i,j)-1}
  \quad (\texttt{correlation\_inducedLatticeGraph\_le\_pow\_latticeDistance}),
\]
for reachable distinct sites, exponential decay in the $\ell^1$ lattice distance
--- the physically natural reading of the random-walk (Simon-Lieb) high-temperature
correlation decay. File:
\texttt{Concrete/LatticeGraphCorrelation/SimonLiebDecayLatticeDistance.lean}.
Issue~\#3543 PR~7.

\paragraph{Walk-sum representation of the two-point function (Issue~\#3543 PR~8).}
The genuine FFS Ch~12 walk-sum bound, finally using the walk-sum framework. The
diagonal $K(j,j)=1$ is exactly the length-$0$ walk $\texttt{walkSum}\,0\,j\,j = 1$
($\texttt{simonLiebIterate\_self}$), and the one-step transfer matches
$\texttt{walkSum\_succ}$, so by induction (uniform in $i$)
\[
  \texttt{simonLiebIterate}\,\langle J,0,\beta\rangle\,j\,n\,i
    \le \sum_{k=0}^{n} \texttt{walkSum}\,(\beta J)\,i\,j\,k + (\beta J\cdot D)^n
  \quad (\texttt{simonLiebIterate\_le\_sum\_walkSum\_add\_pow}),
\]
swapping and folding the neighbour double sum into the shifted walk sum via
\texttt{Finset.sum\_comm} and \texttt{Finset.sum\_range\_succ'}, the not-yet-absorbed
branches giving the geometric remainder $(\beta J D)^n$. For distinct $i \ne j$ the
length-$0$ walk vanishes, giving the finite-horizon walk-sum bound
\[
  \langle\sigma_i\sigma_j\rangle
    \le \sum_{k=1}^{n} \texttt{walkSum}\,(\beta J)\,i\,j\,k + (\beta J\cdot D)^n
  \quad (\texttt{correlation\_inducedGraph\_le\_sum\_walkSum\_add\_pow}),
\]
the finite-horizon form of the random-walk representation
$\langle\sigma_i\sigma_j\rangle \le \sum_{k\ge 1}\texttt{walkSum}\,(\beta J)\,i\,j\,k$.
This is the actual walk-sum representation of the correlation, distinct from the
bare exponential decay. File: \texttt{Inequalities/WalkSumRepresentation.lean}.
Issue~\#3543 PR~8.

\paragraph{Infinite walk-sum (random-walk) upper representation (Issue~\#3543 PR~9).}
In the high-temperature regime $\beta J \cdot D < 1$ the finite-horizon bound
passes to the limit. The number of length-$n$ walks is $\le D^n$, so
$\texttt{walkSum}\,z\,i\,j\,n \le (z\cdot D)^n$
(\texttt{walkSum\_le\_pow\_degree\_bound}), whence the positive-length walk sum is
summable for $z \cdot D < 1$ (\texttt{summable\_walkSum\_of\_lt\_one}, geometric
domination). Passing $\langle\sigma_i\sigma_j\rangle \le \sum_{k=1}^{n}
\texttt{walkSum}\,k + (\beta J\cdot D)^n$ to the limit (\texttt{ge\_of\_tendsto'};
the geometric remainder $\to 0$, the partial sums $\to$ the tsum) gives
\[
  \langle\sigma_i\sigma_j\rangle
    \le \sum_{k\ge 1}^{\infty} \texttt{walkSum}\,(\beta J)\,i\,j\,k
  \quad (\texttt{correlation\_inducedGraph\_le\_tsum\_walkSum}),
\]
the discrete random-walk (FFS Ch~12) upper bound on the Ising two-point function.
This completes the random-walk representation arc (kernel $\to$ walk-sum $\to$
iterate $\to$ decay $\to$ walk-sum representation $\to$ infinite sum). File:
\texttt{Inequalities/WalkSumTsum.lean}. Issue~\#3543 PR~9.

\paragraph{$\mathbb Z^d$ infinite walk-sum representation (Issue~\#3543 PR~10).}
On the cubic lattice $\texttt{latticeGraph}\,d$ (induced degree $\le 2d$) the
abstract infinite walk-sum bound specialises: in the high-temperature regime
$\beta J \cdot 2d < 1$, for distinct sites $i \ne j$,
\[
  \langle\sigma_i\sigma_j\rangle
    \le \sum_{k\ge 1}^{\infty} \texttt{walkSum}\,(\beta J)\,i\,j\,k
  \quad (\texttt{correlation\_inducedLatticeGraph\_le\_tsum\_walkSum}),
\]
the $\mathbb Z^d$ concrete form of the discrete random-walk (FFS Ch~12) upper
bound on the Ising two-point function. File:
\texttt{Concrete/LatticeGraphCorrelation/WalkSumTsumLatticeGraph.lean}.
Issue~\#3543 PR~10.

\paragraph{Cubic box induced-graph connectivity (infinite-volume infrastructure).}
The induced subgraph of $\texttt{latticeGraph}\,d$ on a centred box
$\texttt{cubicBox}\,d\,n = [-n,n]^d$ is connected: a single distance-reducing
lattice step that moves one differing coordinate one unit towards the target stays
between the endpoints, hence inside the box
(\texttt{cubicBox\_exists\_adj\_step}), and induction on the $\ell^1$ distance
gives reachability of any two box sites
(\texttt{inducedGraph\_cubicBox\_reachable}); distinct sites are therefore at
positive induced-graph distance (\texttt{inducedGraph\_cubicBox\_dist\_pos}). This
is a reusable abstraction that discharges the reachability ($0 < \mathrm{dist}$)
hypotheses of the finite-volume correlation-decay bounds at every cubic-exhaustion
stage, so infinite-volume lifts need not re-prove per-stage reachability. File:
\texttt{Concrete/CubicBoxConnectivity.lean}.

\paragraph{Infinite-volume Simon-Lieb exponential decay on $\mathbb Z^d$ (random-walk route).}
The finite-volume random-walk decay bound is \emph{uniform in the volume}: the
rate and exponent depend only on $i,j$, not on the exhaustion stage. Hence the
thermodynamic limit inherits it:
\[
  \langle\sigma_i\sigma_j\rangle_\infty
    \le (\beta J \cdot 2d)^{\mathrm{latticeDistance}(i,j)-1}
  \quad (\texttt{correlationInfinite\_latticeGraph\_le\_pow\_latticeDistance}),
\]
for distinct $i \ne j$ in the high-temperature regime $\beta J \cdot 2d < 1$. Each
stage $\texttt{correlationAlongExhaustion}\ldots n$ is bounded by the same
constant --- stages containing $\{i,j\}$ via the finite bound (reachability from
the cubic box connectivity \texttt{inducedGraph\_cubicBox\_dist\_pos}), stages not
containing $\{i,j\}$ contributing $0$ --- so $\texttt{correlationInfinite} =
\bigsqcup_n \texttt{correlationAlongExhaustion}$ is bounded by
\texttt{ciSup\_le}. This is the random-walk (FFS Ch~12) derivation of
infinite-volume high-temperature correlation decay (explicit $\beta J\cdot 2d$
rate), a contribution to the project's central long-term goal, the infinite-volume
limit. File:
\texttt{Concrete/LatticeGraphCorrelation/SimonLiebDecayInfiniteVolume.lean}.

\paragraph{Infinite-volume random-walk (walk-sum) upper representation on $\mathbb Z^d$.}
Beyond the \emph{decay} bound above, the random-walk \emph{structure} itself
survives the thermodynamic limit. Let $\texttt{walkTsumAlongCubic}\,d\,z\,i\,j\,n$
denote the box-confined positive-length walk sum $\sum_{k\ge 1}\texttt{walkSum}\,z\,i\,j\,k$
over walks inside the exhaustion box $(\texttt{cubicExhaustion}\,d).\mathrm{volume}\,n$
(and $0$ before the box contains $\{i,j\}$). Each stage walk sum is dominated,
uniformly in $n$, by the geometric series $\sum_{k\ge 1}(\beta J\cdot 2d)^k$: the
number of length-$k$ walks from $i$ is at most $(2d)^k$
(\texttt{walkSum\_le\_pow\_degree\_bound}, degree $\le 2d$), both series are
summable, and \texttt{Summable.tsum\_le\_tsum} gives
$\texttt{walkTsumAlongCubic\_le\_tsum\_geom}$. Consequently the family
$\{\texttt{walkTsumAlongCubic}\,d\,(\beta J)\,i\,j\,n\}_n$ is bounded above, and
since each $\texttt{correlationAlongExhaustion}\,n$ is bounded by its own
box-confined walk sum
($\texttt{correlation\_inducedLatticeGraph\_le\_tsum\_walkSum}$ when the box
contains the pair, $0$ otherwise),
\[
  \langle\sigma_i\sigma_j\rangle_\infty
    \le \bigsqcup_n \texttt{walkTsumAlongCubic}\,(\beta J)\,i\,j\,n
  \quad (\texttt{correlationInfinite\_latticeGraph\_le\_iSup\_tsum\_walkSum}),
\]
for distinct $i \ne j$ in the high-temperature regime $\beta J\cdot 2d < 1$, via
\texttt{le\_ciSup} and \texttt{ciSup\_le}. The two-point function in the infinite
volume is thus exhibited as a supremum of box-confined random-walk sums. File:
\texttt{Concrete/LatticeGraphCorrelation/WalkSumTsumInfiniteVolume.lean}.
