...
(PR~\#1820).}
The small module \path{Concrete/LatticeGraphCorrelation/CubicPseudoMass.lean}
...
\paragraph{Anchored cubic pseudo-mass named-rate bridge (PR~\#1821).}
The same module now names the concrete anchored cubic pseudo-mass at the pair
\((0,z)\) as
...
\path{cubicOriginPseudoMassFromParamsAtPair_nonneg}.  The conditional bridge
\path{HasExponentialDecay_cubicOriginPseudoMassFromParamsAtPair_of_le_high_temp_rate}
states that, once this named rate is bounded by
...
membership.  The closed-interval theorems have the shared prefix
\path{cubicNamedRate_ofReal_mem_Icc_latticeMass_} and cover the direct
high-temperature comparison and cubic-profile comparison cases:
\[
  \operatorname{ofReal}(m^-_{\mathrm{cub}}) \in
  [0,\mathrm{latticeMass}],
\]
while the half-open theorems have the shared prefix
\path{cubicNamedRate_ofReal_mem_Ioc_latticeMass_} and cover the positive
comparison, cubic-profile, and active-range plus high-temperature comparison
cases:
\(\operatorname{ofReal}(m^-_{\mathrm{cub}}) \in
...
the validating decay proof itself.  The five theorem names all start with
\path{cubicNamedRate_decay_mem_}; their suffixes cover the closed-interval
high-temperature comparison, the positive half-open comparison, the closed and
half-open cubic-profile cases, and the active-range plus high-temperature
comparison case.  Each theorem returns
\[
...
\texttt{.continuousAt} / \texttt{.differentiableAt}.
...
\texttt{correlation\_\{continuous, differentiable\}\_joint}.
...
the existing general-$h$ wrappers.
...
28 new thin direct-instantiation wrappers.
...
direct-instantiation wrappers. Most ferromagnetic ℤ$^d$ /
...
$t \ge 0$. 20 new thin direct-instantiation wrappers.
...
volume layers, giving $32$ thin direct-instantiation wrappers.
...
giving $16$ thin direct-instantiation wrappers.
...
to the four volume layers, giving $24$ thin direct-instantiation
wrappers in
\texttt{IsingModel/AmbientLattice/Analyticity.lean},
\texttt{AmbientLattice/SpecialCases.lean}, and
\texttt{Concrete/LatticeGraphCorrelation.lean}.
...
derivative (Step~575)) are lifted to the four volume layers,
giving $16$ thin direct-instantiation wrappers in
\texttt{IsingModel/AmbientLattice/Analyticity.lean},
\texttt{AmbientLattice/SpecialCases.lean}, and
\texttt{Concrete/LatticeGraphCorrelation.lean}.
...
lifted to the four volume layers, giving $24$ thin
...
lifted to the four volume layers, giving $16$ thin
direct-instantiation wrappers in
\texttt{IsingModel/AmbientLattice/Analyticity.lean},
\texttt{AmbientLattice/SpecialCases.lean}, and
\texttt{Concrete/LatticeGraphCorrelation.lean}.
...
are lifted to the four volume layers, giving $20$ thin
direct-instantiation wrappers in
\texttt{IsingModel/AmbientLattice/Analyticity.lean},
\texttt{AmbientLattice/SpecialCases.lean}, and
\texttt{Concrete/LatticeGraphCorrelation.lean}.
...
\texttt{IsingModel/AmbientLattice/Analyticity.lean},
\texttt{AmbientLattice/SpecialCases.lean}, and
\texttt{Concrete/LatticeGraphCorrelation.lean}. The two
...
\texttt{IsingModel/AmbientLattice/Analyticity.lean},
\texttt{AmbientLattice/SpecialCases.lean}, and
\texttt{Concrete/LatticeGraphCorrelation.lean}. Each is a thin
direct-instantiation wrapper of the abstract theorem on the
...
\begin{itemize}
  ...
  \item \emph{Along-exhaustion wraps}:
    ...
    \texttt{AmbientLattice/SpecialCases.lean}.
  \item \emph{$\mathbb{Z}^d$ concrete wraps}: 8 theorems in
    \texttt{Concrete/LatticeGraphCorrelation.lean}, covering both
    the $\Lambda$-direct family on $\Lambda \subseteq \mathbb{Z}^d$
    and the along-exhaustion family on
    $\mathrm{Exhaustion}\,(\mathrm{Fin}\,d \to \mathbb{Z})$.
\end{itemize}
...
matrix-tree machinery not yet available in Mathlib).
