...
The composition isolates the prefactor
$\max(1, (M^\alpha)^{-1})\cdot 2^\alpha = M^{-\alpha}\cdot 2^\alpha$ (whenever $M\le 1$;
in particular the GJ p.~312 $m^{-\alpha}$ scaling) from the $(1+t)^{-\alpha}$ polynomial-decay
body matching the existing infinite-sum infrastructure
