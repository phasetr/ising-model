...
(Step 117l, PR~\#2663).} The new module
...
$m := \mathrm{latticeMass}$.  Five named declarations are added:
\begin{itemize}
...
\item \texttt{lemma\_17\_5\_2\_cubic\_high\_temp\_lower\_capstone} and
  ...
  high temperature $\beta J (2d) < 1$, derived from the existing
  \texttt{cubicNamedRate\_capstone\_bundle\_of\_cubic\_corr\_mem\_Ioo\_and\_profile}
  bundle.  These give an unconditional positivity statement on
  $\mathrm{latticeMass}$ in the high-temperature regime.
\item \texttt{lemma\_17\_5\_2\_constant\_existence} -- a strict-positivity
  alias of \texttt{discrete\_hls\_constant} ($2\alpha > d$).
\end{itemize}
The lower-bound side of GJ~Lemma 17.5.2 is therefore now exposed under
the named capstone API on the cubic exhaustion at high temperature; the
upper-bound side remains a hypothesis pending the substantive
Lipschitz~+~HLS argument of GJ~Theorem~17.5.1 (proof on p.~312).
