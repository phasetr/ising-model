...
\path{cubicOriginPseudoMassFromParamsAtPair_eq_iff}.  These rewrite
comparisons between the named rate and the underlying concrete
...
Thus active-range membership alone gives a nonzero named pseudo-mass and a
positive \(\mathrm{ofReal}\) coercion, while the final positive
\(\mathrm{latticeMass}\) bridge only asks for the named-rate comparison with
\(-\log(\beta J\,2d)\).
