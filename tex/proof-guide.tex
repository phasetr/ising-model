...
via \texttt{Complex.exp\_re} and \texttt{Real.cos\_neg}) supply the lower-bound entry:
\(\|Z_{\mathbb{C}}\|\ge\mathrm{Re}(Z_{\mathbb{C}})=Z_{\mathbb{R}}(\beta.\mathrm{re})-(\text{cosine deficit})\),
with the volume-uniform control on the deficit being the remaining quantitative step.
\end{theorem}
