...
subset.  Equivalently, it is enough to verify the Cauchy property on every
closed interval contained in the high-temperature interval: around each
\(\beta\) one chooses
...
\texttt{\_on\_Icc} closed-interval variants.  These statements provide a
non-Dini route to the same provider, suitable for future derivative-profile
Cauchy estimates.  Conversely,
...
pointwise derivative-profile convergence plus compact-interval metric Cauchy
control, then feed the provider into the interval infinite-HLS comparison,
...
hypothesis and the pointwise derivative-profile limit hypothesis used by both
the Cauchy and Dini provider routes.  \texttt{CauchyProvider.lean} proves the
corresponding Cauchy provider helper without importing downstream HLS assembly
layers.  \texttt{CauchyFiniteHLS.lean} then feeds that provider into the
...
and pointwise derivative-profile convergence hypotheses through the provider
helper and into ten concrete automatic-active entry points: compact upper
bounds, rate-comparison sandwiches, profile-lower sandwiches, and the matching
high-temperature capstones.  Thus downstream callers can work directly with
the Cauchy-shaped analytic hypotheses while keeping the endpoint rate or
profile lower input visible.
