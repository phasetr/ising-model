...
stages. The explicit starting point for the per-stage increment bound: the
difference splits into the new shell edges of stage $k+1$ (decaying Ursell, far
from $\{x,z\}$) and the interior edges shared with stage $k$ (Ursell differing only
by the inter-stage correlation differences).
\end{proposition}
