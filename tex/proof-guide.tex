...
\le\langle\cdot\rangle_G$, products of nonnegative correlations monotone). On the cubic
exhaustion the full-graph two-point functions are bounded by the infinite-volume
correlations and decay spatially, so the localized shell term
$J\sum_{e\in E_0}\mathrm{Cov}_0(\sigma^{\{x,z\}},\sigma_e)$ is geometric using the
correlation-side Part-B machinery, without requiring Part B on the bond-deleted graph.
\end{theorem}
