...
2\,\mathrm{cf}^{\dots}$ by $d\,(2(k+1)+1)^d\cdot 2\,\mathrm{cf}^{\dots}$. (Composition
with the tight per-stage correlation increment is deferred pending a single shared
\texttt{Fintype} edge-set instance: the two lemmas currently use distinct file-local
instances, so the shell terms are only \texttt{Subsingleton}-equal.)
\end{proof}
