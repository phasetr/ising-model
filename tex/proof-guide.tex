...
$M_\infty := \sup_n M_\text{along}(n)$. The failure mode is exactly the
odd-$|A|$ obstruction already flagged in
\texttt{correlationInfinite\_neg\_h\_of\_even\_card}: at $h<0$
ferromagnetic, uncovered early stages force $M_\text{along}(n) = 0$,
and covered stages give $M_\text{along}(n) \le 0$, so
$M_\infty(h) = 0$ while $M_\infty(|h|) > 0$. Glimm--Jaffe's standard
thermodynamic limit (e.g.\ $\pm$BC or $h \to 0^\pm$) restores the
equality; upgrading the definition is out of scope here.
...
pointwise equality above. Further $\infty$-volume refinements (sign
control, zero-value at $h < 0$ under a ``missed stage'' hypothesis)
are tracked in issue~\#770.
\end{theorem>
