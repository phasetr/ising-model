...
\paragraph{Edge-peeling bound for \texttt{weightSum} (PR \#898, step 102, partial).}
...
\end{theorem}
\noindent\textit{Status}: Statement compiles; the body uses \texttt{sorry} pending the
tsum injection argument (ENNReal monotone comparison + $n \mapsto n - \mathbf{1}_{e_0}$
injection from $\{\partial n = \{i,j\}, n_{e_0} \ge 1\}$ to $\{\partial n' = \{i,j\} \triangle e_0\}$,
combined with $N \to \infty$ limit via
\texttt{tendsto\_weightSum\_atTop\_iSup\_of\_nonneg}).
