...
\#2706 finite real-centre patch.  Its hypotheses are compactness of each
...
stage branch identities, eventual equality on every pairwise ball overlap, and
a selected ball \(i_0\) with \(h_{i_0}=p.h\) and \(0<r_{i_0}\).  The conclusion
...
The concrete \(\mathbb Z^d\) wrapper is
\begin{Verbatim}
freeEnergyComplexAlongExhaustion_finiteSubseqBranchLimitFamily_compactOpen_patch_real_latticeGraph
\end{Verbatim}
in \texttt{Concrete/LatticeGraphCorrelation/PerStageComplex.lean}.
