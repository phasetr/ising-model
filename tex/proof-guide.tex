...
\sloppy  % prevent overfull hboxes from long identifiers
...
GibbsMeasure.lean            Z, Gibbs expectation, correlation
  ...
  |     LeeYang.lean              Lee-Yang circle theorem
  |
...
\texttt{Asano.lean} $\to$ \texttt{LeeYang.lean} $\to$
\texttt{FreeEnergy.lean}. \\
\emph{Key idea:} the partition polynomial is multilinear; Asano
contraction reduces $n$ variables to $n-1$ while preserving
nonvanishing on the polydisk. Induction on $n$.
...
\begin{itemize}
  ...
  \item \textbf{Axioms.} Four axioms from $\varphi^4$ measure theory
    are used: \texttt{phi4\_single\_site\_nonneg},
    \texttt{lebowitz\_third/four/inductive}.
...
\texttt{Combinatorics.SetFamily.\allowbreak FourFunctions.fkg}
...
\noindent\textbf{Prerequisites:} \texttt{Asano.lean}. \\
\textbf{Role:} Proves that the Ising partition polynomial has no
zeros on the open unit polydisk. \\
\textbf{What to look for:} The proof is by induction on the number
of variables. Each step uses Asano contraction to merge two variables
into one. The base case ($n=0$) is trivial ($P = 1$).
