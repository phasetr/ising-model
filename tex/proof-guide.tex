...
\texttt{outerVertexBoundary G S} (PR~\#785).}
...
the inner box $B$. PR~\#785 formalises this notion: for any
locally finite simple graph $G$ and any Finset $S$ of vertices,
\[
  \mathrm{outerVertexBoundary}\,G\,S
  \;:=\; (S.\mathrm{biUnion}\,G.\mathrm{neighborFinset}) \setminus S
  \;\equiv\; \{\,y \notin S \mid \exists x \in S,\ G.\mathrm{Adj}\,x\,y\,\}.
\]
Together with the obvious membership characterisation
...
\]
(\texttt{outerVertexBoundary\_card\_le\_sum\_degrees}), this is
the indexing object the Simon--Lieb statement plugs into. The
$\mathbb Z^d$ wrapper
$|\partial^v_o S| \le 2 d \cdot |S|$
(\texttt{latticeGraph\_outerVertexBoundary\_card\_le}) follows by
chaining the generic bound with PR~\#784's per-vertex
$\mathrm{latticeGraph\_degree\_le}$.
