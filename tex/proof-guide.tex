...
The finite derivative-profile continuity input is now discharged.  For each
exhaustion stage, the beta derivative of the finite two-point function is
continuous in beta: before \(\{x,z\}\) is contained in the stage volume the
along-exhaustion correlation is identically zero, and afterward the derivative
is the explicit finite-volume beta-derivative formula, a finite sum of
continuous correlations.  The strengthened monotone and antitone Dini criteria
feed this continuity in automatically, leaving only the stagewise order
direction, pointwise convergence, and continuity of the limiting derivative
profile as external provider inputs.
