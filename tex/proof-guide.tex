...
$\forall n,\ \Lambda_n\ne\emptyset$.  The concrete $\mathbb Z^d$
specializations are recorded in
\texttt{Concrete/LatticeGraphCorrelation/PerStageComplex.lean}.

This is a branch-family input for the normal-family/Vitali part of
GJ \S 4.6 Thm 4.6.2; it does not by itself prove local uniform
convergence of the chosen branches.  The branch agrees with the
principal \texttt{freeEnergyComplexAlongExhaustion} value at the
basepoint $h_0$, while the local analytic representative may differ
from the principal \texttt{Complex.log} branch away from $h_0$.
