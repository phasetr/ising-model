...
infinite-HLS bridge interface.  Eleven entry points feed
...
package, the finite-HLS-to-infinite Lipschitz package, and the enlarged
upper/sandwich packages.  This keeps the bridge layer aligned with the
provider-shaped capstone API: callers moving from finite derivative bounds or
the infinite HLS denominator comparison into the infinite-HLS pipeline no
longer need to name the limiting derivative profile \(g'\).

The compact-Cauchy criteria are also wired directly to that bridge layer.
\texttt{DerivativeLimitProviderCauchyHLS.lean} packages seven entry points that
...
\texttt{DerivativeLimitProviderDiniHLS.lean} packages seven entry points that
...
infinite-Lipschitz packages, finite-HLS-to-infinite Lipschitz package,
path-rate enlarged package, and enlarged upper/sandwich assemblies.  This
makes the Dini route directly consumable by the HLS-uniform capstone path while
leaving the stagewise order, pointwise convergence, limiting-profile
continuity, and finite derivative/HLS inputs visible.
