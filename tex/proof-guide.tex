...
\(K\subset\texttt{leeYangDomain}\) containing the real centre.  For that
geometry, compact-open compactness and eventual stage-level overlap equality
then produce a compact finite real-centred Lee--Yang cover package and a patch
differentiable on \(K\), with the real-centre value identified as
\(\texttt{freeEnergyInfinite}\).
...
This remains conditional and post-Montel.  It still assumes the coherent
eventual-overlap data, compact-open compactness of the selected branch
families, and eventual overlap equality; it does not construct coherent
branches, prove compactness of branch families, or perform the diagonal
extraction.
Reference: Glimm--Jaffe \cite{glimm-jaffe}, \S 4.6, pp.~67--70.
\end{theorem}
