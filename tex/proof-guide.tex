...
and the value increment $c_k-c_{k+1}$ extends complex-analytically with a
$\mathrm{poly}\cdot\mathrm{geometric}$ bound on a $\beta$-neighborhood, so its derivative
inherits the same bound — converting the established \emph{value}-increment estimate into
the required \emph{derivative}-increment estimate.
\end{theorem}
