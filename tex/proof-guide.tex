...
prefactor-free distance-decay iteration that will produce the geometric factor
has its inductive step in place: for a non-adjacent pair \(i\neq j\) on
\(\mathbb{Z}^d\), if every neighbour \(k\) of \(i\) satisfies
...
(\texttt{correlationInfinite\_latticeGraph\_le\_of\_neighbors\_le}).  Only this
single step is proved; iterating it over the lattice distance is *intended* to
yield \((\beta J\,2d)^{\mathrm{dist}}\) without the volume-dependent prefactor of
the naive finite-volume bound, but that full distance induction is not yet
formalized.  Applying the one-step bound (under \(0\le\beta J\)) with the universal \(\le 1\)
bound gives the first quantitative decay step
\(\langle\sigma_i\sigma_j\rangle^\infty\le\beta J\,2d\) for every non-adjacent pair
...
\(0\le\beta J\,2d<1\).  Its geometric foundation is in place: a vertex lying in the stage-\(n+1\) cube but not the
