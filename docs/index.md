...
| `vitali_bridge` + `vitali_bridge_leeYangDomain` | **Vitali bridge** (abstract ∞-vol ingredient): locally uniform limit of holomorphic functions on an open set `U` is holomorphic on `U`. Direct application of mathlib `TendstoLocallyUniformlyOn.differentiableOn`. Specialised to `U = leeYangDomain`. | `ComplexAnalyticity.lean` | ∞-vol Vitali (bridge only; boundedness ingredient TODO) |
...
**Not yet formalized**: slitPlane membership on the full Lee-Yang domain
(requires a continuous branch-selection argument from a real-positive
basepoint) and the infinite-volume analyticity of `f(h)` via Vitali
convergence (GJ Thm 4.6.2 full statement).
