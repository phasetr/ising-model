...
| §5.1 | Cluster property — conditional decay: summable ⇒ cofinite → 0 (pp. 72–74) | **Done (conditional real-analysis step)** | `truncated2Infinite_tendsto_cofinite_zero_of_summable` (`AmbientLattice.lean`): if `Summable (fun j : V => truncated2Infinite G Λ p i j)` (a finite-susceptibility / non-critical condition), then the two-point function tends to `0` along `Filter.cofinite`. On ℤ^d, `Filter.cofinite` coincides with the "|r| → ∞" filter, giving a clean conditional cluster decay statement. Direct specialization of mathlib's `Summable.tendsto_cofinite_zero`. ℤ^d wrapper `truncated2Infinite_latticeGraph_tendsto_cofinite_zero_of_summable`. Unconditional exponential decay in pure phases (Simon–Lieb inequality, GJ §5.1) remains unformalized; this PR supplies the elementary real-analysis building block. (PR #779.) |
