...
| §5.1 | Cluster property — truncated 3-point trivial slices (pp. 72–74) | **Done (trivial slices)** | `truncated3_J_zero_of_pairwise_distinct` and `truncated3_beta_zero` (`Inequalities/GHS.lean`): the Ursell 3-point function vanishes at `J = 0` (pairwise distinct sites, via `correlation_J_zero` giving the `t³ - 3t³ + 2t³ = 0` combination with `t = tanh(β·h)`) and at `β = 0` (any sites, via `correlation_beta_zero_vanish_of_nonempty_A` making every Ursell term zero). Extension of `truncated2_*` lemmas to the 3-point case. |
