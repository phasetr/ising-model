...
| §4.6 | **Prop 4.6.1 concrete toolkit — named limits, sandwich, invariances, cross-family equivalences** | **Done (PRs #646–#656)** | Building on the four concrete ℤ^d Fekete applications (linearBox / stripeBrick2D / slabBrick / centeredSlab), PRs #646–#656 assemble a complete toolkit for reasoning about the infinite-volume free-energy density limits. **Named limits** (`Classical.choose`): `freeEnergyInfinite_linearBox`, `_stripeBrick2D`, `_slabBrick`, `_centeredSlab` (PRs #646/#648). **Nonempty lemmas**: per-family nonempty-slab predicates under the positivity hypotheses on widths. **J=0 closed form**: all four `freeEnergyInfinite_* ⟨0, h, β⟩ = log(2·cosh(β·h))` under ferromagnetic `0 ≤ h, 0 < β` (PRs #647/#648), via per-stage `IsingModel.freeEnergy_J_zero` + `tendsto_nhds_unique`. **Infinite-volume sandwich**: `log 2 ≤ freeEnergyInfinite_* ≤ log 2 + |β|·((d+1)·|J| + |h|)` under ferromagnetic parameters, transporting PR #645's per-stage sandwich via `ge_of_tendsto` / `le_of_tendsto` (PR #650). **Nonnegativity**: `0 ≤ freeEnergyInfinite_*` via per-stage `freeEnergy_nonneg_of_ferromagnetic` (PR #653). **Translation invariance of the Fekete limit**: `freeEnergyInfinite_*_tendsto_shift` showing any coord-shift of the slab sequence converges to the same named limit (via PR #255's `freeEnergyΛ_vaddFinset_eq`) (PR #652). **Cross-family equivalences**: `freeEnergyInfinite_linearBox = freeEnergyInfinite_slabBrick Fin.elim0` (PR #651), `freeEnergyInfinite_stripeBrick2D = freeEnergyInfinite_slabBrick (fun _ => w)` (PR #651), `freeEnergyInfinite_centeredSlab = freeEnergyInfinite_slabBrick` via Finset identity `centeredSlab widths n = shift_(-n) (slabBrick widths (2n))` + subsequence convergence at `n ↦ 2n` (PRs #654/#655), direct transitive identities (PR #656). All four `freeEnergyInfinite_*` are now fully related in the matching-widths slice of the parameter grid. |
