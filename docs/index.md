...
| `{hamiltonian,partitionFunction,freeEnergy}_J_zero` + `_eq_bot_at_J_zero` identities + along-exhaustion + `freeEnergyInfinite_J_zero_of_eventually_nonempty` | **J=0 graph-independent closed form**: `Z_G ⟨0, h, β⟩ = (2·cosh(βh))^|ι|`, `f_G ⟨0, h, β⟩ = log(2·cosh(βh))` (any `G`). Core ⊥-equivalence identities `hamiltonian_eq_bot_at_J_zero`, `partitionFunction_eq_bot_at_J_zero`, `freeEnergy_eq_bot_at_J_zero` provide the graph-independence, and `_J_zero` closed forms are `.trans` of these with `_bot`. | `GibbsMeasure.lean`, `FreeEnergy.lean`, `AmbientLattice.lean`, `AmbientLatticeSum.lean` | Fourth slice of zero-parameter closed forms (after β=0 / J=h=0 / bot); ⊥-equivalence chain for future reuse |
...
| §4.6 | **Prop 4.6.1 (`f_Λ` convergence)** | **Partial (disjoint-union super-additivity + upper bound + `log Z → ∞`; Fekete pending)** | Discretized Λ↑: `freeEnergy_convergent_subgraph`. Disjoint-union super-additivity on ambient lattice: `log_partitionFunction_inducedGraph_disjUnion_super_additive`, `log_partitionFunctionΛ_le_of_disjoint_union`, `card_mul_freeEnergyΛ_le_of_disjoint_union`. Companion bounds: `partitionFunction_ge_one_of_ferromagnetic`, `freeEnergy_upper_bound`, `BddAbove_freeEnergyAlongExhaustion_range`. Divergence: `{log_,}partitionFunctionAlongExhaustion_tendsto_atTop` (infinite V, ferromagnetic). Iso-invariance infra: `partitionFunction_map_equiv`. Fekete convergence of `freeEnergyAlongExhaustion` pending. |
