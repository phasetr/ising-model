...
| §4.6 | **Thm 4.6.2 (analyticity)** | **Partial: finite-real Done + finite-complex `Z`/`f` support** | Finite-real free energy analyticity: `freeEnergyH_analyticOn` etc. Finite-complex support: `partitionFunctionComplex_analyticAt_{h,J,beta}` (Z entire in each parameter) + `freeEnergyComplex_analyticAt_{h,J,beta}` under `Z ∈ Complex.slitPlane` (log via mathlib `AnalyticAt.clog`). (ComplexAnalyticity.lean). **Not yet**: slitPlane-membership for specific domains like `|Im h| < Re h` (needs Lee-Yang nonvanishing + a branch-selection / winding-number argument from a basepoint `P(0)=1`), joint analyticity in `(J, h, β)`, infinite-volume Vitali lift. |
