...
| §4.6 | **Concrete 1D linear brick on `latticeGraph 1`** | **Definitions + combinatorics** | `IsingModel.Concrete.linearBox (n : ℕ) : Finset (Fin 1 → ℤ)` (`Concrete/LinearBrick.lean`) — contiguous 1D line of `n` points. `linearBox_card` (= n), `linearBox_card_add`, `linearBox_one_card_ne_zero`, `mem_linearBox`, `linearBox_disjoint_shift`, `linearBox_union_shift`. Foundation for applying the generic-Finset Fekete (PR #638) to get concrete Prop 4.6.1 at general ferromagnetic parameters on the 1D half-line. The `log_partitionFunction` super-additivity step + `BddAbove` step + final `Tendsto` are tracked as follow-up. (PR #639.) |
