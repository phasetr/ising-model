...
| §4.2 | ℤ^d `correlationJ` bounds + J/h/β monotone convergence wrappers | **Done** | `correlationJ_{nonneg,le_one}_latticeGraph` + `correlation_convergent{,_h,_beta}_latticeGraph` (`Concrete/LatticeGraphCorrelation.lean`): ℤ^d direct wrappers for the GJ §4.2 Prop 4.2.1 slice and Thm 4.2.3 J/h/β monotone-bounded convergence, applied to the Λ-induced subgraph of `latticeGraph d`. Thin pass-throughs of the abstract `IsingModel.correlationJ_{nonneg,le_one}` and `correlation_convergent{,_h,_beta}`. (PR #608.) |
