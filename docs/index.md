...
| §4.6 | **Thm 4.6.2 (analyticity)** | **Partial: finite-real Done + finite-complex `Z` support** | Finite-real free energy analyticity: `freeEnergyH_analyticOn` etc. (existing). Support toward Thm 4.6.2: `partitionFunctionComplex_analyticAt_{h,J,beta}` establishes per-parameter analyticity of the complex-valued partition function at finite volume (ComplexAnalyticity.lean). **Not yet**: complex free-energy analyticity on Lee-Yang domain `|Im h| < Re h` (requires `Complex.log` on slitPlane), joint analyticity in `(J, h, β)`, infinite-volume Vitali lift. |
