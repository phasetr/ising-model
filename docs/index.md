...
| §4.6 | **Prop 4.6.1 (`f_Λ` convergence) — generic-Finset-sequence Fekete** | **Advanced** | `freeEnergy_of_finset_sequence_tendsto_of_superadditive` (`AmbientLatticeSum.lean`): given `B : ℕ → Finset V` with `|B(m+n)| = |B m| + |B n|`, `log Z_{B m} + log Z_{B n} ≤ log Z_{B(m+n)}`, `|B 1| ≠ 0`, and `BddAbove (freeEnergy (inducedGraph G (B n)) p)`, the free-energy-density sequence converges. Unblocks concrete ℤ^d Prop 4.6.1 completion for "brick" / "stripe" / "half-line" exhaustions that satisfy the linear-growth hypotheses despite not covering all of `V` (so not an `Exhaustion V` in the strict sense). Identical Fekete-argument structure to `freeEnergyAlongExhaustion_tendsto_of_superadditive` but factored through an arbitrary Finset sequence. (PR #638.) |
