...
| §4.6 | **Thm 4.6.2 (analyticity)** | **Partial: finite-real Done + finite-complex `Z`/`f` support** | Finite-real free energy analyticity: `freeEnergyH_analyticOn` etc. Finite-complex support: `partitionFunctionComplex_analyticAt_{h,J,beta}` (Z entire in each parameter) + `freeEnergyComplex_analyticAt_{h,J,beta}` under `Z ∈ Complex.slitPlane` (log via mathlib `AnalyticAt.clog`). (ComplexAnalyticity.lean). Joint analyticity: `partitionFunctionComplex_analyticAt_joint` / `freeEnergyComplex_analyticAt_joint` (3-variable `(J,h,β) ∈ ℂ³`, slitPlane hypothesis for `f`). Real-complex compat: `partitionFunction_ofReal_eq_partitionFunctionComplex` + `freeEnergy_ofReal_eq_freeEnergyComplex`. Slit-plane membership on the real slice: `partitionFunctionComplex_mem_slitPlane_of_real` (at real params `Z > 0`, hence `z.re > 0`, hence in slit-plane; so `freeEnergyComplex_analyticAt_*` applies). **Not yet**: slitPlane-membership on specific domains like `|Im h| < Re h` (needs Lee-Yang nonvanishing + branch-selection / winding-number argument from basepoint `P(0)=1`), infinite-volume Vitali lift. |
