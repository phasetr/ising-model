...
| §4.1–§4.3 | ℤ^d Hamiltonian spin-flip / J=0 graph-independence / spinProduct helper wrappers | **Done** | `hamiltonian_flip_eq_latticeGraph`, `hamiltonian_neg_h_latticeGraph`, `hamiltonian_zero_params_latticeGraph`, `hamiltonian_eq_bot_at_J_zero_latticeGraph`, `partitionFunction_eq_bot_at_J_zero_latticeGraph`, `correlation_eq_bot_at_J_zero_latticeGraph`, `spinProduct_{singleton,union,sq}_latticeGraph` (`Concrete/LatticeGraphCorrelation.lean`). Thin pass-throughs of the abstract theorems from `IsingModel/GibbsMeasure.lean` and `IsingModel/Hamiltonian.lean` on `Ambient.inducedGraph (latticeGraph d) Λ`. Completes the ℤ^d spin-flip / h-reflection / J=0 graph-independence stack (Hamiltonian → Z → correlation, complementing the prior `freeEnergy_eq_bot_at_J_zero_latticeGraph` wrapper) and adds three basic `spinProduct` helpers. (PR #615.) |
