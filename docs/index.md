...
| §5.1 | Cluster property — `J = 0` instance (pp. 76–77) | **Done (trivial slice)** | `truncated2_J_zero_of_ne` (`Inequalities/GHS.lean`): for `i ≠ j`, `truncated2 G ⟨0, h, β⟩ i j = 0`. Exact (all separations) via `correlation_J_zero` (`⟨σ^A⟩ = tanh(β·h)^{\|A\|}`). Corresponds to the non-interacting / high-temperature limit of the §5.1 cluster property. General-`J` decay-at-large-separation remains unformalized. |
