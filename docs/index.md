...
| §4.6 | **Thm 4.6.2 (analyticity)** | **Done (finite/real + finite/complex Z)** | Finite-real: `freeEnergyH_analyticOn` etc. Finite-complex partition function entire in each parameter: `partitionFunctionComplex_analyticAt_{h,J,beta}` (ComplexAnalyticity.lean). Full infinite-volume complex analyticity (Vitali step): not yet |
