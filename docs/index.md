...
| `susceptibility_eq_abs_h` (§5.3) | **`\|h\|`-closed form** `χ(⟨J,\|h\|,β⟩) = χ(⟨J,h,β⟩) + M(⟨J,\|h\|,β⟩) - M(⟨J,h,β⟩)`: unconditional (no ferromagnetism). `abs_choice h` split, using `susceptibility_neg_h` + `magnetization_neg_h` in the negative case. | `PhaseTransition.lean` | PR #771 |
...
| §5.3 | Z₂ `h`-symmetry (`h → -h`) across the tower: correlations, magnetization, truncated {2,3,4}-point, susceptibility | **Done (finite + Λ + along-exhaustion + ∞-vol + ℤ^d)** | Capstone: `correlation_neg_h` (PR #754, `⟨σ^A⟩(-h) = (-1)^\|A\|·⟨σ^A⟩(h)`), `magnetization_neg_h`, `truncated{2,3,4}_neg_h` (PRs #755–#758), `correlation_eq_abs_h_of_even_card` (PR #760); Λ-layer `correlationΛ_neg_h` / `magnetizationΛ_neg_h` (PR #762), `correlationΛ_eq_abs_h_of_even_card` (PR #763); along-exhaustion (PR #764); ∞-vol even-`\|A\|` `correlationInfinite_neg_h_of_even_card` / `_eq_abs_h_of_even_card` (PR #765); susceptibility `susceptibility_neg_h`: `χ(-h) = χ(h) - 2·M(h)` (PR #767); `abs_magnetization_eq_magnetization_abs_h`: `\|M(h)\| = M(\|h\|)` under ferromagnetism at `\|h\|` (PR #769); `susceptibility_eq_abs_h`: `χ(\|h\|) = χ(h) + M(\|h\|) - M(h)` unconditional (PR #771). ℤ^d wrappers at each layer (PRs #753, #759, #761, #766, #768). J=0 / β=0 / h=0 trivial slices `truncated{2,3,4}Infinite_{J,h,β}_zero_*` (PRs #740–#751). |
