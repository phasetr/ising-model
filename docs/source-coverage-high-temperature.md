...
|---|---|---|---|---|---|---|---|---|
| FV (3.41), p. 116 | One nearest-neighbour Ising bond; the identity has no boundary condition. | Bond spin product `sigma_i sigma_j`. | Source inverse temperature `beta >= 0`; the algebraic identity holds for every real `beta`. | Local identity with no volume limit. | `exp(beta * sigma_i * sigma_j) = cosh(beta) + sigma_i * sigma_j * sinh(beta) = cosh(beta) * (1 + tanh(beta) * sigma_i * sigma_j)`. | `IsingModel.exp_edgeSpin_decomp` for the first equality only. | Take `e = s(i,j)` and Lean `alpha = beta_source` for literal unit coupling, or `alpha = beta_Lean * J` for the project's uniform scalar-coupling rescaling. This gives the exponential-to-cosh/sinh equality exactly. The final factorized equality is derived where used, for example in `boltzmannWeight_h_zero_prod`, from `Real.tanh_eq_sinh_div_cosh`, nonvanishing of cosh, and field algebra. | Exact owner for the first equality plus derived completion of the full FV chain. |
...
| FV (3.44), p. 117 | The same finite plus-boundary volume; exterior spins are pinned and only `i` in `Lambda` is summed. | Local incidence monomial `omega_i ^ I(i,E)` for a fixed edge subset `E`. | No parameter beyond the selected `E`. | Each interior vertex of every finite `Lambda`; no infinite-volume limit. | The two-spin sum is `2` for even incidence and `0` for odd incidence. | `IsingModel.Ambient.Spin.sum_toSign_pow_real`. | Set its natural-number exponent `k = I(i,E)` and identify `Spin.up` and `Spin.down` with the source values plus and minus one. Separately, `sum_indicator_agreesOff_plus_prod_pow` is the pinned-boundary aggregate assembly: on the finite ambient support of `E^b_Lambda`, with `eta = plusConfig` and `k(v) = I(v,E)`, it applies the local collapse across all interior vertices while exterior factors remain one. That aggregate is used toward FV (3.45), rather than being the weakest owner of the literal FV (3.44) row. | Exact literal single-spin identity; separate exact aggregate assembly toward FV (3.45). |
...
| GJ Theorem 18.3.1, p. 330 | Continuum `P(phi)_2` cluster expansion with a finite interaction cutoff, uniformly controlled as the cutoff grows. | Test-function pairing terms `<w,T>` in the cluster-expansion tail. | Any `K > 0`, sufficiently large `m_0`, sufficiently small `epsilon` depending on `K`, and `lambda` in the closure of sector (18.1.6). | Uniform in `lambda`, `m_0`, tail threshold `D`, and the finite-volume cutoff; used to obtain infinite-volume results. | The sum of terms whose associated region X has area at least D is bounded by the n-dependent test-function norm used for w times `exp(-K * (D - n))`. | `Ambient.correlationComplexAlongExhaustion_general_norm_le_uniform`. | None: Lean bounds finite-stage lattice Ising correlations for arbitrary finite `A` and complex `beta` in a high-temperature ball by `generalRatioBoundFun`, not continuum cluster terms. | Architectural analogy only. |
